...
查看 jupyter notebook 地址密码
- 启动 jupyter 服务，提示输入 token
- 另开窗口，输入：jupyter notebook list

```sh
Currently running servers:
http://localhost:8888/?token=abc123def456 :: /path/to/your/notebook
```
...
# Enter password: 
# Verify password: 
# [JupyterPasswordApp] Wrote hashed password to /home/luban/.jupyter/jupyter_server_config.json
```
...
### Web Terminal Ctrl 失效


【2026-4-16】[Ctrl-c doesn't seem to work in web terminal](https://github.com/jupyter/notebook/issues/1227)
- Mac web端使用Jupyter时，执行 `tail -f log.txt`,  ctrl+c 方式失效，无法终止
- 原因：快捷键被占用
