...
tags: 深度学习 对话系统 多轮 FSM 有限状态机 陈蕴侬 AIML DAG 调度 diet rasa dmn
excerpt: 对话管理器技术总结
author: 鹤啸九天
mathjax: true
...
**DMN = Decision Model and Notation**，中文标准译名：**决策模型与符号规范**，由国际标准化组织 OMG（Object Management Group）制定的**业务决策标准化建模语言**，主流稳定版本DMN 1.3/1.4，和BPMN（业务流程建模）、CMMN案例流程配套，专门用来把企业复杂业务规则、人工决策逻辑画成可视化标准图纸，做到决策逻辑可读、可落地、可自动化。

BPMN管**流程步骤怎么走**，DMN管**每一步该怎么判断、怎么选**。

### 解决什么难题

企业大量模糊决策问题：
1. 风控授信、保费定价、优惠折扣、理赔审核、人事定级、费用报销规则全靠老员工口头经验；
2. 规则散落在Excel、文档、代码注释，业务、产品、开发、测试理解不一致；
3. 修改规则要改代码，上线慢，不懂代码的业务人员没法自主维护规则。

DMN让**非技术的业务人员**就能画出决策逻辑，IT可直接对接规则引擎执行，实现**业务说了算、规则可配置**。

典型落地场景
1. 金融：信贷风控审批、信用卡授信、保险保费核算、理赔拒赔判定；
2. 电商：满减优惠、运费模板、活动门槛、优惠券发放规则；
3. 政务：补贴申领资格、资质审核；
4. 人力：绩效考核定级、入职薪资档位、假期审批；
5. 供应链：采购准入、退换货判定。

### DMN 核心优势

1. **标准化**：全球统一画图语法，跨团队、跨公司无理解偏差；
2. **业务友好**：产品/风控/运营看得懂，不用依赖开发翻译规则；
3. **解耦**：决策逻辑和业务代码分离，改规则只改DMN，不用发版本；
4. **可自动化**：对接Drools、Camunda、OpenRules等规则引擎，自动执行决策；
5. **可审计**：每条决策有明确规则行，方便溯源合规（满足金融、监管审计）。

### DMN 问题

DMN 代表决策模型与符号表示法。这是一种由对象管理组(OMG)制定标准的技术，已被广泛应用于各个行业。企业利用 DMN 来设计决策模型，以实现决策过程的自动化。DMN 作为一种通用语言，有助于将业务需求与 IT 系统相结合，确保遵循可重复的业务规则以及决策管理流程。这种符号表示法能够提高业务效率，降低人为错误的风险，并确保决策模型在组织内部可以相互替换使用。

DMN 核心要素包括：
- 决策表——一种简单且直观的表示方法，用于描述由输入、条件和输出构成的决策过程。
- 友善表达语言（FEEL）——用于描述决策表中的条件，以便这些条件能够被执行。
- 决策需求图（DRD）——用于描述那些无法用单一表格来表述的决策情况。


### 常见工具搭配

1. 流程引擎：Camunda、Flowable原生支持DMN，BPMN流程里直接调用DMN决策；
2. 建模工具：Draw.io、Lucidchart、Bizagi支持DMN绘图；
3. 规则引擎：Drools、IBM ODM解析DMN文件自动跑规则；
4. 交付：DMN文件为XML格式，可版本管理、导入导出。

### 核心组成

核心组成
1. 决策需求图 DRD（Decision Requirements Diagram）
2. 决策表 Decision Table（DMN最常用组件）

#### 决策需求图 DRD（Decision Requirements Diagram）

宏观架构图，梳理决策依赖关系，元素分为4类：

|元素|作用|举例|
|----|----|----|
|Decision 决策节点|最终要算出的结果|是否放款、优惠金额、理赔是否通过|
|Input Data 输入数据|决策用到的原始参数|客户年龄、征信分数、订单金额、会员等级|
|Business Knowledge Model 业务知识模型|可复用的规则合集|统一征信打分规则、运费计算通用公式|
|Knowledge Source 知识来源|规则依据文件|监管条例、公司制度、法务文件|

DRD用来理清：做某个决策需要哪些数据、依赖哪些子规则、依据什么制度，解决“决策依赖链路”。

#### 2. 决策表 Decision Table（DMN最常用组件）

把多条件判断整理成规整表格，替代杂乱的if-else嵌套，是DMN落地最多的形式。

结构组成
- 输入列：判断条件（年龄、逾期次数、收入）
- 输出列：决策结果（拒绝/正常放款/限制性放款）
- 规则行：每一行是一套完整判断逻辑
- 命中策略：多规则匹配时怎么处理（唯一命中、多条累加、优先级等）
...
好处：一眼看懂全部分支，不会漏场景，业务可直接改表格，不用改代码。

###  配套语法：FEEL

DMN自带表达式语言 **FEEL（Friendly Enough Expression Language）**，用来写计算公式、复杂条件：
- 不用Java/Python语法，贴近自然语言；
- 支持加减、区间、列表、布尔判断；
- 决策表、DRD全部可以用FEEL写逻辑，规则引擎可直接解析执行。

示例FEEL：`age between 22 and 60`、`credit_score >= 600`
