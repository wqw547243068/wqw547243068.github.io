...
categories: 编程语言
tags: Tensorflow Python 深度学习 可视化 wandb swanlab
...
【2026-6-29】工具选型速记 💡
1. **只看训练loss、本地单机调试** → TensorBoard
2. **海外团队、大量超参搜索、云端协作** → WandB
3. **国内大模型训练、国产框架、需要手机远程看训练** → SwanLab
4. **单独解析GGUF/ONNX模型结构、核对权重算子** → Netron（和其他工具互补，不替代）
5. **企业生产、完整机器学习流水线、模型归档部署** → MLFlow
...
- 复现模型：Wandb更有利于复现模型。Wandb 不仅记录指标，还会记录**超参数**和**代码**版本。
- 自动上传云端：
...
[SwanLab](https://swanlab.cn/) 使用顶尖的 MLOps工具，改进AI开发团队模型训练流程
- 官方文档 [guide_cloud](https://docs.swanlab.cn/guide_cloud/general/quick-start.html)
- GitHub [SwanLab](https://github.com/SwanHubX/SwanLab)

![](https://swanlab-docs-1301372061.cos.ap-beijing.myqcloud.com/assets/zh/guide_cloud/general/quick_start/quick-start.png)
