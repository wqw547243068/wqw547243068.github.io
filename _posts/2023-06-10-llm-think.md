...
方法
- 基于`IE`：将知识限定于可以用三元组形式表示的关系和事件，基于额外的IE模型进行抽取，接着使用额外模型进行验证。
  - 可能存在IE模型的错误传播问题。
  - 知识被限定在三元组形式。
  - 缺点：
- 基于`QA`：
...
- 基于`NLI`：基于NLI的方法通过利用NLI模型评估是否Source Information可以蕴含Generated Text，从而评估是否出现了幻觉现象。
  ...
  - 缺点：
- 基于`Factualness Classification Metric`：标注/构造一批和幻觉/事实有关的数据，训练检测模型，利用该模型评估新生成文本的幻觉/事实问题。
  - Wizard of Wikipedia: Knowledge-powered conversational agents
- 人工评估：目前为止最靠谱的
- 此外还可以依靠LLM打分（比如GPT4，但是GPT4也存在着严重的幻觉问题，除非retrival-augment，但是检索回来的信息也有可能是错误的）
