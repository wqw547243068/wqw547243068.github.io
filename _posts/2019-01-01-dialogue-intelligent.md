...
举例，根据实际研发和市场运作的经验，我们发现有一个算得上“圣杯属性”是特质是：“高度个性化”。
...
为什么现在的助理产品都是坑？很多团队不是底层的算法差，而是<span style='color:red'>团队对产品的理解有问题</span>。

要满足C端用户的需求，确实非常难。10次使用，有一次因为任意原因的失望，用户心理就会开始有疑虑。从体验上来看
- 在用户熟悉的场景下, 全面理解用户提出的需求；
- 在用户自身不清楚场景下，协助用户挖掘需求；
- 获得需求后得帮助用户做决策，并最终呈现结果。

以此来看，对话式的agent就得至少满足以下功能：
...
看看结果，首先看三家的回复，从左到右分别是苹果的SIRI, 微软的CORTANA, Google的ALLO。
