...
PPO 类型
- RLHF: 数据集是人类标注
- RLAIF: 数据集是 GPT，PaLM2等模型标注

PPO 缺点: 训练不太稳定，效率低

非RL技术
- DPO: 直接构造偏好数据集进行优化
  - 跟PPO主要区别: 不用单独训练reward模型，直接构造偏好数据集,对齐原来的模型即可
...
直接合成数据，用 MCTS（exploration and exploitation保证多样性和质量）等方式造完数据，直接DPO开始训练就完了，完美的数据闭环，alignment的论文有一个特点，就是数学推导过程超级的长
