---
layout: post
title:  大模型评测 LLM Evaluation
date:   2023-06-03 12:00:00
categories: 大模型
tags: gpt chatgpt 评测 数据集 coze openai elo agent 高考
...
【2026-6-5】Current Best Open Source Models right now: [X 帖子](https://x.com/i/status/2062905703342420307)
...
- 10th: Minimax 2.7 > Best 

|排名|模型名称|亮点定位|
| ---- | ---- | ---- |
|1|Kimi 2.6|Best all-round model（综合性能最优）|
|2|Deepseek v4 pro|Best instruction following + API cost（指令遵循&API性价比最优）|
|3|Minimax M3|Best OS coding agent（系统代码智能体最优）|
|4|GLM 5.1|Great at long-horizon tasks（超长时序任务表现优异）|
|5|MiMo v2.5|Best harness integration（框架集成适配最优）|
|6|Deepseek v4 flash|Great at long analysis + speed（长文本分析+推理速度优异）|
|7|Qwen 3.7 Max|Best multimodal capabilities（多模态能力最优）|
|8|Qwen 3.6 27B|Best on-device dense model（端侧稠密模型最优）|
|9|Gemma 4 12B|Best SLM on-device（端侧小参数量模型最优）|
|10|Minimax 2.7|备注信息不全|
...
【2026-6-17】[Independent analysis of AI](https://artificialanalysis.ai/) 澳大利亚创业公司 Artificial Analysis 领先的独立AI基准测试和见解提供商
- 覆盖多个维度的榜单：速度、能力、费用
- 专项能力榜单：Coding Agent
- 各个评测集上效果
- 各家模型迭代历史

[历史](https://www.datalearner.com/ai-organizations/artificial-analysis)
- 2024 年，联合创始人 George Cameron 和 Micah-Hill Smith 在悉尼地下室启动这个项目，最初旨在比较 AI 模型和托管提供商。
- 2026 年初，获得 Nat Friedman 和 Daniel Gross 通过 AI Grant 计划的种子资金后，发展成一家成熟的公司。
- 如今，Artificial Analysis 是领先的独立 AI 基准测试公司，其使命是“理解 AI 景观，选择最适合您用例的模型和提供商”。

核心理念强调独立性
> 不与模型创建者或提供商有任何关联，确保评估保持公正。

在智能、质量、性能和成本等方面进行广泛基准测试，使用优先考虑真实世界任务的方法，而不是抽象测试。

这种方法在 VentureBeat、《经济学人》和 Latent Space 播客等媒体中获得认可，被誉为“AI 基准测试的金标准”。

LLM 排行榜，根据关键指标对模型进行排名：
- 智能：强调知识可靠性、幻觉率和长上下文推理等功能。
  - 10 项评估，如 GDPval-AA（测试 44 个职业的真实世界经济价值任务，使用 web 和 shell 访问进行代理式执行）、Terminal-Bench Hard（代理式编码和终端使用）和 GPQA Diamond（科学推理）。分数越高表示整体智能越强，顶级模型如 Claude Opus 4.6 Adaptive 和 GPT-5.2 领先
- 性能和速度：每秒输出token和延迟
- 价格：每百万token的混合成本
- 上下文窗口：海量输入支持

还提供专业基准测试
- 多语言 AI 模型基准（通过 Global-MMLU-Lite 覆盖语言）
- Image Arena（文本到图像，使用盲投 ELO 分数）
- Video Arena（包括音频能力，Veo 3.1 Preview 在文本到视频和图像到视频类别中领先）。
- 开放性指数，对模型透明度打分（最高 18 分，包括预训练数据披露等方面），并标注开放权重模型（商业使用受限需付费许可）。
...
【2026-7-6】全方位测评（效果、时延、价格等），实时性也很好。
- Fable 5在能力上第1，但费用、时延贼高
- 其次是opus 4.8，GPT-5.5
- 国内GLM 5.2 MAX最强，全球第7，Qwen3.7 Max第11，Minimax-M3第13，Deepseek V4 Pro第14

当然也没考虑PE自适应的问题（可能潜在假设PE不该过重，要agent友好）

artificialanalysis 甚至能显示供应商能力、显卡加速、不同国家模型整体表现、同一品牌模型[进化趋势](https://artificialanalysis.ai/trends)

#### LLM Stat

【2026-4-22】[LLM Stat](https://llm-stats.com/) 大模型各类榜单（文本/语音/图像/视频），评测集，AI/LLM最新国外资讯
- 各个模态维度的榜单非常详细
- leaderboard还有趋势榜，同一个模型在多个任务（Chat、Website、Image、Video、Games、SVG）上排名变化



#### Arena

【2026-7-16】[Arena](https://arena.ai/)
- 排行榜 [leaderboard](https://arena.ai/leaderboard/agent)
- 竞技场 [battle](https://arena.ai/text) 提供多种模式，对战、agent
...
LMSYS Org 的 [Chatbot Arena](https://arena.lmsys.org/)（UC伯克利主导，前小羊驼发明者, [twitter](https://twitter.com/lmsysorg)）的研究人员又搞了个大新闻——大语言模型版排位赛！
...
【2024-6-13】 [字节扣子搭建大模型擂台：匿名PK效果，用户当裁判，跑分时代要结束了](https://mp.weixin.qq.com/s/T_0jxSC4f_XXXuiqIlQGNA)

字节跳动的**扣子**（coze.cn），给国产大模型们组了个大局：同一个“擂台”上，两个大模型为一组，直接以匿名的方式PK效果

[模型广场](https://www.coze.cn/model/arena)与大模型擂台 `Chatbot Arena` 类似
...
主流LLM能力榜单 
- 【2026-4-20】[LLM Rank](https://llmrank.cn/) 主流LLM能力榜单，含费用
- 【2026-4-22】[LLM Stat](https://llm-stats.com/) 大模型各类榜单（文本/语音/图像/视频），评测集，AI/LLM最新国外资讯
...
【2023-9-25】Paper: [Pretraining on the Test Set Is All You Need](https://arxiv.org/pdf/2309.0863)

文章以一项大胆的实验为开端，采用一个**高质量**数据集
- 然而，这个数据集并非人为合成，而是源自 huggingface上 众多评估基准数据

完成基于 Transformer 的语言模型的预训练，模型被命名为 `phi-CTNL`（发音为“fictional”）。
...
这篇文章不是在搞笑，而是讽刺那些**不知道眼前有坑**的学术研究。

尽管评估和基准测试对于语言模型的发展至关重要，但这个领域经常受到夸夸其谈的宣传，却忽视了**数据污染**的潜在风险。
- 含蓄地点名了一些模型，例如: `phi-1`、`TinyStories` 和 `phi-1.5`。
  - Phi-1.5模型的数据污染问题很严重
...
【2024-5-17】[国内外百余大模型测评，国产大模型更懂中国用户](https://news.bjd.com.cn/2024/05/17/10776633.shtml)，[完整解读](https://www.jiqizhixin.com/articles/2024-05-18-5)

5月17日，智源研究院举办大模型评测发布会，解读国内外140余个开源和商业闭源的语言及多模态大模型的能力评测结果。
- 测评首次引入人类学生熟悉的**学科测试**，让AI考生和三年级到高三学段的人类考生平均水平一较高下。
- 根据大模型企业在语言模型、多模态理解与生成模型以及K12学科测验上的综合表现进行评比后，**阿里云、百度、字节跳动、智谱华章、百川智能**跻身“优秀”行列。
...
中文语境下，国内头部语言模型的综合表现已接近国际一流水平，但存在**能力发展不均衡**的情况。

多模态（多模态指视频、语音和文本等多种信息表现形式）
- 理解**图文问答**任务上，国产模型表现突出，特别是在中文语境下的文生图能力与国际一流水平差距较小。
  - 图文问答: 通义`Qwen-vl-max` > `InternVL-Chat-V1.5` （上海AI实验室） > `GPT-4` > `LLaVA-Next-Yi-34B`（UW Madison WAIV） > `Intern-XComposer2-VL-7B`（上海AI实验室）
- 生成
  - 文生图: 
    - `DALL-E3` > `CogView3`（智谱） > `Meta-Imagine`（meta） > `文心一格` > `Doubao-Image`（字节）
  - **文生视频**: 
    - 对比各家公布的演示视频长度和质量，美国OpenAI公司的视频大模型Sora有明显优势，其他开放评测的文生视频模型中，爱诗科技研发的国产模型PixVerse表现优异。
    - `Sora` > `Runway` > `PixVerse`（爱诗科技） > `Pika` > `VideoCrafter-V2`（腾讯）

[img](https://image.jiqizhixin.com/uploads/editor/dd62ec30-b861-4d99-8d65-cac211b7156d/640.png)
- ![](https://image.jiqizhixin.com/uploads/editor/dd62ec30-b861-4d99-8d65-cac211b7156d/640.png)

语言模型评测结果 [img](https://img1.bjd.com.cn/2024/05/17/d45295b80e5c29a4c97833d5c77501febaa226e4.png)
- **主观评测**结果显示，在中文语境下
  - 字节跳动豆包 `Skylark2` > OpenAI `GPT-4` > `文心一言` > `Kimi` > `GLM-4`
- **客观评测**中
  - OpenAI `GPT-4` > 百川智能`Baichuan3` > 百度`文心一言4.0` > 智谱华章`GLM-4` > 月之暗面`Kimi`
- ![](https://img1.bjd.com.cn/2024/05/17/d45295b80e5c29a4c97833d5c77501febaa226e4.png)
...
大模型表现 [img](https://img1.bjd.com.cn/2024/05/17/409d7c9476d3aecfc013bf35e4cff482fd79cbef.png)
- 通义`Qwen-vl0max` > `文心一言`（Ernie-bot-4） > `GLM-4` > `Baichuan3` > `GPT-4` 
- ![](https://img1.bjd.com.cn/2024/05/17/409d7c9476d3aecfc013bf35e4cff482fd79cbef.png)
...
| 一级评测大类 | 细分评测数据集 | 数据集说明 |
| ---- | ---- | ---- |
| 通用能力 | MMLU | 多学科 |
| 通用能力 | Hellaswag | 常识推理 |
| 通用能力 | ARC | 科学推理 |
| 通用能力 | Winogrande | 代词消解 |
| 通用能力 | TruthfulQA | 事实性 |
| 数学能力 | GSM8K | 小学数学 |
| 数学能力 | MATHQA | 代数应用 |
| 数学能力 | MATH | 竞赛数学 |
...
| 中文能力 | CMMLU | 中文多学科 |
| 中文能力 | C3 | 阅读理解 |
| 安全性 | TruthfulQA | 事实真实性校验 |
| 安全性 | ToxGen | 毒性内容检测 |
| 微调对比 | Prompt Injection | 提示注入鲁棒性 |
| 微调对比 | Data Extraction | 数据抽取能力 |
| 微调对比 | Model Extraction | 模型抽取防护 |
| 微调对比 | Instruction Hierarchy | 分层指令跟随 |
...
【2026-6-6】阿里智能体评测基准 
- [x](https://x.com/i/status/2062858393820467469)

阿里通义实验室推出智能体评测基准 PawBench v1.0，首次将底座模型与运行框架纳入统一评测体系。

评测针对 9 个大模型与 Hermes、OpenClaw、QwenPaw 三款框架进行交叉测试，包含 150 道真实任务与 4050 个测试单元。
...
优秀的设计甚至能让小模型实现「下克上」：
- 在 Hermes 框架中 GLM 5.1 仅得 68.2 分，而在 QwenPaw 框架下较小规模的 Qwen3.6-35b-a3b 却拿到了 70.4 分。

通过分析运行轨迹，框架表现差异源于对工作区产物缺乏实质校验、工具路径约束宽松以及工具表过大增加了模型决策负担。多数框架在本地专属技能（Skill）的主动发现以及网页搜索的零配置可用性上也存在明显短板。

评测团队提出了框架设计的四项基本原则：
- 充分告知（Inform Fully），明确 cwd 与工作区等环境上下文；
- 按需装备（Equip on Demand），控制工具数量并确保免密搜索等关键工具默认可用；
- 主动监控（Monitor Actively），校验文件写入等任务产物是否真正落地；
- 弹性恢复（Recover Gracefully），在工具异常或产物缺失时提供纠偏续推机会。
...
【2024-9-24】上海AI实验室推出 [OpenDataLab](https://opendatalab.com/), 为国产大模型提供高质量的开放数据集, [说明](https://github.com/opendatalab/opendatalab-datasets/blob/main/introduction%20CN.md)
...
![](https://picx.zhimg.com/v2-123721fe1d7ac402160343690d27974b_1440w.jpg)

##### 2025 高考

【2025-6-8】[2025年数学高考，大模型表现突出](https://zhuanlan.zhihu.com/p/1915161085641724791)
- 实验源自 [IT之家](https://view.inews.qq.com/a/20250608A05OZ900)
...
- ![](https://pic4.zhimg.com/v2-0d5ee56f67b2636833f540692acc0951_1440w.jpg)
...
[OpenCompass](https://opencompass.org.cn/) 是上海AI Lab 开发的开源大模型评测体系，国内最全面、使用最广泛的大模型评估框架。
...
```sh
# Python 3.10/3.11 直接源码安装！千万别 pip install opencompass
git clone https://github.com/open-compass/opencompass
```

OpenCompass 为 MMLU 提供了多种评估方式：
- mmlu_gen.py：生成式评估，适合 instruct 模型
- mmlu_ppl.py：困惑度评估，适合 base 模型
- mmlu_zero_shot_gen.py：零样本生成式评估
- 带 nocot（无思维链）：如 aime2024_0shot_nocot_*.py→ 若论文强调“零样本”或“禁用思维链”
- 带 llmverify/llmjudge：如 aime2024_llmjudge_*.py→ 需论文明确使用 LLM 作为评分器。
- 带随机哈希后缀：如 aime2024_gen_6e39a4.py→ 通常是临时实验文件，论文倾向用稳定版本（无哈希或统一哈希）。

参考：
- [qwen.py](opencompass/configs/datasets/collections/leaderboard/qwen.py)
- [qwen_chat.py](opencompass/configs/datasets/collections/leaderboard/qwen_chat.py)
