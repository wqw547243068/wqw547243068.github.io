...
<iframe src="//player.bilibili.com/player.html?isOutside=true&aid=116453927814340&bvid=BV1BFouBYERu&cid=37753913393&p=1" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true" height="100%" width="100%" ></iframe>
...
#### 【2026-9-7】Cornell：Skill组织方式，SubAgent模式更好

Skill 越装越多，Agent 可能先被自己的「说明书」塞满 Context。

【2026-9-7】Cornell 和 Microsoft Research Cambridge 论文
- 论文 [Subagents vs Agent Skills: Executing Reusable Knowledge for Long-Horizon Agentic Tasks](https://arxiv.org/abs/2609.09233)

当Skill包具备清晰的输入输出，且指令中封装了过程性知识时，SubAgent执行方案效果优于传统Agent Skill 执行方案。

代价是额外通信开销：
- 主智能体与子智能体之间的协同交互需要消耗更多 token。

实验结果证明，可复用知识能带来多大增益，不只取决于知识本身的内容，还取决于知识的组织方式与调用方式。

两种常见Skill 用法，长任务会被迅速放大
- （1）Agent Skill 直接加载：调用 Skill 后，把 SKILL.md 直接塞进主 Agent 上下文里；—— 简单知识
- （2）SubAgent 分割：把 Skill 交给独立 Subagent，让它在 Context 里完成任务，最后只把结果返回主 Agent。

主 Agent 一旦同时背着 Skill 指令、工具输出和历史轨迹，Context 很容易越来越重。
Subagent 则能把一部分执行过程隔离出去。

当 Skill 本身写得足够清楚，明确输入、步骤和输出时，交给 Subagent 效果更好。
- 以 GPT-5.3 Codex 为例，95.3% 的任务都降低了峰值 Context 长度。
- 代价：Context 更干净了，但 Agent 之间来回传递信息，会增加总 Token。

把 Skill 理解成一种「接口」。 简单知识可以直接放进 Context，复杂流程更适合封装起来独立执行。
...
【2026-7-4】[skill进化综述](https://mp.weixin.qq.com/s/UGeoZdYEp6_ArA3ceuvvmQ)
...
SkillOpt 思路极其反直觉：
- 把技能文档当成神经网络的权重参数，用前向传播 + 反向传播的逻辑，在纯文本空间做**梯度下降式**的优化。
- 最终拿到 `best_skill.md` —— 人类可读、可审计、可版本管理、可跨模型迁移。部署时往 prompt 里一贴就完事，零额外推理开销。

把传统 ML 整套优化哲学完美平移到自然语言空间，产出的却是一个人人都能读懂的 .md 文件。
- 前向传播收集证据、反向传播更新参数、学习率控制步长、验证集早停

SkillOpt 最漂亮的地方: <span style='color:red'>把 DL训练核心概念一对一刻进了文本世界</span>：
...
图解 [skillopt_loop.html](https://wqw547243068.github.io/wqw/demo/skillopt_loop.html)

<iframe src="https://wqw547243068.github.io/wqw/demo/skillopt_loop.html" scrolling="yes" border="0" frameborder="no" framespacing="0" allowfullscreen="true" height="900" width="100%"> </iframe>
...
#### 【2026-7-3】SkillOpt-Lite

【2026-7-3】新加坡南洋理工 LMMs-Lab 与微软推出 SkillOpt-Lite
- [SkillOpt-Lite: Better and Faster Agent Self-evolution via One Line of Vibe](https://arxiv.org/pdf/2607.03451)
- 代码开源：[SkillOpt-Lite](https://github.com/EvolvingLMMs-Lab/SkillOpt-Lite)

SkillOpt-Lite 最关键的认知转换
- 既然优化器能读 trace，就没有必要先把轨迹压成固定形状的“伪梯度”；
- 更自然的做法是让 coding agent 像工程师一样浏览日志、定位共性故障并提交最小 d

现有方法依赖复杂流水线。未解决的根本问题：**技能优化的最小可行流水线应当由什么构成？流水线中每一个模块都必须有理论依据或实证必要性支撑。**

借助`零阶`（Zeroth-Order, ZO）优化对技能优化进行形式化建模，将经典零阶优化方法（中心差分、信赖域）与近期相关研究建立对应关系。

研究发现：与经典零阶优化中无导向的数值扰动不同，**技能轨迹**可提供具备可解释性的调试反馈。

以 Claude Code 设计思想与PAC学习理论为基础，确立保障算法收敛性与泛化能力的三条核心原则：基于文件系统的轨迹探索、共识属性挖掘、独立验证门控。

在剔除冗余模块后，提出 **SkillOpt-Lite**。

该算法收敛速度更快，性能优于完整版 SkillOpt：
- 在 LiveMath 评测集上，GPT-5.5 提升8.8个点位，GPT-5.4-nano 提升25.4个点位；使得轻量的nano模型性能超越经原版SkillOpt优化的标准GPT-5.4。

最后，将该框架集成至VSCode Copilot等工程级代码智能体，开发者仅需一行指令即可迭代优化智能体技能。由于本框架将智能体所有组件统一视作可编辑标准代码，这套最小流水线可以自然拓展至全调控优化（HarnessOpt）。在SpreadsheetBench基准测试中，HarnessOpt使GPT-5.4-nano取得0.7758的准确率，优于采用标准流水线的更大模型GPT-5.5（0.7620）。


【问题】

【问题 1】流程状态：一轮 Lite 如何更新 skill？

某轮开始时，当前活跃技能 S_t 的验证分为 72，历史最佳技能 S* 的验证分为 75。优化器面对本轮 40 条独立轨迹文件，提出候选技能 A，验证分为 74；下一轮又提出候选技能 B，验证分为 73。

请说明：从轨迹文件到候选 A 需要经过哪些关键步骤；A 和 B 分别会被接受还是拒绝；两轮结束后活跃技能、best_skill.md 和历史日志分别是什么状态。

从轨迹文件到候选 A 的过程：
- 轨迹暂存：将 40 条 rollout 分别保存为独立文件，包含规划过程、工具调用、环境状态和任务得分。
- 轨迹探索：优化器利用文件工具扫描目录，按错误类型或任务阶段聚类，选择代表性成功与失败轨迹深入阅读。
- 共识挖掘：识别跨任务重复出现、影响较大且能由 skill 修复的失败模式，排除单个样本的偶发现象。
- 最小补丁：把共识转化为针对当前 skill 的紧凑 diff，生成候选 A。
- 验证门控：在独立验证集上评估 A，并分别与当前活跃技能和历史最佳技能比较。

状态变化：
- A 得分 74，高于当前活跃技能的 72，因此 A 被接受，成为新的活跃技能。
- A 的 74 低于历史最佳的 75，因此 不覆写 best_skill.md。
- 下一轮 B 得分 73，低于当前活跃技能 A 的 74，因此 B 被拒绝并归档。
- 两轮结束后：活跃技能：A，验证分 74；
- best_skill.md：仍为原历史最佳技能 S*，验证分 75；
- 历史日志：记录 A 的接受结果，并归档 B 的补丁、验证分和拒绝原因。

核心区别：活跃技能决定下一批 rollout 使用哪个版本，best_skill.md 保存迄今验证分最高的生产候选，两者不一定相同。


【问题 2】 证据边界：哪些结论可以成立？

判断下列五个结论是否得到本文实验支持，并为不成立的结论各写一句修正表述：
- A. Pilot 说明文件系统探索可以替代独立验证门控。
- B. SkillOpt-Lite 总体优于 SkillOpt，但并未在全部模型—任务单元上形成支配。
- C. best-so-far 验证曲线单调上升，证明实际优化过程没有震荡且已获得更好的渐近收敛率。
- D. SpreadsheetBench 案例说明，优化 harness 在特定任务上可以弥补部分模型能力差距。
- E. MUSE 自创建技能在覆盖子集达到 85.24%，说明它在全部 75 个任务上已经超过人类技能。

选项	判断	修正或说明
- A	不成立	Pilot 只说明直接探索原始文件可能产生高价值修改。Spreadsheet 从 29.9 降至 15.3，恰好证明独立验证门控不能省略。
- B	成立	Lite 总体表现更好，但 30 个模型—任务单元中有 4 个低于 SkillOpt，因此不是逐单元支配。
- C	不成立	曲线展示的是历史最佳验证分，按定义不会下降；它不能证明每轮候选没有震荡，也不能证明更好的渐近收敛率或测试集泛化。
- D	成立，但需限定范围	SpreadsheetBench 案例说明，在特定任务和配置下，优化工具可见性、状态管理和失败恢复能够弥补部分模型能力差距；不能外推为小模型普遍超过大模型。
- E	不成立	85.24% 只针对成功生成技能的 47 个覆盖任务。全 75 任务中，未覆盖任务按 0 计分，自创建技能为 53.42%，低于人类技能的 59.67%。

Lite 的 4 个回退单元是：
- GPT-5.4-nano / SearchQA：66.9 vs. 68.8；
- GPT-5.4-nano / OfficeQA：15.5 vs. 18.4；
- GPT-5.4-mini / OfficeQA：30.4 vs. 32.1；
- GPT-5.4 / SearchQA：76.3 vs. 77.5。
...
#### 【2026-5-31】上海交大 SkillSmith

已有skill工作的三个痛点
- 工具层固定不变：工具缺陷只能靠 Skill 层绕，导致 Skill 膨胀、脆弱、逻辑重复，工具层的可复用能力缺口无法直接修复
- 各 Skill 独立评估 / 独立变异	忽视技能间协同 / 冲突 / 输出不匹配（实际执行中多技能是同时检索组合的）
- 失败经验被丢弃：反思后只保留 "好技能"、丢弃诊断信息 → 重复诊断 + "失忆式回归"（退役的 Skill 又长回来）

两个问题都基于这篇论文（arXiv:2606.01314）的内容来分析。

问题 1：扩展到三阶及以上高阶协同的挑战

SkillSmith 用 Lotka–Volterra 交互矩阵 ℳ 建模的是 **pairwise（二阶）** 的互补/冲突关系。往三阶及以上扩展，会撞上如下几类硬约束：

| 维度 | 二阶（现状） | 三阶+ 的挑战 |
|---|---|---|
| **搜索/状态空间** | 交互矩阵 O(n²)，可控 | 交互张量 O(n³)，组合数 C(n,3) 指数爆炸；直接破坏论文附录强调的"**有界原子操作控制搜索复杂度**"这一前提 |
| **数据稀疏性** | 每对 skill 需共同出现多次才能可靠估计 | 特定三元/多元组合共同出现的轨迹**指数级稀疏**，Lotka–Volterra 参数在样本不足时本就易失稳，高阶项几乎无法可靠估计 |
| **归因与反模式记忆的冲突** | 交互语义清晰（互补/冲突），可归因到某个 pair | 高阶存在真正的**涌现效应**，无法分解为两两之和；"哪个组合导致失败"变得病态，与反模式记忆 φ=(p,a,c) 的**因果归因**机制直接冲突 |
| **生态动力学稳定性** | 竞争-互惠方程低阶近似，可解 | 高阶相互作用项使动力学高度非线性，交互张量估计难收敛，易出现振荡/不收敛 |
| **治理决策爆炸** | 检索/变异排序/退役由 pair 信号驱动 | 决策空间随阶数膨胀，原子 bundle（同时改 Skill+Tool）的设计复杂度失控 |
| **计算/时延成本** | 每轮进化成本可界定（附录 F 有分解） | 高阶张量估计与推理显著推高每迭代成本 |

**本质矛盾**：SkillSmith 的成功恰恰建立在"**用低阶、可估计、可归因的交互信号做治理**"上。引入高阶协同会同时击穿它的**复杂度可控性**和**归因可解释性**两条支柱——这往往不是"加一项"能解决的，而需要换建模范式（例如用**基于嵌入的隐式协同表征**替代显式高阶张量，或只在**局部小区块**内做高阶建模，而不做全局高阶）。

问题 2：相比传统 Skill Evolution 最核心的改进

**一句话**：核心不是某一个机制，而是**把"工具层固定"这个默认假设打破，将 Skill、Tool、失败记忆纳入同一个可联合进化的外部状态空间（Σ_t = (𝒮_t, 𝒯_t, ℱ_t)）**。

拆开看三层（都是传统方法缺的）：

| 传统 Skill Evolution | SkillSmith 的核心改进 |
|---|---|
| 工具层**固定**，只会"只改 Skill" | **Skill–Tool 联合进化**：统一提案空间 + 原子 bundle，Tool 支持 wrap/edit/compose/split/retire 五种生命周期操作 |
| 每个 Skill **独立评估、独立变异** | **生态动力学治理**：用交互矩阵 ℳ 考虑技能间互补/冲突，决定检索、变异优先级、退役 |
| 失败轨迹用完即弃，只留"好技能" | **反模式记忆**：保留失败签名/归因/补救，加速诊断 + 否决重复错误 |

**如果必须挑出"最核心"的那一个点**：是 **Skill–Tool 协同进化**——因为它是和所有前作（Trace2Skill、EvoSkill、D2Skill、CoEvoSkills 等）**区分度最高**的范式跃迁，而且论文消融给了直接证据：**锁定工具层造成单点最大性能下降（WildClawBench −6.8%）**，说明这是整套设计中价值最高的一环。

另外两个机制并非配角，而是让"联合进化"**不跑偏**的必要支撑：
- **生态动力学**负责"该改谁、何时退役"的**选择性治理**（否则库会膨胀到 21+7 组件）；
- **反模式记忆**负责"避免重蹈覆辙"的**安全性约束**（移除后回归率 ×3）。

所以更准确的表述是：**它把 Skill 从"孤立、工具静态"的框架里解放出来，升级为 Skill + Tool + 失败记忆三方协同进化的"生态化自进化"系统**——其中"工具进入进化空间"是区别于传统方法最根本、也是收益最大的一步。


【2026-5-31】上海交通大学等五校合作提出 SkillSmith：Skill 与 Tool 协同进化（arXiv:2606.01314）
- [SkillSmith: Co-Evolving Skills and Tools for Self-Improving Agent Systems](https://arxiv.org/pdf/2606.01314)

在自进化 Agent 中，不再把 Skill 当作**唯一可变**对象，而是让 Skill/Tool/失败记忆三者协同进化—— 把技能库当成一个 "生态系统" 来治理。

通过三种机制联合演化 Skill、Tool 和失败记忆：
- 原子化 Skill–Tool 修改包；
- Skill 生态效用动力学；
- Anti-pattern veto。

三个基准上的实验表明，SkillSmith 持续优于 Skill-only 基线，而且模型规模越大、任务越复杂，优势越明显。

核心扩展
- ① Bundle-based Skill–Tool 协同进化
  - 失败轨迹 → LLM 反思 → 生成原子化 bundle，同时修改 Skill 和 Tool。Tool 有 5 种类型化生命周期操作：
  - Wrap 包装、Edit 编辑、Compose 组合、Split 拆分、Retire 退役
  - 原子性保证 Skill/Tool 变更同时生效，避免 "只改一边" 的无效中间态，可直接修复工具层瓶颈而不把 Skill 描述撑爆
- ② 协同感知的 Skill 生态动力学
  - 受 Lotka–Volterra 竞争 - 互惠方程启发，从执行日志估计技能交互矩阵 ℳ：
  - 正值 = 互补（+ 强化），负值 = 干扰（− 抑制）
  - 全局容量约束刻画 "有限检索 / 上下文 / 维护预算" 下的竞争
  - 由此得到变异优先级信号：不只按孤立性能排优先级，还看它是否抑制了有用邻居、重复了能力、缺少必要互补—— 指导检索、变异排序、退役
- ③ 反模式记忆（Anti-pattern Memory）
  - 结构化记录失败条目 φ=(p, a, c)：失败签名 p、因果归因 a、补救 c。
  - 反思时检索相似历史失败 → 加速诊断
  - 否决与已知失败模式相似的变异提案 → 防止已失败的 Skill-Tool 配置重现
  - 让 Agent 同时积累 "什么有效" 和 "什么该避免"

局限性

论文明确指出三点：
- 依赖基础 LLM 的反思质量如果模型无法正确诊断失败原因或生成可靠修改，系统的演化质量会受到限制。
- 生态模型存在冷启动噪声当 Skill 之间的共现数据较少时，交互关系和协同效用的估计不够稳定。
- 实验覆盖范围有限当前只在三个基准上验证，具身 Agent、多用户 Agent 等更广泛场景仍留作未来工作。


实验对比
- Base Agent；
- 只演化 Skill 的方法；
- Skill 和 Tool 联合演化的 SkillSmith

#### 【2026-7-24】阿里自博弈 Skill Self-Play

让模型自己出题、自己解题、自己训练，最怕的就是把错误题目也一起学进去。

【2026-7-24】Qwen 团队提出 [Skill Self-Play]()，给模型的自我进化加入持续更新的技能库。
- [Skill Self-Play: Pushing the Frontier of LLM Capability with Co-Evolving Skills](https://arxiv.org/abs/2607.22529), [解读](https://zhuanlan.zhihu.com/p/2066222981026780965)
- 代码地址：[skill-self-play](https://github.com/Qwen-Applications/skill-self-play)

痛点: 
> 自博弈训练卡在"多样性"和"可验证性"的死结上

LLM 训练从人工设计与人工标注转向**以交互为驱动**的自主进化模式。然而，现有自主进化方案普遍存在根本性矛盾：**任务多样性**与**校验可靠性**无法兼顾。
- 依托固定环境的训练方式能够获取精准反馈，但会将模型学习限制在狭小领域；
- 开放式自主生成虽拓宽了任务范围，却缺少可靠校验机制，极易出现误导性奖励污染训练闭环的问题。

agent 能力技能是调和这一矛盾的优质折中路径：每项技能可保障模型在特定场景下完成深度可核验的执行过程，同时跨技能动态调度能够保留开放式的丰富任务多样性。

Skill Self-Play 工程哲学: 
> 承认 LLM 自由生成不可靠,但也别把发挥空间全砍掉 —— 框在可验证、可扩展、可演化的技能库里自由发挥。

这套思路对做 LLM 训练的人来说是必读,对做 Agent 系统的人来说也是 —— 不需要完整的 Skill-SP 框架,但"动态维护一个技能库"这个抽象, 自己的项目大概率能用得上。

于是提出**技能自博弈（Skill Self-Play，Skill-SP）** 协同进化框架，由任务`生成器`、`求解器`与`动态技能控制器`三大模块构成。

整套框架依托强化学习闭环协同运转，各组件在持续自博弈循环中共同迭代进化：
- 任务生成器基于动态采样的各项技能生成高难度任务；
- 求解器尝试各类备选方案，不断突破自身能力上限；
- 技能控制器汇总执行反馈，完成技能库的更新与扩充。

这套交互式协同进化方案有效打通了结构化可靠校验与开放式探索之间的壁垒。

过去两年 LLM 训练范式从"人工标注 + SFT"切到"自演化 (self-evolution)", 核心思路让模型自己生成训练数据、自己给自己打分。

但这条路走到 2026 年,卡在非常具体的矛盾上:
- `环境绑定` (environment-bound) 路线 (比如让 LLM 写代码、再跑 unit test): **反馈精确,但能跑的任务域很窄**,基本就是编程和数学证明这类能机器验证的场景。
- `开放式自生成` (open-ended self-generation) 路线 (让 LLM 写一道题、再让 LLM 自己判): 任务域宽,但 **reward 信号不可靠**。模型很容易学会"糊弄判官",reward 一旦被 hacking,训练就崩了。

这两个问题在工业界各占一半的失败案例。`Skill Self-Play` (下文简称 `Skill-SP`) 切入点:
- 技能 (skill) 天然是中间地带 —— skill 是一段可复用的"结构化执行能力",对单个任务来说执行深度足够、可验证性高,换一个任务又可以通过动态路由拓展覆盖域。

Skill-SP 架构非常干净,核心是三个角色在 RL loop 里 co-evolve:

![](https://pica.zhimg.com/v2-7ec8646232fa9b470bb03546e9e659b6_r.jpg)

训练中有三个角色：
- Proposer 根据技能生成刚好有挑战的任务，Solver 尝试解决，Controller 再从执行结果中更新技能库：有效技能继续完善，过时技能被淘汰，新模式则沉淀成新技能。
- 系统还保留一条开放探索路径，避免模型只围绕已有技能反复出题，逐渐失去任务多样性。
- Skill 在这里从推理时的外挂，变成了训练时的课程和验证接口。它既约束任务结构，也帮助系统判断答案是否可靠。

实验覆盖 5 个 3B–14B 开放模型。工具调用能力最高提升 42.9 个百分点，逻辑推理最高提升 12 个百分点。

最终模型在训练和评测时都不读取这些技能包。技能负责组织学习过程，学到的能力最终进入模型本身。

这篇工作我觉得重要的部分是给「模型自我进化」增加了一层可维护的中间结构。

未来的模型训练，可能会逐渐从人工准备固定数据，走向任务、能力和课程一起演化。


在工具调用、逻辑推理两类主流评测基准上开展的大量实验证明：
- Skill-SP 可作为稳健的模型进化引擎，既能稳步拔高优质基座模型的性能上限，也能让初始适配度较差的模型实现大幅效果逆转。

#### 【2026-8-27】Google 推出 WikiSkill

9B 模型，只加一套会持续进化的 Skill，平均表现就超过了没有 Skill 的 27B 模型。

Agent 每轮任务都在产生经验，但这些经验如果一直散落在执行轨迹里，下一轮其实很难真正复用。

【2026-8-27】Google Research 推出 WikiSkill：
- WikiSkill 给 Agent 加一层持续积累的「Wiki」。
- 📎 arxiv: [WikiSkill: Compiling Agent Experience into Persistent Knowledge for Skill Evolution](https://arxiv.org/abs/2608.27454)

整个系统分成三层：
- Raw 保存原始成功/失败轨迹；
- Wiki 把这些轨迹整理成失败模式、有效策略和历史经验；
- Skill 再把稳定下来的知识编译成 Agent 可以直接执行的流程。

每轮 Skill 更新都会经过验证。有效的留下，效果变差就回滚；但 Wiki 不会清空，所以失败的尝试也会成为下一轮进化的经验。

效果

实验覆盖数学、搜索、Excel、长文档和具身任务等 5 个 benchmark、5 个模型。
- Qwen-3.5-9B + WikiSkill 平均达到 47.4%，超过没有 Skill 的 Qwen-3.6-27B 的 39.4%。
- WikiSkill 超过 EvoSkill 和 SkillOpt 

而且 Skill 还能跨模型迁移。有些由其他模型进化出来的 Skill，甚至比模型自己的 Skill 效果更好。


Agent 自进化如果只有「轨迹 → Skill」，很容易反复重新理解过去。中间多一层持续整理的知识库，才能让成功、失败和被拒绝的修改真正积累下来。

Skill 像独立于模型权重的知识资产：可以积累、验证、版本化，还能跨模型迁移。
