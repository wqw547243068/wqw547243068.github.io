...
【2026-5-10】关木@ZeroZ_JQ 的文章 [从 Skills 到分层 Workflow：AI Agent 工程化的下一层抽象](https://x.com/i/status/2053004758508843319)
- 问题：skills 的数量增加，并不必然带来 Agent 行为的稳定性。

怎么办？
- 表面上是能力组织问题；但机制层面其实是工程控制问题。
- 小规模时，问题是“有没有合适的 skill”；
- 规模扩大后，真正的问题变成了：这些能力在什么阶段被调用，由什么责任视角调用，依据什么输入推进，留下什么产物，又由什么机制决定是否可以进入下一步。

skill 库方案存在瓶颈

Unified Skills 真正想解决的事情：不是再做一组 skills，而是把 skills 组织成一套分层 workflow。
- skills 解决“怎么做”，而 workflow 解决的是“什么时候做、由谁做、做到什么程度算通过、失败时退回哪里、过程证据留在哪里”。
