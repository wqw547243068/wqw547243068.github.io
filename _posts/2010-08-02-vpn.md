...
【2026-5-10】各种工具总结：[fanqiang](https://github.com/bannedbook/fanqiang)，含翻墙知识、免费工具、各种浏览器/设备的一键工具包等

【2023-11-18】2022苹果手机iPhone VPN推荐，[中国翻墙软件VPN避坑](https://iosvpn.github.io/)

前几年还不错的 VyprVPN、NordVPN、PureVPN和iVacy，付费购买并在iPhone上安装了，但是最近2年越来越难用
...
【2026-5-1】[中國防火牆2026黑科技曝光](https://www.youtube.com/watch?v=C0WQ5I9YOYQ) 

中國防火牆2026黑科技曝光：AI抓翻牆準確率94%，省級獨立部署天狗系統，商業VPN全軍覆沒！唯一破局方案Reality協議詳解，100萬自建節點如何讓GFW陷入政治困境

| 年份 | 代理 | 设计思路 | 被破原因 | 状态 |
| :--- | :--- | :--- | :--- | :--- |
| 2012 | Shadowsocks | 流量完全随机化，看起来像随机字节流 | 随机 = 异常，GFW按熵分布0.6%误报率 | 【已破】 |
| 2016 | V2Ray / VMess | 多路复用 + WebSocket + TLS | 时间戳重攻击 + TLS-in-TLS 双层握手纹 | 【已破】 |
| 2019 | Trojan | 做一个真 HTTPS，443端口跑真Nginx | TLS-in-TLS 包长模式 + TrojanProbe 实现指纹 | 【已破】 |
| 2022 | VLESS+XTLS-Vision | 取消内层加密，透传真TLS，消除双层 | 仍需自购域名 + 证书透明度 CT 日志暴露 | 【已破】 |
| 2023 | VLESS+Reality | 偷大厂网站握手，连证书都是微软真签的 | 流量模式 + IP 灰名单，协议本身未破解 | 【当前主力】 |

[自建終極防封鎖 VPN VLESS + Reality](https://timely-vacherin-fcf11a.netlify.app/)

【2026-5-3】目前为止可用的免费vpn
1. [Cloudflare WARP](https://developers.cloudflare.com/warp-client/)（1.1.1.1）,全球最大CDN之一，无隐私担忧 。
  - [one.one.one.one](https://one.one.one.one/) 站点
2. [Psiphon VPN](https://psiphon.ca/), 由多伦多大学公民实验室开发的翻墙代理加速器。  
3. [ProtonVPN](https://protonvpn.com/?srsltid=AfmBOorLLOeVLeeFWKbRG2sFleRSKGWWpebWmRIyYNwIFKzUGwCLKcgs), 瑞士公司可发，支持日、美、荷三个节点免费  。
4. [Windscribe](https://windscribe.com/)，每个月10G免费额度，加拿大公司，2016年创立。
  - [下载地址](https://windscribe.com/download?cpid=features&pcpid=features)
5. [TunnelBear](https://www.tunnelbear.com/)，每月2G免费额度。加拿大公司，2011年创立。
6. [Mullvad VPN](https://mullvad.net/)，瑞典，隐私第一，支持加密币支付


system proxy vs tunnel 

区别
- System Proxy（系统代理）：工作在**应用层**，浏览器等软件 “自愿” 走代理；很多软件不走，不全局。
- Tunnel / TUN（虚拟网卡隧道）：工作在**网络层**，系统生成虚拟网卡，**强制接管整机**所有流量，软件无感知，真正全局

选择
- ✅ 只需要浏览器 / 网页工具走代理 → 用 System Proxy
- ✅ 需要游戏、聊天软件、命令行、ping 都走代理 → 必须用 Tunnel / TUN
...
- GitHub [Clash Verge](https://github.com/clash-verge-rev)

上网模式
- 系统代理
- tun模式

【2026-4-24】问题：[[BUG] 退出后无法上网](https://github.com/clash-verge-rev/clash-verge-rev/issues/2853)
- Mac安装Clash Verge，启动全局模式（），所有网络流量都经过 vpn, 推出 Clash Verge后，国内网站+本地服务（localhost）都无法访问

分析
- （1）浏览器还在使用代理，没有还原代理设置
  - 执行：netsh winhttp reset proxy
- （2）本地有两个路由器都开了ipv6dhcp，电脑获取到两ipv6地址的原因

修复方法
- 浏览器安装 SwitchyOmega 插件, [使用方法](https://cloud.tencent.com/developer/article/2391479)
- GitHub 地址：[SwitchyOmega](https://github.com/FelisCatus/SwitchyOmega/releases)
  - 打开 Chrome 浏览器，地址栏输入 [chrome://extensions/](chrome://extensions/)
  - 将 `.crx` 文件拖拽到浏览器中间，出现拖拽以安装的提示
- chrome 商店 [SwitchyOmega 地址](https://chromewebstore.google.com/detail/proxy-switchyomega-pro/nbbinpgfldgjgbmfnjlhglpaihlhmdho?hl=zh-CN&utm_source=ext_sidebar) (需要先vpn)
