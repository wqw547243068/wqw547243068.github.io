...
categories: 大模型
tags: autoresearch ak47 skill jeff 谷歌 归纳 演绎 爱因斯坦 agent
...
|时间|人物|结论|分析|资料|
|---|---|---|---|---|
|2026年H1|`Demis Hassabis`|提出`Einstein Test`，验证AI科研能力|||
|2026年7月9日|DeepMind `Tom Zahavy`|大模型永远无法做真正的科学发现|LLM熟悉`归纳法`（统计模式匹配），快速攻克`演绎法`（形式化证明），但缺少`溯因推理`机制|[Position: LLMs can’t jump](https://openreview.net/pdf?id=klU4737opt)|
|2026年8月14日|斯坦福|autoresearch 缺乏`元认知`闭环||[How Do Agents Fail on AutoResearch: End-to-End Diagnostic Evaluation on 100 Real-World Frontier Research Tasks](https://arxiv.org/abs/2608.14905)|


### 【2026-7-9】大模型没有溯因推理能力

2026 年上半年，`Demis Hassabis` 公开提出 `Einstein Test`，即用 1911 年之前的知识训练 AI，独立发现`广义相对论`。
...
`阿尔伯特・爱因斯坦`在写给`莫里斯・索洛文`的信中，将科学发现归纳为一套**循环**过程：
- 人依托感官经验，获取原始观测数据
- 凭借直觉 “跃迁” 提炼出公理：依靠难以言明、非逻辑的直觉，完成跨越，提炼出抽象公理
- 随后经由逻辑推导完成后续推演。

生成式人工智能熟练掌握`归纳法`（统计模式匹配），且正快速攻克`演绎法`（形式化证明），但缺少`溯因推理`的运行机制(构建具备全新解释力假说的能力)。
- `归纳法`：对上百亿文本符号做统计规律匹配
- `演绎法`：生成严谨形式化证明，例如 AlphaProof 能攻克高难度奥数题
- 大模型完全不具备`溯因推理`能力：观测数据不足时，创造出全新、可解释的假说

科学推理分为三种：

| 推理方式 | 易懂解释 | 例子 |
|---|---|---|
| `归纳`（Induction） | 从大量案例和结果中总结规律 | 根据许多实验数据拟合公式 |
| `演绎`（Deduction） | 从已有规则推出具体结果 | 根据物理定律计算物体运动 |
| `溯因`（Abduction） | 面对现象时提出最有解释力的新假设 | 根据异常现象提出一种新理论 |

以`爱因斯坦`创立`广义相对论`作为可计算研究案例，证实当下流行的 “创造力即数据压缩” 理论（归纳逻辑范畴），无法解释观测数据十分匮乏时诞生的各类科学发现。
...
当时`牛顿`理论在绝大多数场景中表现得非常好，并不存在足够大的数据误差，能够引导一个优化系统主动推翻牛顿时空观。
`爱因斯坦`通过“自由下落的人感觉不到重力”这一思想实验，提出了重力与加速度局部等价的解释。

后续数学工作可以从这个前提出发进行演绎，但“为什么要提出等效原理”并不能直接从已有公式中推导出来。

这种从经验或模拟到新概念的过程称为`溯因跳跃`。
...
【2026-8-14】斯坦福 AutoResearchEval
...
- 数据、代码已全面开源
- Code: [AutoResearchEval](https://github.com/PrentisAI/AutoResearchEval)
- Data: [AutoResearchEval](https://huggingface.co/datasets/PrentisAI/AutoResearchEval)
- Website: [AutoResearchEval-site](https://prentisai.github.io/AutoResearchEval-site/) 包含分析报告，可视化

《LLMs can’t jump》进一步提出：
> 即使修复了这些流程性问题，当前 LLM 仍可能主要是在已有知识空间中归纳、演绎和重组，缺少从经验或模拟中提出新解释、新概念和新公理的溯因跳跃。

AutoResearch 真能做研究吗？把 100 个真实前沿任务交给 8 个模型组合，把每条轨迹逐阶段审，结论出人意料
- 别自嗨、跟风和炒作，甚至幻想中稿了, AutoResearch 离能放心托付那天，还差得远。

AI 长期以来一直在辅助科研工作，但LLM与Agent框架的飞速发展正在重塑科研格局：单个系统如今能够完成从初始假设直至最终成文发表的全流程科研工作，这一范式被称作 `AutoResearch`（自动科研）。

然而现有的评测几乎无法揭示这类智能体的运行机制与故障位置。
- 现有任务范围狭窄；
- 评测只衡量最终性能，忽略执行过程；
- 故障诊断缺乏系统性覆盖，也无法观测产出物层面的细节。

AutoResearch 主要问题
- 任务链条很长，错误容易逐步放大。 早期提出错误假设，或者使用了质量有问题的数据，错误就可能一直传播到分析和最终报告。
- 科研目标通常比较开放。 智能体可能选择容易优化的指标，但这个指标并不能真正回答原始研究问题。
- 结果正确不等于方法正确。 智能体可能通过数据泄漏、循环验证或利用评分规则的漏洞获得漂亮结果。
- 智能体经常同时扮演研究者和审稿人。 既产生结论，又负责审查，容易形成“看起来合理但实际上错误”的自洽解释。

#### AutoResearchEval

现有评估为什么不够？
- 任务太窄。 很多基准集中在编程、机器学习或有标准答案的任务，难以覆盖真正开放的科学发现。
- 只看终点。 一个最终分数无法说明科研过程是否可靠。智能体可能用错误方法得到正确答案，也可能通过迎合评分器把指标刷高。
- 看不到中间产物。 只读最终报告，无法发现报告与代码、数据、日志之间的矛盾。

因此，AutoResearchEval 将评估单位从“最终答案”改成了“完整科研轨迹”。

为填补这一空白，提出 [AutoResearchEval](https://prentisai.github.io/AutoResearchEval-site/) 评测基准，包含 100 项基于已发表前沿成果构建的任务，覆盖七大科学领域以及完整科研生命周期：构思、信息检索、实验执行、数据分析、文稿撰写与成果审阅。

通过评测 8 套「调度框架‑大模型」组合，得到了 800 条自动科研智能体的运行轨迹，并完成过程层级标注。

基于观测结果，本文构建了 `ARFT`（AutoResearch Failure Taxonomy，自动科研故障分类体系），包含 45 条经过实证总结的故障模式。

为实现可规模化、细粒度的故障归因，使用经过人工校准的 “智能体裁判” 流水线，对完整运行轨迹和中间产出物进行审查。尽管故障遍布科研全生命周期，但所有问题指向同一个核心短板：
> 当前智能体缺少**元认知回路**，也就是对照已获取的证据校验自身产出、当结论站不住脚时主动修正、并且反思研究路径是否可靠的能力。

该故障模式在全部 8 套调度‑模型组合（包括本次测试中性能最优的模型）中均复现，说明缺陷根源存在于模型本身，而非某一特定调度框架；

调度层面的优化能否解决该短板仍是一个开放性问题，不在本研究验证范围内。开源发布 `AutoResearchEval` 和 `ARFT`，以期推动自主科学发现方向后续的研究与开发

AutoResearchEval 评测集

从 9 个学科顶刊收集 5878 篇论文，每篇拆成前提、矛盾、动机、方法、实验、关键结论、结论 7 个字段：
- 只把“前提＋待解矛盾”发给模型，答案留作参考答案；
- 按方法能否运行、数据是否公开、是否只是查表过滤，筛出 100 个任务，跨 7 个学科，其中 70 个开放式发现、30 个带明确指标
	
每条任务交给 Claude Code、Codex、Gemini CLI 三个框架配 8 个模型，共 8 个组合，在能执行代码的沙箱里全自动跑完构思、检索、执行、分析、写作、自审 6 个阶段，累计 800 条轨迹、7.3 万次工具调用。

难点在标注：代码、日志、数据、报告全留档，专家先逐条归纳失败模式，直到没有新模式出现，交给一个能翻代码和日志的裁判 Agent 批量标注，与专家一致性 κ 达 0.75，高于只读对话的单次裁判的 0.53
	
📊 800 条轨迹共命中 12712 处失败，平均每条近 16 处。45 种失败模式归入 4 类根因：Grounding 31.0%、Integrity 33.5%、Depth 27.6%，三个认知根因合计 92.1%，工程故障只占 7.9%，其中 Integrity 类最高，指向刷指标、隐藏负面结果的捷径。

最说明问题的是自审：
- 82.5% 的轨迹查出致命问题，却照样把没改的结论发出去；
- 77.5% 的结论，方法站不住脚；
- 78.1% 把负面结果藏起来不报。
...
案例里
- 有的把 README 里的答案直接当方法提交、训练数据没读
- 有的把真搜索藏进永不触发的开关、交上去的是提前写好的常量

🎯 所有失败收敛到同一个根因：
> 缺一个**元认知闭环**，做完不核对产出和证据、发现矛盾不改、也不追问路径是否靠谱。

同一批失败在 8 个组合里反复出现，连 opus 4.8、sonnet 5这类模型也不例外，说明是模型的短板，换个框架也解决不了。

元认知并不是“多思考一下”，而是三个具体动作：
- 核对证据：结论是否与文献、代码、数据和日志一致？
- 发现问题后行动：是否需要修复、重跑、降低结论强度或终止？
- 质疑研究路径：即使结果很好，获得结果的方法是否正当、是否真正回答了原问题？

```
提出计划 → 搜索证据 → 运行实验 → 分析 → 写作
    ↑                                  ↓
    └──── 修复或重规划 ← 判断缺陷 ← 对抗性审查
```

当前系统沿着上方流程向前执行，但下面的反馈回路没有真正闭合。可能生成“这里有问题”的判断，却不会让这个判断改变上游状态。
...
现在有很多科研 Agent ，但常常还是“一次性科研”：
- 做完一个任务，知识不好复用；
- 跑完一次实验，经验没留下；
- 写完一篇论文，下一篇工作还要重新开始。

所以做一个不只是“帮你写论文”的 Agent，而是一个真正能持续做科研的系统。

2026 WAIC OPC 创新赛道，北大自动科研Agent `AutoSci`，面向完整科研生命周期的科研 Agent 系统。
- 开源仓库：[AutoSci](https://github.com/skyllwt/AutoSci)  

AutoSci 核心想法：让科研Agent 具备记忆、执行、协作和自我进化能力。

Karpathy's LLM-Wiki vision, fully realized — wiki-centric full-lifecycle AI research platform powered by Claude Code

传统科研高度依赖人力：科研人员需要在漫长的项目周期中统筹文献梳理、思路构思、实验落地、稿件撰写与返修回复全流程。

AutoSci 是以记忆为核心的智能体系统，可实现从文献收录到审稿答复全科研生命周期自动化；系统在跨项目中留存结构化持久记忆，并持续迭代优化自身作业流程。

四件事：
- 🌐 端到端科研自动化：从文献调研、idea 生成、实验设计与运行，到论文写作和 rebuttal，AutoSci 覆盖完整科研流程。还可以生成学术海报！
- 🧠 跨项目持久记忆：Agent 不再只依赖一次对话上下文，而是把论文、方法、实验、失败经验、review 反馈等沉淀成wiki形式的结构化科研记忆。
- 🔁 越研究越聪明的 SciEvolve：每一次研究运行都不只是产出结果，也会留下经验。AutoSci 可以根据反馈、实验结果和失败模式，持续升级自己的 workflow、skills 和 orchestration。
- 🤖 多 Agent 增强科研质量：复杂阶段，AutoSci 可以调用多个 specialized agents，一起探索 idea、互相 critique、优化实验与写作质量。

总结：
- AutoSci is building the infrastructure for scientific agents that research, remember, reason, and evolve.
- 不是一个“单次科研助手”，而是一个能长期积累、持续进化的科研基础设施。
...
### 【2026-9-11】上海AI Lab [Atria Dawn Preview](https://atria-asi.ai/) 

【2026-9-11】ATRIA 是实验室下的**长周期智能体（Agent）专项项目组** [Atria Dawn Preview](https://atria-asi.ai/)  把研究能力真正沉淀到模型之中
- 技术报告 [Atria Dawn: The Dawn of Agentic Superintelligence](https://github.com/atria-asi/Atria-Dawn-Preview/blob/main/atria-dawn-preview.pdf)
- **744B MoE 稀疏专家架构的文本型 Agent 模型（预览版）**，基座基于智谱 GLM-5.2，256K 上下文窗口，**MIT 协议完全开源权重**。

科研生产力的关键，不只在于能否提出一个问题、生成一段代码或给出一个答案，更在于能否把一个任务持续推进到可执行、可验证、可复现的结果。

当前Agent 模式：Model + Harness
- 模型提供理解、推理、工具选择与局部恢复等可复用能力；
- Harness 负责维护目标、状态、上下文与长程执行过程；
- 环境则提供真实的数据、文件、工具与执行结果。

Atria Dawn Preview 围绕这三者之间不断移动的能力边界展开探索：
- 对于能够跨任务复用、稳定出现的行为，通过训练将其逐步内化为模型自身的能力；
- 对于任务过程中不断变化的计划、状态、权限与恢复信息，则继续由 Harness 显式组织；
- 而代码是否运行、文件是否生成、实验是否成功、证据是否支持结论，始终交由外部环境进行验证。

原则概括为：模型压缩过去的经验，Harness 组织当下的过程，环境决定行动的后果。

面向**长周期科研、工程类 Agent 任务**，不是普通对话模型。

核心训练范式: **Verifiable Experience Pipeline（可验证经验流水线）**：
- 不以 “对话看起来通顺” 作为奖励，而是把模型接入可执行环境，**用最终可核验的真实结果来打分**，解决长轨迹智能体 “看起来对、实际执行失败” 的痛点。

四大能力方向
1. **Discovery（探索发现）**：深度文献检索、证据整理、把科研问题转化成可执行实验方案
2. **Creation（工程创造）**：软件开发、可视化、机器学习系统搭建
3. **Delivery（成果输出）**：把数据、实验结果整理成报告、PPT
4. **Cybersecurity（网络安全）**：漏洞分析、漏洞验证、修复复测（仅限授权环境）

Atria Dawn Preview 并不是围绕某个固定 Agent Workflow 构建的模型，而是通过一套 Verifiable Experience Pipeline 学习跨任务、跨工具复用的行动经验。

- 每一个训练任务都与真实可执行环境相连：模型观察状态、调用工具、产生中间产物，并根据环境反馈持续调整；
- 最终结果则通过测试、指标、文件状态、几何结构、来源证据或人工标准等外部信号进行验证。
- 只有能够将任务、轨迹、产物与验证证据对应起来的经验，才进一步成为模型可复用能力的一部分。

面对陌生领域与复杂科研问题，[Atria Dawn Preview](https://atria-asi.ai/) 不只回答问题，而是尝试参与研究本身：
- 从问题拆解、资料检索和证据梳理，到代码实现、实验验证与结果分析，持续推动一个模糊问题走向可检验、可验证的研究结论。
- 在海量知识与复杂实验空间中识别关键线索、连接不同证据，并帮助研究者更快完成从发现问题、形成假设到验证想法的完整探索过程。

效果

16 项 Agent 基准（工具调用、科研、软件工程、终端、ML 工程、网络安全等）上对标 GPT、Claude、DeepSeek 等前沿 Agent；在 5 项基准上取得最优成绩（DeepSearchQA、BrowseComp、BFCL v4、AutomationBench、CyberGym）

案例

（1）天气预报

全球天气预报系统搭建是一项检验 AI 编程能力的高难度任务：
- 面对超过 100G 的全球气象数据，AI 需要自主设计神经网络，并通过端到端训练实现对复杂大气系统的准确预测。Atria Dawn Preview 在禁用网络搜索的受限环境中完成了这一流程：它设计了一种基于 ViT 骨干的神经网络，参数量超过0.4B，随后进行了45k steps的大规模训练以建模69个气象变量随时间的演化规律。最终，Atria Dawn Preview 搭建的预报系统能够在1分钟内快速预测未来一周的全球天气，并且在使用更少训练数据的情况下，该系统的部分预测指标已经优于英伟达的经典预报模型FourCastNet，这体现了Atria Dawn Preview 强大的 ML coding 能力。

（2）自主科研
- 从研究想法到可运行实验
- 复杂问题中建立完整证据链

① 自主科研场景中，Atria Dawn Preview 能够把一个研究问题进一步推进为可以实际运行和验证的实验。

理解论文中的方法与算法，围绕研究目标选择技术路线、实现模型与训练代码、配置实验环境，并在执行过程中分析报错、调整方案和解读实验结果。对于研究者而言，这意味着许多原本分散在论文阅读、代码实现、环境调试和实验分析之间的工作，可以被组织成一条连续的研究链路，让一个初步设想更快进入实现、验证与迭代阶段。

② 科研深度研究场景中，Atria Dawn Preview 面向开放、复杂且缺少标准答案的问题，主动拆解研究目标，检索和筛选相关资料，比较不同观点，并持续追踪关键证据与来源之间的关系。

无论是快速进入一个陌生领域、梳理技术演进与研究脉络，还是围绕一个新假设寻找支持与反例，Atria Dawn Preview 都能够将分散的信息组织成结构化、可引用、可继续验证的研究结论，帮助研究者从“找到信息”进一步走向建立可信的证据链与研究判断。
