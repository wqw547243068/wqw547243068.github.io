...
【2026-7-30】[Agentic RL超详细指南](https://zhuanlan.zhihu.com/p/2066107786128315926)

很多团队上来就搜 VeRL / AReaL / GRPO，等于还没确诊就开刀。

Agentic RL 是 <span style='color:red'>高成本、高杠杆、高副作用</span> 的手段：
- 做对了, 能把「会跑的 Agent」推到「更会决策的策略」；
- 做错了, 会烧算力、污染模型、还掩盖真正的产品问题。

决策逻辑
- 链路不通 → 先修产品，不要 RL。
- 链路通了但决策笨 → 先示范（SFT/DPO），再考虑 RL。
- 示范学完仍不会探索 / 纠错，且成功可自动验证 → 才认真谈 Agentic RL。

缺的是球场，还是球员？

| 痛点 | 缺什么 | 优先动作 |
| ---- | ---- | ---- |
| 工具调不通、schema 乱、写盘无门禁 | 球场（产品/环境） | 路径 A：约束、沙箱、校验 |
| 偶发会做，但多数时候漏读、乱改、不会根据报错纠错 | 球员习惯（策略） | 路径 B：SFT⁺ / DPO⁺，再评估 C |
| 示范已经很多，离线对齐后仍系统性卡在「探索与纠错」 | 需要实战反馈的策略 | 路径 C：真 Agentic RL |
|「再智能一点」「对标某某论文」 | 目标不清 | 先定量评测，再谈训练 |

时机判断
- 不必「全部满分」才启动，但下面 五组信号多数成立 时，上 Agentic RL 才划算。任一组严重缺失，先补那一组

Agent强化学习训练前四项必备前置条件，针对代码智能体（Coding‑Agent）场景：
1. **终局可验证**：具备客观、低噪音的结果判定标准，是强化学习奖励信号的基础；
2. **硬约束可编码**：设置行为红线，规避模型投机取巧、通过违规操作刷取奖励；
3. **失败可复现**：沙箱环境稳定可控，保证相同任务能够复现失败，模型可以稳定从错误样本学习；
4. **预留测试集评测**：分离训练集与离线测试集，用来检验模型泛化能力，规避过拟合。


| 就绪信号 | Coding Agent 例子 | 为何是前提 |
| ---- | ---- | ---- |
| 终局可验证 | run_tests 绿 / 编译过 / 基准任务 pass | RLVR⁺ 需要低噪声主奖励 |
| 硬约束⁺可编码 | 禁止删测例、禁止改白名单外路径 | 防止「刷分」式黑客 |
| 失败可复现 | 同一题沙箱可重复跑 | 否则学的是噪声 |
| 有 held‑out⁺ 评测 | 训练题与回归题分开 | 否则「训练集涨、线上崩」看不出来 |
...
| 派系 | 核心思想 | 代表算法 | 说明 |
|------|----------|----------|------|
| **策略梯度**<br>Policy Gradient | 直接优化动作选择 | REINFORCE | 鼻祖算法 |
| | | PPO | 工业界扛把子 |
| | | GRPO | 新贵 |
| | | RLOO | GRPO 的表亲 |
| **偏好对齐**<br>Preference Alignment | 好坏对比就行，不用打分 | RLHF | 开山之作 |
| | | DPO | 最简洁优雅 |
| | | KTO | 省钱版 DPO |
| | | SPIN | 自己跟自己比 |
| **自举式**<br>Self-Improvement | 自己产数据、自己训练自己 | ReST / ReST^EM | — |
| | | Expert Iteration | — |
...
【2026-6-13】详见[RL全景图](https://mp.weixin.qq.com/s/e95K6ttsAKaUeP_E0QDRyw)
