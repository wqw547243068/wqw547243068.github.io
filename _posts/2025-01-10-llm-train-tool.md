...
【2026-9-11】实测
- 采用统一的 `Qwen3-4B-Instruct-2507` 模型作为基础模型，通过 WebShop 官方人工交互轨迹完成 SFT 冷启动，再从同一个冷启动 checkpoint 分别进入四个框架执行 GRPO
- swift、roll、verl、areal 对比指标：swift 最优，roll 指标最低

| 排名 | 模型 | 横向排名分 | 绝对效果分 | Task Score | 备注 |
|---|---|---|---|---|---|
| 1 | `MS-Swift` | 95.3 | 75.5 | 77.01 | 综合效果领先，但 95% CI 跨 0，不能断言显著优于第二名 |
| 2 | `VeRL` | 88.8 | 75.0 | 76.27 | 与第一名绝对效果分差 0.5，差异不显著 |
| 3 | `ROLL` | 79.2 | 73.1 | 74.61 | — |
| 4 | `SFT` Baseline | 60.1 | 67.0 | 68.34 | 基线 |
| — | `AReaL` | — | — | — | 尚无已完成评测，不参与综合分和排名 |


数据来自各框架官方仓库的 `requirements.txt` / `setup.py` / `pyproject.toml`。

一、微调框架(8 个)

| 框架 | 训练引擎 | 分布式 | 推理/部署引擎 | HF生态依赖 | 其他关键依赖 |
|---|---|---|---|---|---|
| **LLaMA-Factory** | transformers | DeepSpeed / FSDP / PT | vLLM(部署侧) | transformers / PEFT / accelerate / datasets | bitsandbytes / flash-attn(可选) |
| **ms-swift** | transformers + **Megatron-LM** | DeepSpeed / FSDP / FSDP2 / Megatron(TP/PP/SP/CP/EP) | vLLM / SGLang / LMDeploy(自带export) | transformers / PEFT / accelerate | cuddle(Ascend) / flash-attn / UnSloth(可选) / Liger-Kernel |
| **Unsloth** | **PyTorch 原生**(绕过 transformers 训练循环) | 无(单卡为主) | 无(导出 GGUF / HF 格式) | transformers(仅模型加载/tokenizer) | **Triton**(手写kernel) / bitsandbytes |
| **Axolotl** | transformers | DeepSpeed / FSDP | 无 | transformers / PEFT / accelerate / datasets | flash-attn / liger-kernel(可选) |
| **Firefly** | transformers | DeepSpeed ZeRO | 无 | transformers / PEFT | **支持 Unsloth 加速**(可选) / bitsandbytes |
| **XTuner** | transformers + **MMEngine**(OpenMMLab) | DeepSpeed | vLLM / LMDeploy(部署侧) | transformers / PEFT | mmengine / flash-attn(可选) |
| **torchtune** | **PyTorch 原生**(无 transformers) | torch.distributed(FSDP2) | 无 | **无**(不依赖 HF 生态) | 无额外重依赖,极简 |
| **HF Trainer** | **就是 transformers 本身** | FSDP / DeepSpeed / Accelerate | 无 | transformers / PEFT / accelerate / datasets | 自身即底层 |

二、RL/后训练框架(5 个)

| 框架 | 训练引擎 | 分布式 | 推理/部署引擎 | HF生态依赖 | 其他关键依赖 |
|---|---|---|---|---|---|
| **VeRL** | transformers + **Megatron-LM** | **Ray**(调度核心) / FSDP / Megatron | vLLM / SGLang(rollout引擎) | transformers(模型加载) | Ray / flash-attn / Megatron-LM |
| **OpenRLHF** | transformers + DeepSpeed | **Ray**(调度核心) | vLLM(rollout引擎) | transformers | DeepSpeed / Ray |
| **slime** | **Megatron-LM**(训练核心) | **Ray**(资源调度) / Megatron原生并行 | **SGLang**(rollout+teacher引擎) | **无**(不依赖 HF 训练栈) | Megatron-LM / SGLang / sgl-router / torch_memory_saver |
| **ROLL** | transformers + DeepSpeed | **Ray**(调度核心) | vLLM(rollout引擎) | transformers | Ray / Freshness-Aware PER(自研) |
| **trl** | **transformers**(Trainer 封装) | Accelerate / 简单多卡 | 无 | transformers / PEFT / accelerate / datasets | **无 Ray / 无 vLLM**(轻量定位) |

三、预训练框架

| 框架 | 训练引擎 | 分布式 | 推理/部署引擎 | HF生态依赖 | 其他关键依赖 |
|---|---|---|---|---|---|
| **torchtitan** | **PyTorch 原生**(无 transformers / 无 Megatron) | **原生 PyTorch**(TP / PP / SP / CP / FSDP2 / Asynchronous Checkpoint) | 无 | **无**(不依赖 HF 生态) | 仅 PyTorch + torch.distributed |
...
| 路线 | 依赖栈 | 代表框架 |
|---|---|---|
| **HF 生态系** | transformers + PEFT + accelerate + DeepSpeed/FSDP | LLaMA-Factory / ms-swift / Axolotl / Firefly / trl / OpenRLHF |
| **PyTorch 原生系** | 纯 PyTorch + torch.distributed | torchtune / torchtitan / Unsloth |
| **Megatron 系** | Megatron-LM + SGLang + Ray | slime / VeRL(ms-swift 跨此+HF) |

**2. Ray 是 RL 框架的硬依赖**

VeRL / OpenRLHF / slime / ROLL 四个大规模 RL 框架全部依赖 Ray 做资源调度。没有 Ray 跑不起来。唯一例外是 trl——它用 Accelerate,定位轻量研究,不需要 Ray。

**3. HF 生态是大多数微调框架的基座**

8 个微调框架里 6 个依赖 transformers。只有 torchtune 和 Unsloth 走 PyTorch 原生路线。这意味着:
- 换模型时,HF 系框架天然兼容 ModelScope/HF Hub
- torchtune/Unsloth 要等社区适配新模型架构

**4. 推理引擎是 RL 框架的必选件**

RL 训练的 rollout 阶段需要高速推理生成,vLLM 或 SGLang 是标配:
- vLLM:OpenRLHF / ROLL / VeRL(可选)
- SGLang:slime(核心) / VeRL(可选) / ms-swift(部署)
- slime 对 SGLang 是**硬依赖**,不像 VeRL 可以 vLLM/SGLang 二选一

**5. 依赖越少 ≠ 越差**

torchtitan 几乎不依赖第三方(纯 PyTorch),但它是预训练专用,定位最高端。trl 依赖也少,但定位是研究原型。依赖数量跟框架能力无关,跟定位有关。
...
【2026-9-17】[Meshy: 角色驱动下SPMD范式的RL训练框架](https://mp.weixin.qq.com/s/9uTLgJn8RbbDkBpOo9c-ZA)

### 早期 RLHF 框架

早期 RLHF 围绕**同步训练**展开：模型按固定顺序进行生成、打分和训练，每个阶段之间有着明确的同步边界。如 PPO
...
特点
- 首先，各个阶段之间显式同步，整个 batch 的 rollout 结束后才计算 reward 和 advantage，参数更新完成后下一轮才使用新权重。
- 其次，同步 RLHF 的控制流固定，每轮跑哪些阶段、阶段之间如何依赖，在训练开始前就已确定。
- 第三，轨迹一次性生成，绝大多数样本是单次生成的结果，无需跨轮维护环境状态。

因此，传统 RLHF 的计算组件虽然是异构的（Actor、Critic 和 Reward Model 可能使用不同的引擎和并行策略），但控制流是 **同步、规则、可预测**的。
...
如今，系统还需要处理**异步生成、持续训练、多轮 Agent 以及与外部环境的交互** —— 那些在同步执行中原本隐含的状态、版本和故障边界，现在都必须显式地进行管理。

Single-Controller 非常直接：既然系统遵循单一的全局执行时序，那就用一个顺序程序来编写它。
- Google 的 Pathways（arXiv:2203.12533）最早系统地提出这一设计
- verl 的 HybridFlow 将其与 SPMD 执行结合，成为 RL 训练框架的**主流形态**：顶层控制流集中在一个进程里，每个角色对外表现为可调用的对象，每次调用的背后，控制器把输入按数据并行切分、向所有 worker 进程发起远程调用、收集并合并分片结果。一组分布式计算就这样被折叠成一个普通的函数调用

与同步 RLHF 的三个特征恰好一一对应：
...
Single-Controller 在小规模的同步 RLHF 中几乎没有额外开销：一次 rollout 或参数更新动辄数秒甚至数分钟，中央进程派发调用的开销相比之下可以忽略。

而收益却是实打实的：
- 算法与分布式 infra 实现了解耦 —— 调整算法只需修改 controller，替换引擎只需修改 worker；
- 出了问题，沿着一个进程的调用栈就能定位到当前阶段、当前 batch 和失败的调用。工程上也发展得相当成熟，hybrid engine 让训练和生成共置共享同一组 GPU、按阶段原地切换显存布局，进一步提高同步流水线的硬件利用率。

对中小规模、流程固定的同步训练，这套设计的效率和开发体验至今仍是标杆。

**Single-Controller 缺陷**

Single-Controller 的整套抽象建立在一个假设上：系统只有一个统一的全局执行进度。

当负载走向异步生成、持续训练和多轮 Agent 交互时，这个假设便失效了 —— 同一时刻，系统里可能有多个权重版本的样本在生成，有轨迹停在工具调用上等待恢复，有训练正在消费上一个版本的数据。
...
社区变化
- verl 引入独立的数据队列 TransferQueue，让样本绕开 driver 直达消费者，异步训练则由独立的 fully-async 分支承担；
- slime 把推理端做成独立的 HTTP 服务、数据经对象存储传输，顺序 driver 只剩不到一百行。单控制器框架自己也在一步步把数据传输、推理服务和异步驱动从控制器里搬出去。

问题：如果这些职责都已独立，RL 框架是否还需要保留中央控制器？

### 去中心化

以 verl 为典型代表的 `Single-Controller` 架构，用顺序程序表达异构的分布式计算，有效解决了同步 RLHF 时代最重要的编排问题。但一旦执行不再严格同步、任务开始跨越多轮训练，Single-Controller 就会逐渐从简化系统的抽象，变成数据传输和任务调度的阻碍。

越来越多的系统把通用的分布式编排框架从核心执行引擎的默认依赖中剥离，类似的解耦也发生在推理基础设施中。
- vLLM 在 V1 引擎中为多机张量并行和流水线并行提供了不依赖 Ray 的原生执行路径：各节点分别启动 vLLM 进程，由 PyTorch torch.distributed 建立跨节点进程组。

与其不断扩充 Single-Controller 设计，不如从一开始就将系统建模为一组通过精简接口协作的服务。

【2026-9-8】Meshy 伴随 MiniCPM5-2B（小钢炮）一起开源
- GitHub：[https://github.com/OpenBMB/Meshy](https://github.com/OpenBMB/Meshy)
- 定位：**面向 LLM 后训练、Agent 强化学习的 SPMD 范式 RL 训练框架**，用于 GRPO、DPO、OPD 类离线 / 在线策略优化，MiniCPM5-2B 的 RL 训练就是基于 Meshy+JustRL II 完成

Meshy 基于 TorchTitan 和 SGLang 实现完整的 RL 与蒸馏训练：其不依赖 Ray，也没有中央调度器；服务之间只通过队列中的数据列约定、控制生成进度的 gate 信号，以及少数用于健康监测的 HTTP 管理端点进行协作。

Meshy 将 Inference、Training、Rollout 和 Teacher 等角色建模为独立服务，不再将分布式编排框架作为 RL 系统的中心。所有样本数据通过统一的 TransferQueue 数据面在服务之间流动，控制流程由数据来驱动，拓扑则由各进程在启动时在本地按相同配方推导，因此使得 RL 的流程更接近预训练框架常用的 SPMD 设计。

Meshy 结构天然支持**全异步训练**，同时兼容传统的同步训练。
- 相比 Single Controller 设计，Meshy 避免了诸多的 RPC 调用，摆脱了沉重的分布式框架 Ray，在故障排查、性能、持续集成和代码简洁度上，都具有较大的优势。
- blog：[Meshy: 角色驱动下 SPMD 范式的 RL 训练框架](https://maydomain.notion.site/meshy-blog-zh)
- Github: [Meshy](https://github.com/OpenBMB/Meshy)
...
架构特点
1. **移除中央控制器、无 Ray 依赖**
传统 RL 训练框架依赖 Ray 做全局调度，存在大量 RPC、全局 barrier 同步开销。Meshy 去掉中心调度节点，把 Rollout 推理、模型训练、奖励计算全部建模成**对等 Service 服务**。
2. **数据驱动控制流 TransferQueue**
不依靠主进程派发任务；由数据就绪信号驱动流转，生产者数据直接送到消费者，不经过编排进程。
3. **灵活切换三种模式**
仅修改配置，即可在 **同步 / 异步 / 全异步** RL 训练模式之间切换，适配小规模调试到大分布式集群扩展。
4. **声明式拓扑定义 ServiceGroup**
推理、训练、Teacher、Reward 等角色统一接口，训练拓扑通过配置文件`ServiceGroup`声明，新增算法模块不需要修改框架底层代码，组件可自由组合。
5. **SPMD 分布式原生设计**
各进程本地推导分布式拓扑，权重版本仅保留轻量控制信号，大幅降低通信开销，适合长思维链、Agent 多轮交互 RL 任务
