...
【2024-7-4】 阿里推出训练框架 ms-SWIFT (Scalable lightWeight Infrastructure for Fine-Tuning)
- [Modelscope Swift](https://docs.swanlab.cn/guide_cloud/integration/integration-swift.html)
- [SWIFT Github](https://github.com/modelscope/ms-swift/blob/main/README_CN.md)，官方[文档](https://github.com/modelscope/ms-swift/blob/main/README_CN.md)，[快速开始](https://swift.readthedocs.io/zh-cn/latest/GetStarted/Quick-start.html)
- ![](https://docs.swanlab.cn/assets/logo.vW4ukSLF.png)

## 介绍

`ms-SWIFT` 是 [ModelScope](https://modelscope.cn/) 魔塔社区提供的，基于pytorch的轻量级框架，灵活且高效的微调与推理大型语言模型和多模态大型模型。专为大模型（LLM）和多模态大模型（MLLM）的开发设计，支持从数据处理到模型部署的全流程操作，适用于科研、企业和个人开发者
...
- 支持用 vLLM 和 LMDeploy 加速推理、评估和部署模块，并支持使用 `GPTQ`、`AWQ` 和 `BNB` 等技术进行模型量化。
...
核心功能与特点
- `ms-SWIFT` 框架支持超过 **450 种**大语言模型（如 GPT、Llama、ChatGLM）和 **150 种**多模态模型（如图像、视频、语音任务）。
...
- `ms-SWIFT` 提供丰富工具箱能力，包括推理加速引擎（如 vLLM、SGLang、LMDeploy）和全面的模型评测体系（EvalScope）。开发者可以通过简单的接口实现模型训练、推理、评测和部署，甚至支持多种 tuner 的混合使用。
...
**结论**
- **个人快速微调、模型种类多、社区资料** → `LLaMA-Factory`
- **Qwen 系为主、要训练+部署一条龙、企业/国产化场景** → `ms-swift`

没有谁更好,只有谁更适合，模型、卡、训练目标。

关键维度对比

| 维度 | LLaMA-Factory | ms-swift | 胜者 |
|---|---|---|---|
| **易用性** | YAML + WebUI,上手最快 | CLI + WebUI,CLI 参数多 | LLaMA-Factory |
| **模型覆盖** | 100+,最全 | 600+ 文本 + 400+ 多模态(官方宣称) | ms-swift(数字更大,但注意"支持"≠"深度适配") |
| **多模态** | 有,但不是强项 | **强**,Qwen-VL/InternVL/LLaVA 等深度适配 | ms-swift |
| **训练算法** | SFT/DPO/RLHF/预训练 | SFT/DPO/GRPO 家族/蒸馏/预训练 | ms-swift(GRPO 系更全:DAPO/GSPO/SAPO/CISPO/CHORD/RLOO/Reinforce++) |
| **分布式** | DeepSpeed/FSDP/PT | DeepSpeed/FSDP/**Megatron**(TP/PP/SP/CP/EP) | ms-swift(大模型/MoE 更强) |
| **部署链路** | 配合 vLLM 等,**要自己拼** | **自带 export + vLLM/SGLang/LMDeploy** | ms-swift |
| **量化** | AWQ/GPTQ/BNB | AWQ/GPTQ/BNB/**FP8** | ms-swift |
| **国产硬件** | 一般 | **Ascend NPU 一等公民** | ms-swift |
| **社区/资料** | **最大**,中文教程满地 | 官方文档全,但第三方教程少 | LLaMA-Factory |
| **Qwen 适配** | 通用 | **Day-0 支持,官方背书** | ms-swift |
| **代码可改** | 封装厚,改逻辑难 | 有 Python API,相对可改 | ms-swift(略) |

决策规则

选 `LLaMA-Factory`
1. **个人/小团队,想最快上手 SFT 一个 7B/8B** → LLaMA-Factory。WebUI + YAML, 教程遍地, 踩坑有人陪。
2. **不是 Qwen模型,且不在 ms-swift 重点适配列表** → LLaMA-Factory。模型覆盖更"广而平"。

其余，都选 `ms-swift`
1. **模型是 Qwen 系列** → ms-swift。 ModelScope 官方框架,Qwen 是一等公民, Day-0 支持,出问题能找到官方。
2. **多模态训练(VLM)** → ms-swift。多模态 packing、混模态数据、vit/aligner/llm 独立控制,这些是强项。
3. **GRPO 系强化学习** → ms-swift。GRPO/DAPO/GSPO/SAPO/CISPO/CHORD 全家桶, LLaMA-Factory 这块弱。
4. **训 MoE 或大模型(几十亿+),要 Megatron 并行** → ms-swift。LLaMA-Factory 没接 Megatron。
5. **有 Ascend NPU 或国产化合规要求** → ms-swift,没得选。
6. **要训练完直接部署,不想拼链路** → ms-swift(`swift export` → `swift deploy`,一条命令)。


ms-swift 官方宣称"600+ 文本 + 400+ 多模态",数字碾压 LLaMA-Factory。但**"支持"≠"深度适配"**: 很多模型是走 transformers 通用路径跑通的,不一定有专门优化。LLaMA-Factory 数量少,但对热门模型(Llama/Qwen/Mistral)的适配同样不差。
...
原因：官方[issue](https://github.com/modelscope/ms-swift/issues/1629)
...
【2026-1-28】[ms-swift命令行参数详解](https://blog.csdn.net/weixin_33256096/article/details/157458410)
...
【2026-9-9】**共 16 个命令:12 个主命令 + 4 个 Megatron 命令。**

| 场景 | 命令 |
|---|---|
| 微调 | `swift sft` |
| 预训练 | `swift pt` |
| DPO/GRPO | `swift rlhf --rlhf_type dpo` |
| 测试模型 | `swift infer` |
| 部署 API | `swift deploy` |
| 聊天界面 | `swift app` |
| 合并 LoRA | `swift merge-lora` |
| 量化导出 | `swift export --quant_method awq` |
| 评测 | `swift eval` |
| 生成数据 | `swift sample` |
| 可视化全流程 | `swift web-ui` |
| 大规模并行训练 | `megatron sft` |
| RL rollout | `swift rollout` |

ms-swift 全部子命令(源码验证)

按功能分类
- 训练:
  - swift sft / swift pt / swift rlhf
  - megatron sft / megatron pt / megatron rlhf
- 推理部署:
  - swift infer / swift deploy / swift app / swift rollout
- 工具:
  - swift export / swift merge-lora / swift eval / swift sample / swift web-ui
  - megatron export

源码 `main.py` 确认:
- 仅 `pt` / `sft` / `rlhf` / `infer` 四个命令在设置 `NPROC_PER_NODE` 或 `NNODES` 环境变量时自动走 `torchrun`。
- 其余命令不走分布式启动。
...
来源:`swift/cli/main.py` → `ROUTE_MAPPING` 字典。
...
| `swift sft` | 指令微调 |
| `swift pt` | 预训练 |
| `swift rlhf` | 人类对齐训练(DPO/GRPO/KTO/SimPO/ORPO/CPO/RM) |
| `swift infer` | 推理(单条/批量) |
| `swift deploy` | 部署 OpenAI 兼容 API 服务 |
| `swift app` | 交互式聊天应用(Web UI) |
| `swift eval` | 评测(基于 EvalScope) |
| `swift export` | 导出/量化/合并 LoRA |
| `swift sample` | 数据采样/生成 |
| `swift rollout` | RL rollout 引擎(独立启动 vLLM/SGLang) |
| `swift merge-lora` | 合并 LoRA 权重到基座 |
| `swift web-ui` | Web UI 可视化训练/推理/导出 |

#### Megatron 命令

(4个)

来源:`swift/cli/_megatron/` 目录文件 + 官方文档 `megatron sft/rlhf`。

| 子命令 | 用途 |
|---|---|
| `megatron sft` | Megatron 并行 SFT |
| `megatron pt` | Megatron 并行预训练 |
| `megatron rlhf` | Megatron 并行 RLHF/GRPO |
| `megatron export` | Megatron 模型导出 |
...
| 动作     | 常用子命令          | 典型用途                  | 注意                    |
| :------- | :--------------- | :--------------------- | :---------------------- |
| **训练** | `swift sft`<br>`swift pt`<br>`swift rlhf` | 微调指令、预训练、强化学习对齐   | 参数最多、最易出错，本文重点覆盖      |
| **推理** | `swift infer`<br>`swift app`   | 本地测试、Web界面交互、批量生成  | 参数精简，重在“快跑通”     |
| **部署评测** | `swift eval`<br>`swift deploy`<br>`swift export` | 模型打分、服务化上线、量化导出     | 多数参数可默认，优先保证`--model`和`--infer_backend`正确 |
...
三个环境变量
- WEBUI_PORT / WEBUI_SERVER / WEBUI_SHARE 
...
# ------------------------
swift web-ui --server_port 8089 --server_name 0.0.0.0 --lang en
# 【2026-9-7】以上方式在Ubuntu gpu上执行报错，
...
- --stream：推荐开，流式输出
- --`infer_backend`: 推理后端，取值 transformers / vllm / sglang / lmdeploy

```sh
# ✅ swift infer 里用 --infer_backend
swift infer --model qwen3-7b --infer_backend vllm

# ✅ swift deploy 里用 --infer_backend
swift deploy --model qwen3-7b --infer_backend vllm --port 8000
# sample 环节是 sampler_engine
swift sample --model qwen3-7b --sampler_engine vllm
```
...
- --`per_device_train_batch_size`：必填
  ...
  - A100（40GB）：可填 4
  - 显存爆了？立刻减小
  - 和 `--gradient_accumulation_steps`一起控制“实际batch size”，先填1保稳
...
- --`lora_rank`（必填！）LoRA **矩阵“宽度”**，越大越强、越费显存
  - 7B模型：8（安全）、16（进阶）
  - 13B模型：8（保稳）、16（推荐）
- --`lora_alpha`（必填！）LoRA**更新“力度”**，一般设为 alpha = rank × 2
