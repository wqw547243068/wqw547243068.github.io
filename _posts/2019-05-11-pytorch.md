...
2. 第二点是使用**GPU训练**
  - 如果需要使用GPU训练时，需要使用到 `to`(device) 函数，将数据或者模型放到GPU上面。
