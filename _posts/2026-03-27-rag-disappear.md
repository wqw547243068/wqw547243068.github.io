...
场景：
- 适合简单问答。
- 金融财报、法律合同或技术手册等**复杂的专业文档**时，**相似度**与**相关性**之间的差距就是准确率走坏的开始。
...
【2026-6-13】[分块时代结束，无向量RAG的核心拆解](https://mp.weixin.qq.com/s/eo-XPq2jFqPOVjhuH0kZqQ)

基于**推理**的检索（Reasoning-based retrieval）正在取代大多数团队好几年才搭起来的技术栈……
- 固有认知：`RAG` = `向量`

`无向量 RAG`（Vectorless RAG）是完全不同的检索架构

无向量 RAG 不再用**文档切片**和**向量嵌入**，直接读取文档**组织结构**，构建完美映射文档实际目录的`分层树状索引`（Hierarchical Tree Index）, 大章节、小章节、子章节, 让LLM进行逻辑推理的结构化目录。
- **检索**从此变成**推理**过程，而不再是**相似度匹配**。

运作机制
1. 文档索引
  - 项目符号将完整文档解析为`分层树状`结构。
  - 无需分块、无需向量化、无需向量数据库。
2. 树状推理
...
<img width="1000" height="100%" alt="image" src="https://github.com/user-attachments/assets/6ecad0d7-3eef-4c5e-8379-1e2b0547c5aa" />


### 效果

FinanceBench 中
- 行业领先的开源无向量RAG框架 `PageIndex` 拿到 98.7%的高分。
- 带联网搜索功能的`ChatGPT-4o` 仅为31%。
