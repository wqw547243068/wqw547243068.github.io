...
AAAI 2022 ，论文刷新了14个中英文数据集的SOTA
- paper: [Unified Named Entity Recognition as Word-Word Relation Classification](https://arxiv.org/pdf/2112.10070.pdf)
- code: 
...
- ②、`NNW`：即Next-Neighboring-Word，表示这两个word是在同一个实体中相邻的位置；
- ③、`THW-*`：即Tail-Head-Word-*，表示这两个word是在同一实体中，且分别是实体的结尾和开始。用THW-*来判断实体的类别和边界。
