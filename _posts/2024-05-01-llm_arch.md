...
完成以下脚本, 理论上可以加载任何一个开源模型。
- `trans_qwen_to_llama.py`
- `trans_llama_to_qwen.py`

以后出现任何新开源模型，就都可以通过“`trans_newModel_to_myModel.py`”，快速对该模型进行微调操作，而不用修改任何训练代码。
...
Multi-query Attention和Grouped-query Attention 是两种不同的注意力机制变种，用于改进和扩展传统的自注意力机制。
- Multi-query Attention：每个查询可以与多个键值对交互，捕捉更多的上下文信息。
  - 提高模型的**表达能力**和**性能**，特别是在处理长序列或复杂关系时。
