---
layout: post
title:  LLM 基础知识及原理详解 LLM Theory & Code
date:   2024-05-01 12:00:00
categories: 大模型
tags: gpt ChatGPT LLM norm 陈丹琦 ak47 可视化 zero
...
资料
- [llm_interview_note](https://github.com/wdndev/llm_interview_note/blob/main/01.%E5%A4%A7%E8%AF%AD%E8%A8%80%E6%A8%A1%E5%9E%8B%E5%9F%BA%E7%A1%80/1.llm%E6%A6%82%E5%BF%B5/1.llm%E6%A6%82%E5%BF%B5.md)
...
- [大模型补全代码时信息流](https://www.xiaohongshu.com/explore/69d7819a000000001b000db2)
- [大模型翻译时信息流](https://www.xiaohongshu.com/explore/69d780fe0000000023020e2a?xsec_token=ABFVXr6QmM34eYIXG3Cp99Iz6IOp11jl5YPAjW6S9RfdA=&xsec_source=pc_user)
- [大模型写诗时信息流](https://www.xiaohongshu.com/explore/69e63ce3000000001a02a8af?xsec_token=ABY_5Hz0zJYL4sCMUX2i9KAMzj_Jc34Aj0nkNa1JfT3g8=&xsec_source=pc_user)，图片[版本](https://www.xiaohongshu.com/discovery/item/69e6dd0b000000001a0266a9?source=webshare&xhsshare=pc_web&xsec_token=ABY_5Hz0zJYL4sCMUX2i9KAIp1x9wnvjyqgPrkoIvIDOQ=&xsec_source=pc_share)

【2026-5-21】[小红书帖子](https://www.xiaohongshu.com/explore/6a0e6dac00000000350220a7) 大模型 CoT 思维链的信息流, 反向传播式归因
- 反向传播式归因可以一步步反向追溯中间推理token的归因来源，直到用户输入，形成一个“因果流”。可以从图里看到，答案是怎么一步步产生的，以及哪些原始输入对结果影响最大
- huggingface 工具: [InfoLens](https://huggingface.co/spaces/dqy08/InfoLens) 实时体验，[LLM Causal Flow](https://dqy08-infolens.hf.space/client/gen_attribute.html?content=13dmoez)

<img width="600" height="100%" alt="image" src="https://github.com/user-attachments/assets/19185847-2d8e-4ec0-a69c-d596ab32f2e1" />

<iframe src='https://huggingface.co/spaces/dqy08/InfoLens' width='900' height='100%' frameborder='0' scrolling='no' allowfullscreen="true"></iframe>
...
- github主页 [dive-into-llms](https://github.com/Lordog/dive-into-llms)

《开源大模型食用指南》
- 针对中国宝宝量身打造的基于Linux环境快速微调（全参数/Lora）、部署国内外开源大模型（LLM）/多模态大模型（MLLM）教程
- Github主页 [self-llm](https://github.com/datawhalechina/self-llm)
...
- 【2025-6-23】2025年，斯坦福课程 [CS336: Language Modeling from Scratch](https://cs336.stanford.edu/), github [stanford-cs336](https://github.com/stanford-cs336)
  - youtube 视频 [Stanford CS336 Language Modeling from Scratch-Spring 2025-Lecture 1: Overview and Tokenization](https://www.youtube.com/watch?v=SQ3fZ1sAqXI&list=PLoROMvodv4rOY23Y0BoGoBGgQ1zmU_MT_)
...
- [MiniMind](https://jingyaogong.github.io/minimind/) 文本LLM训练，Train a 64M ChatBot from zero 2 hours. ¥3. One 3090.
- 【2026-5-10】[minimind-o](https://jingyaogong.github.io/minimind-o/)
...
<img width="915" height="100%" alt="image" src="https://github.com/user-attachments/assets/c4c8f805-72b5-4dc0-a4fd-00f7f6603ed1" />


【2026-9-9】MiniMind 轻量LLM后训练与Agent RL实践 

基于 MiniMind-3 64M Dense / 198M-A64M MoE，复现从监督微调、偏好优化到在线策略优化和多轮工具调用的轻量后训练链路。
- 轻量后训练体系：支持 SFT、DPO、PPO、GRPO 与独立 LoRA 微调流程，共用对话模板、assistant-only token mask、token-level log-prob 计算和奖励接口；实现 DPO chosen/rejected 偏好损失。
- PPO/GRPO：实现 token-level GAE、优势标准化、policy/value clipping 与 reference-policy KL 约束；通过跨卡归约 approx-KL 保证 DDP 早停决策一致，并实现 GRPO 多响应采样及组内相对优势计算。
- 可插拔 Rollout Engine：抽象同步轨迹采样接口，支持 PyTorch 本地生成与 SGLang HTTP 推理后端，统一返回 completion token、old log-prob 和有效 mask；支持通过共享目录导出与 HTTP reload 将训练权重同步至推理服务。
- Tool-Use Agent RL：构建包含6类模拟工具、最多3轮交互的“调用—观察—再生成环境”；组合格式闭合、工具合法性、GT命中、未完成及重复惩罚等轨迹级奖励，并从策略损失中排除工具 observation token。
- 实验结果：2 * RTX 4090 上训练，相同评测与解码配置下，20条轻量数学 ToolUse 样例的最终答案命中率由 SFT 的 60%（12/20）提升至 Agent RL 的 85%（17/20）。


#### 练手项目


Long-Horizon 购物 Agent 后训练与 GRPO 优化

基于淘宝真实商品库 ShopSimulator 搭建端到端购物 Agent 后训练系统，根据用户需求自主完成商品搜索、详情浏览、规格选择与购买。

以 Qwen3.5-2B 为基座，完成教师轨迹构建、LoRA SFT、在线 GRPO 训练及统一评测。
- Teacher 数据构建与 SFT：使用 DeepSeek V4 Flash 采集2498条多轮购物轨迹，通过环境回放和 Reward 严格筛选出1000条有效轨迹；采用 LoRA SFT 和 assistant-only loss mask，仅监督 Agent 决策及工具调用 token。
- Agent Loop 与 GRPO 优化：基于 veRL 实现多轮环境交互，支持并发 Session、动作合法性校验、Observation 投影和上下文管理；设计确定性分段 Reward，区分目标购买、有效替代、部分匹配、错误购买、合理放弃和重复循环；采用每个 Prompt 4条轨迹、无 KL 约束及同奖励组动态过滤的 GRPO 训练策略进行优化。
- Agent 评测 Pipeline：构建与训练数据严格隔离的 Final-200 测试集，采用“代码硬检查 + 任务 Rubric + LLM-as-Judge”评测 Pipeline，隔离 Reward、Gold 商品信息。在单张 RTX PRO 6000 96GB 上，严格成功率由 Base 的 0% 提升至 SFT 的 68.5%，GRPO 进一步提升至 70.5%。
...
【2025-7-19】[Sebastian Raschka](sebastianraschka.com/blog/2025/the-big-llm-architecture-comparison.html) 聚焦 DeepSeek V3、OLMo 2、Gemma 3、Mistral Small 3.1、Llama 4、Qwen3 和 Kimi 2 架构亮点，总结其关键技术进步。
- 原文 [The Big LLM Architecture Comparison](https://magazine.sebastianraschka.com/p/the-big-llm-architecture-comparison)
- 【精】[LLM Architecture Gallery](https://sebastianraschka.com/llm-architecture-gallery/#card-llama-3-8b): 所有大模型结构可视化，交互式查看，链接到技术报告、配置文件、训练代码
...
【2025-12-14】[2025 年最全面的 LLM 架构技术解析](https://mp.weixin.qq.com/s/MCyMmmKhkUeJ0X62-9QbVA)
- 原文 【2025-7-19】[the-big-llm-architecture-comparison](https://magazine.sebastianraschka.com/p/the-big-llm-architecture-comparison)

<img width="1080" alt="image" src="https://github.com/user-attachments/assets/1993ffbe-c3f8-435d-8b33-93b78cb0a3e6" />
...
|---|---|---|---|---|
|位置编码|Positional Embedding|绝对位置编码|旋转位置编码 RoPE||
|多头注意力|Multi-Head Attention|常规|Grouped-Query Attention组注意力<br>DeepSeek V3使用另一种方法 MLA||
...
【2026-8-27】国内开源前沿模型正在快速趋同：
1. 除 DeepSeek 外，多数已用线性注意力（Kimi KDA、Qwen GDN、GLM KDA 风格）
2. 除 Kimi 外，普遍用稀疏注意力 + indexer/压缩（DSA、QSA、LSA、IndexPool）
3. 残差连接升级成标配（mHC / AttnRes）
4. 训练侧几乎都上了 Muon

【2026-9-10】[帖子](https://www.xiaohongshu.com/explore/6aa0e824000000000d0251a3) 主流LLM架构的演进方向：
- ① 注意力替代与混合；
- ② MoE 与路由；
- ③ 残差与连接拓扑；
- ④ 位置编码与外推；
...
- ⑧ 系统-架构协同；
- ⑨ 训练目标（MTP）​。
- 优化目标包括：FLOPs、KV显存、带宽、通信、稳定性、数据效率、长度外推。
	
预测：
- Qwen​：研究型激进派。全行业只有它把"架构决策"做成了对照实验（GDN vs SWA、GR vs AttnRes、QSA vs IndexShare）。
- DeepSeek：工程师的确定性基准。noaux_tc 路由和稀疏索引成了行业事实标准（Kimi/GLM/混元都在抄它），代价是它必须永远领先半步当"标准定义者"，慢了就失去存在感。
- Kimi：规模派。896 专家能跑起来靠的是 LatentMoE + Quantile Balancing + MoonEP 整条 infra，这本身就是护城河；AttnRes 是残差之争里唯一能和 GR/mHC 打平的第三条路。
- GLM：实验派。KDA 线性层 + DSA 稀疏层 + DeepSeek 系 mHC + noaux，全是已验证组件——赢在"不发明、只选最优"，风险是纯实验没有自己的经验曲线。
- Hunyuan：纯序列压缩的收敛派。IndexCache 跨层复用只在"零线性层"的纯 DSA 下成立（§3.3 实验已定论），所以它的路线自洽、但也有回旋余地小的代价。
- Seed：评测黑箱。规格零披露却握着 VWN 这张真正的架构牌——它若把 VWN 量产，就是"hidden 不再是容量唯一瓶颈"的开路者。
...
Qwen3 提供**密集模型**（0.6B-32B）和**MoE模型**（30B-A3B、235B-A22B）。
- 密集模型如0.6B以小巧高效著称
...
【2026-3-9】[多轮对话微调，Loss 为什么要 Mask 掉 User 的部分？](https://mp.weixin.qq.com/s/ydlm_zrwBFXgbgXBlJlF4w)
...
- 3、**归一化**与**可控性**。缩放因子也提供了一种对微调程度进行“手动控制”的机制。
...
- 1、对于**注意力**层，通常微调w_q和w_v，因为w_q决定关注哪些信息，w_v决定输出什么内容，直接影响模型对任务的理解和生成能力。
  - 疑问: 为什么没有 w_k 层？
- 2、对于**前馈**层，通常微调第一层w1，因为它可以调整激活空间特征。
...
解决
- 把`数据并行`和`专家并行`混用，比如让几个 Expert 挤一张卡，用动态批处理把零散的任务攒起来再算，GPU 立马就没空摸鱼了。
- 再给路由网络加点 “规矩”—— 训练时加个负载均衡损失，别老盯着那几个 Expert 薅，推理时看哪个卡闲就把任务派过去，负载立马匀过来。
...
[月之暗面](https://platform.kimi.com/playground)输出检测工具`月宫` MoonPalace，可检测是否被截断、内容不完整、重复，go写的
- [moonpalace](https://github.com/MoonshotAI/moonpalace)
...
ZeRO本质是改造「数据并行(DP)」，不是传统意义的模型并行（TP张量并行 / PP流水线并行）
...
详见站内专题: [神经网络训练技巧： loss曲线分析](ann_tune#loss曲线分析)
