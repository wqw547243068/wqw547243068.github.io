...
|Dual LoRA|2025.12|斯坦福|[Dual LoRA: Enhancing LoRA with Magnitude and Direction Updates](https://arxiv.org/abs/2512.03402)|并行幅度分支+方向分支；双路径协同更新|比DoRA精度+1~3%；复杂任务泛化更强|无（核心图解为双分支协同更新架构，可参考论文Figure 1）|
|HydraLoRA|2026.01|学界|[HydraLoRA: An Asymmetric LoRA Architecture for Efficient Fine-Tuning](https://arxiv.org/abs/2601.19267)|共享A矩阵+多分支B矩阵；捕捉共性+特性；多任务适配|多任务干扰-40%；复杂数据集精度+5~8%|无（核心图解为不对称共享架构，可参考论文Figure 1）|
|MiSS-LoRA|2026.02|ICLR 2026|[MiSS-LoRA: Sliced Shared Structure for Efficient Low-Rank Adaptation](https://openreview.net/pdf/0565197f.pdf)|分片共享结构；按权重重要性动态分片|参数量-50%；训练速度+30%；精度基本持平|无（核心图解为分片共享结构示意图，可参考论文Figure 3）|
|VeriLoRA|2026.03|NDSS 2026|[VeriLoRA: Fine-Tuning Large Language Models with Verifiable Security via Zero-Knowledge Proofs](https://www.ndss-symposium.org/ndss-paper/verilora-fine-tuning-large-language-models-with-verifiable-security-via-zero-knowledge-proofs/)|结合零知识证明（ZKP）；训练过程可验证、隐私保护|安全合规；不泄露数据/参数；正确性可验|无（核心图解为ZKP与LoRA融合流程，可参考论文Figure 2）|
