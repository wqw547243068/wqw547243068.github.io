...
【2026-5-23】[小红书视频](https://www.xiaohongshu.com/explore/6a106af40000000035025a17)

大模型根本不是知识库 —— 连最便宜的固态硬盘都比不上
- 大模型不存原文，只学分布，70b参数=130GB权重
- 2T SSD硬盘，写进去，10年后再读出来，还是那个数字

大模型（概率直觉） vs 硬盘（确定性）

2025年，研究证明
- 7B 模型在 int 8 量化下，最多能稳定承载 1.75GB 的结构化事实

不是模型忘了，而是压根就没打算记：<span style='color:red'>大模型是有损拟合，非无损压缩，不还原字节</span>
- 高频共现的规律被刻进权重深处，低频新界早被后续几万次训练冲的渣的不剩
...
【2026-5-13】[OpenHuman](https://openhuman.ai/) 把卡帕西这套手工活，变成了全自动流水线。
...
## 【2026-8-28】TrustGraph

【2026-8-28】企业知识库 【[开源本体RAG TrustGraph：企业知识库的下一站](https://www.toutiao.com/article/7679002594345239081/)】

RAG 知识库问题：AI 能找到**相关**资料，却不一定真正理解企业的业务。

同一个词在不同部门可能有不同含义；一项业务决策，往往同时涉及客户、产品、合同、人员、审批、政策和时间；一条结论，也可能需要跨越多个系统才能验证。

企业问题经常不是**单文档**问题，而是**跨实体、跨系统、跨关系**的问题。

问题不再是“能不能检索到一段文字”，而是“能不能理解企业世界中的概念和关系”。

于是，Ontology，本体，越来越重要。

TrustGraph 思路不是用图谱完全替代向量检索，而是让两者各自发挥作用：
- 向量检索帮助系统找到相关内容
- 本体和图谱帮助系统理解这些内容之间的关系。

<span style='color:red'>向量检索擅长找“相关内容”，Ontology 和知识图谱擅长解释“为什么相关”</span>。

TrustGraph 是围绕企业上下文、Ontology、知识图谱、RAG 和 Agent 构建的开源项目。不只是让大模型搜索文档，而是如何把企业数据变成一层可理解、可检索、可追踪、可治理的 AI 上下文。
- [TrustGraph](https://trustgraph.ai/videos/) —— 疑似官网
- GitHub [trustgraph](https://github.com/trustgraph-ai/trustgraph)

本体领域有两个很重要的概念：`T-Box` 和 `A-Box`。
- `T-Box` 是知识的结构层。定义企业有哪些概念、概念之间有什么层级、有哪些属性和关系，以及这些关系在什么情况下成立。
  - 比如，T-Box 规定“设备属于生产线”是一种合法关系；也可规定“设备编号”是设备的属性，而不是生产线的属性。
- `A-Box` 是知识的事实层。记录现实世界中的具体对象和具体事实。
  - 例如：设备 D-1001 属于生产线 Line-A；D-1001 当前处于故障状态；工单 WO-2026-001 由张三处理；张三属于维修部门。

`T-Box` 规定世界应该怎样描述，`A-Box` 记录这个世界里具体发生了什么

<img width="1672" height="941" alt="image" src="https://github.com/user-attachments/assets/8b00e387-e69a-4b9a-8b52-a46c263fbfb5" />


TrustGraph 对这两层知识的处理方式。

企业可以先建立或导入自己的 Ontology。项目提供 Ontology 管理能力，可以编辑、校验并导入导出 OWL/XML、Turtle 等本体文件。进入运行环境后，本体会被加载为类、对象属性和数据属性，并检查类别层级、属性起点、属性终点和继承关系等结构。

TrustGraph 在逻辑上区分 T-Box 和 A-Box，但当前代码并没有简单地把它们实现成两个完全独立的数据库。
- T-Box 主要以本体配置和语义约束的形式参与抽取与查询，同时也可以被转换成本体定义三元组；
- A-Box 则来自文档抽取、结构化数据和直接知识导入，最终与本体定义共同服务于图谱查询。

变化：
- 大模型不是在没有边界的知识空间里自由猜测，而是在企业定义的语义框架中理解和生成事实。

TrustGraph 处理过程可用一条业务流程来概括：
> 接入数据 → 解析和切分 → 识别实体关系 → 匹配本体 → 校验事实 → 写入图谱 → 提供给 RAG 和 Agent

当企业文档进入系统后，TrustGraph 会先将文档解析成可以处理的内容，再把内容切分为较小的片段。

接着，Ontology RAG 会根据每个片段的语义，找到当前最相关的本体概念。系统不会把整个企业本体一次性塞给大模型，而是动态选择当前内容需要的那部分类、属性和关系。

模型随后按照这部分本体约束抽取具体事实。抽取结果经过验证后，才会被写入知识图谱，并进一步形成实体上下文和向量索引。

TrustGraph 的 RAG Query 有哪些模式？

根据问题类型和数据形态，它提供了多种查询路径。
- Document RAG：从文档中寻找答案
- Graph RAG：沿着实体关系寻找答案
- Ontology-aware Query：先理解概念，再构造查询
- SPARQL 和 GraphQL：面向不同数据形态
  - SPARQL 适合查询 RDF 形式的知识图谱，尤其适合本体和三元组结构比较规范的场景。
  - GraphQL 则更适合结构化业务数据和表格型数据。企业中的客户表、订单表、员工表和业务指标，也可以通过相应的查询方式被纳入统一的 AI 数据访问体系。


<img width="1672" height="941" alt="image" src="https://github.com/user-attachments/assets/ed55fa60-e059-4f03-b405-502bdb677013" />

TrustGraph 的功能并不集中在一个问答窗口中。项目提供了 14 种 Workflow，覆盖知识接入、本体管理、数据探索、查询问答和 Agent 运营。
