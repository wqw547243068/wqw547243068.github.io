...
categories: 大模型
tags: 对话系统 ChatGPT tableqa nl2sql lora 高德
...
【2026-9-25】[问数准确率从 50% 到 95%，高德只做对了一件事](https://mp.weixin.qq.com/s/dkRv23RrZDjqQtwwxoeW9Q)

企业数据服务长期困在两个瓶颈上：看板永远做不完，长尾需求覆盖不全。

而业务真正想问的“为什么、怎么办”，传统取数只会回答“是多少”。

高德智能问数产品（`小豪`）没有选择堆更大的模型，而是把沉淀多年的 BI 底座，改造成**可路由**、**可加载**领域知识的 Agent 底座。核心是语义层建模，不是模型本身。

但这条路是用真实的翻车换来的。

|阶段|架构|优点|缺点|
|---|---|---|---|
|MVP|RAG+Workflow|39张表，准确率 89.4% |331张表后，准确率下滑到 50%|
|改进|Skill|准确率回升到 95%||
|||||

（1）MVP 阶段：39 张表，准确率 **89.4%** —— 看起来已经能上线了
- 全量扩表后：331 张表端到端准确率暴跌到 **50%**

（2）切换架构后：从 RAG + Workflow 改为 Skill 架构，准确率回升到 **95%**

为什么 RAG 一扩规模就塌？
- 一次性知识召回、多步信息损失、不支持跨表复杂计算。

解法
- 换成 Skill（渐进式披露）架构: 按需召回 + AI **自反思纠错**，同时在 Agent 框架下用 Skill 保证用户深度、延展用数场景的可控性与扩展性。

2026年10月23-24日，DACon 2026北京站，高德商业智能数据产品专家`林宇航`将带来演讲《从“答得对”到“看得透”——高德智能问数产品，从 BI 底座到 Agent 底座迭代之路》，按 Gartner L2→L3→L4 的坐标，完整复盘这条决策链。

判断：行业整体正处在 L2 智能问数向 L3 智能诊断过渡的阶段，L4 自主决策仍是空白，窗口期 12–18 个月。

两处“脏活”他们也讲透了。
- 一是语义层的三类知识来源（元数据自动 / 业务知识人喂 / 路由规则）；
- 二是评测飞轮三件套（AI-Friendly 标准分、NL2SQL EM+EX、BIAS 端到端对标 OpenAI Trace Grading）。
