---
layout: post
title:  大模型推理优化方法 LLM Inference Method
date:   2023-09-22 17:02:00
categories: 大模型
tags: gpt 量化 vllm deepspeed 推理 推测解码 sglang 投机采样 美杜莎
excerpt: 如何提升LLM推理效率？
mathjax: true
permalink: /llm_infer_method
---
...
#### 【2026-6-18】Unsloth 1-bit 压缩 GLM 5.2

【2026-6-18】[资讯](https://x.com/i/status/2069418532375564484)

Unsloth 团队 居然用 1-bit 压缩的极端技术，把顶级开源模型GLM 5.2 优化并转换成轻量级的 GGUF 格式。
- 完全在本地（Locally）运行GLM5.2（256G内存的Mac Studio），不需要任何互联网连接或外部服务器，而且速度超棒，达到了 21 个 token 每秒（人类说话大概十几token每秒）。
- 还直播，把本地模型放到对决中，直接面对世界上最强大、最昂贵的付费云服务器 Claude 4.8 Opus 和 GPT-5.5。
...
[llama-factory 教程](https://llamafactory.readthedocs.io/zh-cn/latest/advanced/quantization.html)
- 浮点数由3部分组成：符号位、指数位和尾数位。
- 指数位越大，可表示的数字范围越大。尾数位越大、数字的精度越高。

![](https://llamafactory.readthedocs.io/zh-cn/latest/_images/quantization_0.png)

量化分类
- 可根据何时量化分为：**后训练**量化和**训练感知**量化
- 也可根据量化参数的确定方式分为：**静态**量化和**动态**量化。
