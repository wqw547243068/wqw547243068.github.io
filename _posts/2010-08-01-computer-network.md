...
categories: 计算机基础
tags: 网络 OSI 路由器 交换器 调制解调器 猫 lan wan wlan ap tcp udp 下载 种子 磁力 电视 卫星 地面波 广播 蓝牙 耳机 wifi 内网穿透 云盘 浏览器
excerpt: 计算机网络知识点
mathjax: true
permalink: /network
---
...
千兆网口（1G）、2.5G网口（2.5G）、5G网口（5G）是不同速率的以太网接口，主要区别在于传输速率、应用场景、线缆要求以及成本等方面

| 端口类型   | 理论速率   | 实际传输速度（约） | 比千兆快多少 |
|------------|------------|--------------------|--------------|
| 千兆（1G） | 1 Gbps     | 100-125 MB/s       | 基准         |
| 2.5G       | 2.5 Gbps   | 250-312.5 MB/s     | 2.5倍        |
| 5G         | 5 Gbps     | 500-625 MB/s       | 5倍          |


应用场景
- 千兆（1G）：
  - 家用宽带（≤1000M）、普通NAS、办公网络。
  - 目前普及，但逐渐成为瓶颈（如4K视频剪辑、大文件传输）。
- 2.5G：
  - 性价比过渡方案：适合家庭/中小企业，无需更换Cat5e网线即可升级。
  - 场景：2.5G NAS、多设备高速内网、Wi-Fi 6（AX3000以上）回程。
- 5G：
  - 高性能需求：如多用户4K视频协作、企业级存储、虚拟化环境。
  - 需搭配Cat6及以上网线，成本较高，普及度低于2.5G
