...
categories: 大模型
tags: LLM ilya 缩放定律 ttt 乔姆斯基 扩展法则 openai google deepmind agent 吴恩达 anthropic 翁荔
...
【2025-7-1】[一文详细了解：大模型三大缩放定律（Scaling Law）](https://www.icviews.cn/semiCommunity/postDetail/10263)
...
|Test-Time Scaling Law<br>又称 Long Thinking|推理|||增加测试时计算（如思维链、自洽性采样、集成等方法）提升模型表现，但边际收益递减。|
||||||
...
计算工具
- [Scaling calculator](https://scaling.mishajw.com/) 大模型规模化计算器，解决问题：
  - 训练13b模型要多少计算量
  - 1T token的数据量，训练成本多少
  - 指定计算量下，要多少GPU
...
- [论文](https://arxiv.org/abs/2410.12119)
...
- 论文 [paper](https://arxiv.org/abs/2504.02495)
...
Test Time Scaling Law 存在**边际收益递减**特性，模型固有能力会限制测试时优化的上限（例如基座模型无法解决数学问题，再多采样也没用）。
...
【2026-6-26】[翁荔博客上新：谨慎对待Scaling Law](https://mp.weixin.qq.com/s/FIIr4ye0TJNdXg4nnRUCPw)
- 博客原文：[Scaling Laws, Carefully](https://lilianweng.github.io/posts/2026-06-24-scaling-laws/)
...
- 2020年，Kaplan 在语言建模社区普及 Scaling law 概念，交叉熵测试损失 L 分别与模型规模 N（不包括嵌入层）、数据集规模 D 和训练计算量 C 呈幂律缩放关系，跨越了多个数量级。
  ...
  - 每个 token 训练总 FLOPs 约为 6N ，在 D 个 token 上训练的总 FLOPs 为 `C ≈ 6ND` 。
