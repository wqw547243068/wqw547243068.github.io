...
categories: 大模型 对话系统 agent mermaid 推理 安全 工具调用 GRPO
...
多轮对话优化主要从四方面入手：
1. 上下文管理（滑动窗口保留近期对话、关键信息摘要、状态变量跟踪）；
2. 指代消解（将"明天"等替换为具体实体）；
3. prompt精细化（结构化历史对话、明确角色职责）；
4. 记忆机制（建立用户画像和长期偏好知识库）。
...
【2026-5-3】[小红书视频讲解](https://www.xiaohongshu.com/explore/69f5ff1d00000000220282da)

LLM 指令半衰期：
- 分类规则 2-4 轮就忘
- 安全约束 3-5 轮就忘

【2026-4-30】德国慕尼黑工业大学（Technical University of Munich）发布评测集 DriftBench，评估多轮对话中指令遵循能力
- 论文 [Models Recall What They Violate: Constraint Adherence in Multi-Turn LLM Ideation](https://arxiv.org/pdf/2604.28031)
- 评测集 [DriftBench](https://github.com/kruthof/driftbench)

不同类型的规则/约束，在多轮次交互中的**稳定性（半衰期）差异**，轮次越高代表抗衰减能力越强。

| 类别 | 轮次区间 | 特点与解读 | 核心表现 |
|--------|----------|------------|----------|
| 格式/schema | 12-15 | 结构最稳 | 模型对固定格式、JSON schema、语法结构的遵守度最高，衰减最慢 |
| 数值边界 | 8-10 | 边界中衰 | 对数字范围、上下限的约束，在中高轮次开始失效 |
| 流程/计划 | 5-8 | 步骤脱钩 | 多步骤流程、任务计划，在中低轮次开始出现逻辑断裂 |
| 安全伦理 | 3-5 | 早期松动 | 安全护栏、伦理约束，在中低轮次就开始出现弱化 |
| 分类规则 | 2-4 | 最快破裂 | 分类、标签、简单判断类规则，衰减速度最快，轮次很低就失效 |

💡 关键结论
- **轮次指针持续扫过，越早破裂表示半衰期越短**：
  - 模型对规则的“记忆/遵守时长”从强到弱依次为：格式/schema > 数值边界 > 流程/计划 > 安全伦理 > 分类规则
- 解释了长对话/多轮任务中：
  - 用**JSON schema、固定格式**做约束最可靠；
  - 依赖“伦理、分类判断”的规则，最容易在后续轮次被突破或遗忘。


### 对话迷失


#### 【2025-5-9】微软 分片式对话导致迷失

【2025-5-9】ICLR'26杰出论文，多轮对话让大模型“翻车”
- ChatGPT、Claude这些大模型时，越聊越不对，不是我们的问题
...
"分片模拟"方法
- 将单轮完整指令分解为多个信息片段(shards)，通过控制实验比较 LLMs 三种设置下的表现差异。
- （1）单轮完整指令
- （2）单轮拼接指令
- （3）多轮分片指令
...
核心结论：
- ✅ 所有主流大模型，多轮对话性能平均暴跌39%
- ✅ 单轮90%+正确率，多轮直接掉到60%左右
- ✅ 越强的模型，跌得一样惨，没有例外

为什么会“迷路”？
- 过早答题：信息没给全就瞎猜，强行出答案
- 死磕错误思路：一旦跑偏，后面怎么补都拉不回来
- 回答越来越臃肿：越聊越长、越乱，全是无效假设
- 忘中间：只记得开头和最后一句，中间信息直接丢了
...
不是模型变笨，是多轮+信息不完整，直接击穿LLM的软肋。就连调低温度、加思考步骤，都救不回来…
...
【2026-5-15】如何保证Agent调用工具可靠性？

四层架构：
1. 结构化定义参数规则（JSON强类型约束）避免70%低级错误；
2. 强制模型输出思考过程，通过正反示例提升准确性；
3. Python自动化校验拦截格式错误；
4. 建立错误反馈闭环重试机制。

核心公式：
> 可靠性 = 结构化定义 + 执行约束 × 反思重试机制
> 让工具调用从随机化变为标准化、可控制的稳定执行。
...
### 【2026-5-14】26M Needle

【2026-5-14】[26M 小模型，开始接管工具调用](https://mp.weixin.qq.com/s/_AwmxV8YAX0OuailEqQTYg)

模型判断该调用哪个工具，填哪些参数，最后吐出一段 JSON。

现在很多系统会直接把这件事交给 7B、32B、甚至更大的通用模型。

这个能跑，但可能不是最合理的架构。

Cactus Compute 开源的 Needle 做了一个很激进的实验：把 Gemini 工具调用能力蒸馏成一个 26M 参数的小模型。

目标也不是聊天、写代码、做数学题，而是单点突破：single-shot function calling。给它用户请求和工具定义，输出该调用的工具和参数。

Needle 的 repo 里给出的数据是：
- 26M 参数
- 从 Gemini 3.1 蒸馏
- 生产环境跑在 Cactus 上
- prefill 约 6000 tok/s
- decode 约 1200 tok/s
- 预训练 200B tokens，用 16 个 TPU v6e 跑了 27 小时
- 后训练 2B tokens 的单轮函数调用数据，用了 45 分钟
- 权重、代码和数据生成流程公开

工具调用可能不需要一个通用聊天模型来完成。

Needle 暗示了一种更拆分的架构：**小模型做快速路由，大模型做复杂判断**。

Needle 背后的 Simple Attention Network 更激进：整个模型没有传统 Transformer 里的 FFN。普通 Transformer 里，FFN 通常吃掉大量参数。它很擅长存储和变换知识，但 Needle 的作者认为：在工具调用这种任务里，知识已经在输入里了。

Needle 用 encoder-decoder 架构：
- encoder 双向读取用户请求和工具定义
- decoder 生成工具调用结果
- cross-attention 负责把输入里的结构对齐到输出 JSON

另有一个对比学习的工具选择头，用来从大量候选工具里先筛 top-k

Needle 限制。
- 第一，面向 single-shot function calling。一次性判断和输出工具调用。复杂多轮 agent、长上下文记忆、跨工具规划，不是它现在最强的场景。
- 第二，小模型会比较 finicky。应该用自己的工具集合测试，并按自己的数据微调。
- 第三，打败一些更大的小模型，是在单轮函数调用这个窄任务上。那些模型在聊天、泛化、复杂推理上仍然有更大能力边界。
- 第四，工具调用一旦接上真实世界，错误成本会变高。发错消息、删错文件、下错单，比答错一个问题严重得多。

所以 Needle 不应该被理解成“大模型不需要了”。更准确的理解是：大模型不该包办所有环节。


### 【2026-2-12】GRPO提升工具调用质量

【2026-2-12】[历时两个月的GRPO 训练踩坑实录：63% -> 96% tool selection](https://zhuanlan.zhihu.com/p/2005390937266857291)

任务：基金助手 SubAgent，多工具调用
- 目标：7B 小模型代替已有的 Mixtral 8X22B，实现低延迟、低成本的特定场景 SubAgent。
- 问题：如何在 7B 模型的能力边界内，把工具选择准确率做到极致？

可用工具：
- check_balance、transfer_funds、withdraw_funds、place_order、get_transaction_history、check_market_status
...
结论：精心设计的 GRPO 训练流程，7B 模型完全可以在特定任务上达到很高的准确率。
...
核心建议：
> 先确保问题定义正确，监控 frac_reward_zero_std，奖励函数最大分要能达到 1.0，针对错误模式设计惩罚，不要迷信"高级"算法。
...
经验
- 奖励函数设计：最大分必须能达到 1.0，要有足够区分度，针对具体错误模式设计惩罚，持续监控 frac_reward_zero_std。
- 数据教训：简单重复数据会导致灾难性遗忘，LLM 生成数据质量难保证，改变数据格式（ReAct）有效，修正错误 label 是关键。
- 算法选择：NGRPO 通用解决零梯度问题，DART 适合复杂场景，PRS/难度感知 需要足够难样本才有效。
...
各类别得分差异明显：
- history/market/balance 在 82-89%，但 settlement 仅 18%、intraday 23%、deposit 31%、combo 12%。

直觉：弱类别是数据不够，于是 3 倍过采样，结果全面退化。查看日志发现近 50% 步数梯度为零，过采样让简单样本重复出现，没有提供新的学习信号。


#### 数据扩充失效

领域数据扩充：100+ 条新样本覆盖 T+1 结算、周末处理等场景。结果完全无效。日志显示 frac_reward_zero_std 几乎 100%——模型已经"会"了，没有学习信号。GRPO 需要**奖励方差**才能学习。

核心问题：所有生成的 reward 相同时，advantage = 0，模型无法学习。

#### NGRPO 提效

受 NGRPO 论文启发，添加虚拟满分样本：
...
结果：
- 平均分 +13 点，tool 准确率 +14 点，intraday +35 点。
- 关键指标 frac_reward_zero_std: 0.0。
...
SFT 实验中观察到奇怪现象：reasoning 变好但 tool 选择变差。

DART 论文 (arXiv:2602.00994) 提到：
- Reasoning 和 Tool-use 的梯度方向几乎正交，联合训练会导致两种能力互相干扰。

实现简化版 Token-Level 梯度分离：
...
重新思考：Combo 场景本质是多步规划问题，不是简单的工具选择。
...
结果：combo 91%，平均分 89%，tool 准确率 92%。

<span style='color:red'>问题定义比算法更重要</span>。

#### 奖励规则升级

ReAct 成功后继续强化弱类别，但提升有限。
