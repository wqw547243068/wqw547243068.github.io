...
- 8月22日，OpenAI 宣布企业现在可以使用自己的数据对 `GPT-3.5 Turbo` 进行**微调**，OpenAI 声称最终的定制模型可以赶上甚至超过 GPT-4 执行某些任务的能力。
- 传入和传出微调 API 的数据归客户所有， OpenAI 或任何其他组织不会使用这些数据来训练其他模型。
- 今年秋天 OpenAI 将开放更先进的 GPT-4。

GPT 的“微调”与 Llama2 之类的微调不同，因为不会调整网络的所有权重，只是会调整网络的一小部分。代价是 OpenAI 微调的成本较低，但功能也没有“真正的”微调强大。
...
初版 GPT-3 基础模型（ada、babbage、curie 和 davinci）将于 2024 年 1 月 4 日正式关闭。
