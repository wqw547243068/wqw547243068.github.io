...
这种有上下文（Context）的多轮对话, 包含：**上下文相关记忆**, 每一个**单轮对话逻辑**, **退出规则**等
- 也可以多个 stage 组合成管道
...
许多基于 slot filling (**槽填充**) 的任务型多轮对话, 就是实现了一个单向的多轮对话
...
由单轮对话构成多轮对话, 再加上**分支**和**循环**, 一个初步的复杂多轮对话就成型了. 理论上它可以**无限轮次**.
...
最大的特点是, 依赖关系不仅接受层层回调, 而且共享中断机制. 遇到cancel, failure, reject等事件时, 也会冒泡式地层层回调. 任何一层如果拦截了事件, 可以自己定义重定向的 stage.
