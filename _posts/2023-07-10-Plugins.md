...
categories: AIGC
tags: gpt ChatGPT prompt plugin 插件 函数调用
excerpt: ChatGPT插件开发、函数调用知识总结
mathjax: true
permalink: /plugin
---
...
- 创建聊天机器人：开发者可以调用外部工具(如 ChatGPT 插件)回答问题
  - 将查询“北京的天气如何？”转换为调用 `getCurrentWeather(location: string)` 的函数。
- 将自然语言转换为 **API 调用**或**数据库查询**：
  - 例如，将查询“**这个月我的前十个客户是谁？**”转换为调用 `get_customers_by_revenue(start_date, end_date, limit)` 的内部 API 调用，
  - 将查询“**上个月 Acme 公司下了多少订单？**”转换为使用 `sql_query(query)` 的 SQL 查询。
- 从文本中提取**结构化数据**：开发者可以定义一个名为 `extract_people_data(people)` 的函数，以提取在维基百科文章中提到的所有人物。


### Function call 原理

从人机交互上来说， Function Call 本质上只做了一件事
- 实现了「准确**识别**用户语义，将其转为**结构化**指令」。
...
Function Call 实现的最大的价值是: 让机器轻易地理解了用户**模糊化**输入，将其转换为机器可以理解的**技术指令**。这对于人机交互的范式来说，完全是质的改变。
