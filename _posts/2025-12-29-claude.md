...
Claude Code 7 个常见错误正在浪费你的时间
	
错误 1：一个长会话做所有事
实现功能、修 bug、写测试、review PR 全在一个对话里。上下文满到 90% 时，模型质量明显下降——忘记你之前的约束，输出变得泛泛而谈。
修复：每个任务之间输入 /clear，只需 1 秒。
	
错误 2：写模糊的 Prompt
"修一下 bug" 不是 prompt。5 分钟的任务变成 45 分钟，往往就是因为 prompt 不够具体。你只需要三件事：什么坏了、在哪里、怎么算修好。
修复：按回车前问自己——Claude 能不用追问就一次完成吗？
	
错误 3：没给反馈循环
Claude Code 创始人 Boris Cherny 最强调的一点。让 Claude 写代码却不给验证手段 = 蒙眼编程。加上反馈循环后质量提升 2-3 倍。
修复：每个 prompt 后加 "跑测试并修复所有失败项"。更好的做法：写进 CLAUDE.md。
	
错误 4：没用 CLAUDE.md
每次新会话都在重复："用 TypeScript"、"只用函数组件"、"改完跑 Prettier"……CLAUDE.md 放在项目根目录，Claude 启动时自动读取。写一次，永远生效。
修复：现在就创建 CLAUDE.md，写入技术栈、代码规范和测试命令。
	
错误 5：直接在主分支改代码
Claude 默认在当前分支工作。出了问题你得回滚 commit。Worktree 让 Claude 在隔离副本上工作，满意再合并，出问题直接删。
修复：用 claude -w branch-name，特别是重构和新功能。
	
错误 6：每次权限提示都无脑点同意
又慢又不安全——你并没有认真审核，只是肌肉记忆在点。
修复：用 --permission-mode auto（AI 安全审核）或配置 allowedTools 精确控制权限。
	
错误 7：没用并行会话
一次只做一个任务是大项目的巨大瓶颈。Claude Code 支持多会话同时运行，各自在独立 worktree 中。3-4 个独立任务并行 = 轻松 3 倍加速。
修复：用 claude -w branch-name --background 并行执行多个独立任务。
