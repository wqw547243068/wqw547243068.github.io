...
tags: 图形学 opengl webgl three.js 游戏 vtk slam 大模型 高斯泼溅 机器人 浏览器
excerpt: 计算机图形学及相关应用
mathjax: true
permalink: /graphic
---
...
【2025-12-10】2026 CVPR Best Paper，谷歌 DeepMind 最新力作：[D4RT](https://d4rt-paper.github.io/)
- 快速、精准的全像素级感知，终于能够实时地理解动态的、复杂的环境。
- 开创性地将复杂的 4D 世界理解问题从需要多模型协作的"工程难题"转变为了一个高效的"搜索引擎式"查询问题，为 AI 真正理解和融入动态的物理世界铺平了道路
- 论文 [Efficiently Reconstructing Dynamic Scenes One 🎯 D4RT at a Time](https://storage.googleapis.com/d4rt_assets/D4RT_paper.pdf)
...
Web 端 3DGS 渲染有两种主流 LOD（Level of Detail）组织方式：
- Spark 2.0 采用 Splat-based LOD Tree：从单个高斯点（splat）粒度自下而上做合并。好处是逐层加载细节，但内存和显存开销大，扩展性偏弱。
- Aholo Viewer 采用 Chunk-based LOD Tree：把原始数据先切成 N 个 chunk（数据块），再分别为每个 chunk 生成不同层级的 LOD，运行时以 chunk 为单位做层级切换。

### 【2026-5-25】Aholo Viewer

Chunk-based 方案的两个关键优势：
- 开销更可控：以 chunk 为单位做层级切换，内存调度的颗粒度更粗、缓存命中更好
- 扩展性更强：chunk 是清晰的数据边界，未来扩展到城市级、街区级场景更容易

2026年5月25日，群核科技开源 **3D高斯浏览器** [Aholo Viewer](https://aholojs.dev/)，全面适配手机、电脑、VR 设备等终端，用户无需安装客户端，只需打开浏览器，即可流畅浏览超过 10 亿个高斯点的城市级超大3D场景 —— 这一规模相当于一整个杭州西湖的面积。
- 内存减半、渲染快3倍，让任何设备的浏览器流畅加载10亿+粒子超大3D场景。
- 让任何设备的浏览器流畅加载 10 亿+ 高斯粒子的超大 3D 场景。
- 性能测试中，内存占用只有李飞飞团队 Spark 2.0 的一半，渲染速度快 3 倍，容量上限大 10 倍。

简单理解：像刷视频一样在浏览器里流畅浏览超大的 3D 世界。
- 主页：https://aholojs.dev/zh-CN/
- 体验地址：https://aholojs.dev/zh-CN/playground/
- GitHub：https://github.com/manycoretech/aholo-viewer


应用场景
- 电商产品展示：商品从静态图变成可旋转的 3D 场景
- 线上看房/看车：用户直接在浏览器里漫游真实空间
- 博物馆线上展：3D 交互体验无缝融入日常浏览
- 数字孪生：城市级、建筑级的 3D 场景在浏览器中实时渲染
- UGC 3D 内容：普通用户用手机拍摄即可生成可分享的 3D 场景
