...
- [垃圾刷多了AI也会变蠢！“年度最令人不安的论文”](https://mp.weixin.qq.com/s/tG5PE-DtLUCmkbIm2U2Q3Q)

“脑损伤”（Brain Rot）
- 因为长期接触碎片化、低价值网络信息而逐渐变得记忆紊乱、注意力下降的情况
- 俗称：碎片化垃圾信息刷多了，抖音刷多了人会变傻，详见站内专题: [大脑工作原理](brain#看抖音停不下来)

2024年，这个词一度被选为牛津年度词汇。

【2025-10-15】研究显示，AI也一样。大模型灌多了垃圾内容也会变蠢降智脑损伤，而且后面变不回来了。
- 项目主页：[LLMS CAN GET “BRAIN ROT”!](https://llm-brain-rot.github.io/)
- 论文：[LLMS CAN GET “BRAIN ROT”!](https://arxiv.org/pdf/2510.13928)
...
<img width="821" height="326" alt="image" src="https://github.com/user-attachments/assets/6631f104-e324-4a8b-bf3e-862ac941e653" />
...
启发：
- 1、首次把“持续预训练的数据筛选”归为“训练时**安全问题**”，提醒行业不能只关注“训练后对齐”（如安全微调），更要在源头**把控数据质量**。
- 2、给大模型加上“认知体检”非常重要，建议部署大模型时使用 ARC、RULER等基准测试AI认知，避免AI长期接触低质量数据导致能力退化。
- 3、类似“热度”指标比文本长度更能判断数据质量，未来筛选训练数据时，可优先排除“短+高传播”的**碎片化内容**，尤其是社交平台数据。
...
### 【2026-9-8】Meta Autodata

【2026-9-8】[Meta提出Autodata！训练数据合成全面自动化](https://mp.weixin.qq.com/s/a7IZLpU5b3SRyT9j7OUbBQ)
- 论文：[Autodata: An agentic data scientist to create high quality synthetic data](https://arxiv.org/abs/2606.25996)

创新点
- 让AI代理充当数据科学家搭建优质训练与评估数据，还可通过元优化进一步迭代该数据科学家代理，产出更优质的数据内容。
- 在计算机科研、法律推理等多任务场景效果优于传统合成数据集方法，可将推理算力转化为更高质量的模型训练数据，有望变革AI数据构建模式。

Autodata 是一套通用方法，可让AI代理充当数据科学家，自主搭建高质量的训练数据与评估数据。在此基础上，研究团队提出了具体可落地的实践方案 Agentic Self-Instruct，还引入元优化策略对作为数据科学家的AI代理本身进行训练迭代，让该代理能够产出质量更高的数据。

研究将该方法在计算机科研、法律推理、数学对象推理等多类任务场景中开展验证实验，同时通过元优化进一步升级AI数据科学家代理，最终实现将增长的推理算力转化为更高质量的模型训练数据，相较于传统合成数据集构建方法取得了更优的实验效果。
