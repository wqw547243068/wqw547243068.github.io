...
【2025-12-16】整个互联网简单的分层定义
- [参考](https://zhuanlan.zhihu.com/p/1929446090530357651)

<img width="900" height="800" alt="image" src="https://github.com/user-attachments/assets/d1613c2b-5da9-4dd8-bf46-8609387c37e8" />

互联网的冰山：表网、深网与暗网

<img width="900" height="800" alt="image" src="https://github.com/user-attachments/assets/14d11a0a-8fcd-4f1b-8d89-5266ac5316b0" />

如果互联网是一座巨大的冰山
- 日常使用的微信、淘宝、微博，只是浮在水面上那一小角。
- 水面之下，隐藏着一个庞大却鲜为人知的深层网络——而暗网，则是这片深海中最为神秘的区域

![](https://www.secpulse.com/wp-content/uploads/2018/12/internet.png)

- `表网`（Surface Web）： 也称表层网络，熟悉的"阳光地带"。普通用户或者搜索引擎能直接访问的内容属于表网。表现形式为网页或者 APP 提供的内容。
- `深网`（Deep Web）： 潜藏在水面之下。不能被标准搜索引擎检索到的网络数据, 通常是存储在各公司或组织的数据库中，需要用专有的接口查询或干脆不对外查询，例如 Google 的后端数据库。
  - 深网数据量**远大于**表层网络，犹如海面和大海的关系。
- `暗网`（Dark Web）： 深网中最隐蔽的角落。必须使用特殊工具（如Tor浏览器）才能进入，普通搜索引擎无法索引。如同藏在深海沟壑中的秘密基地，位置和进入方式都高度加密。暗网通常具有匿名的特性，既保证访问者的匿名，同时也保证服务提供者的匿名。因此，其中充斥着各种犯罪信息和违法交易（*支、毒品、色情、暴恐、黑客等）。

关键区别： 
- 深网强调内容不公开，暗网则强调访问方式的匿名和隐蔽性。
- 大部分暗网内容也属于深网，但深网不全是暗网。

暗网有多个不同的实现版本，一般特指「`Tor 网络`」。
