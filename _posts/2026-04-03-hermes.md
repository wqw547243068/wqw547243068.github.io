...
【2026-7-2】Hermes Agent Chrome 浏览器扩展，装完直接在侧边栏就能干活
最实用的：
- 1️⃣ 随便哪个页面都能实时聊，不用来回切窗口
- 2️⃣ 本地模型一键换，Grok、GPT、Claude 想用哪个用哪个
- 3️⃣ 能做页面视觉分析加截图
- 4️⃣ 本地或者远程 Gateway 都连得上

直接对接 Hermes runtime，tools、skills、memory、MCP， 全继承过来，不用重新配
...
[Hermes 完全教程](https://mp.weixin.qq.com/s/rSpv1bSKffnor_LpN3_5eA)
...
[hermes入门指南](https://mp.weixin.qq.com/s/DrdgcuAYl034j7ZGEjjxAA)
...
Profile 把 Hermes「分身」成**多个独立账号**，每个账号有自己的配置、密钥、聊天记录、技能库，谁也看不到谁的。
...
【2026-6-27】Hermes Agent 正式支持混合智能体（Mixture of Agents，简称 [MoA](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/mixture-of-agents)） 做成虚拟模型，HermesBench 上比 Opus 4.8 强 8%、比 GPT 5.5 强 11%。

原理：
> 先让多个顶级参考模型并行思考（只给对话内容，不给工具），再由一个 aggregator（汇总模型）综合它们的结果去输出答案并调用工具。

不是自己从头训新大模型，而是把现有强模型聪明地组合起来用。

装个 Hermes Agent 后直接输入 `/moa` 就能切换（推荐先跑 hermes setup --portal 一键搞定）。

Hermes 将 MoA 预设为虚拟模型，用户通过 /model 命令或桌面应用的模型下拉菜单直接选用。
- 除了 `/model` 切换，还可用 `/moa [prompt]` 语法实现单次快捷调用, 系统在单轮对话中临时启用默认 MoA 预设，完成回答后自动恢复先前的模型。
MoA 运行机制，用户选定的预设中包含参考模型与聚合模型。
- 系统首先调用参考模型，输入不含系统提示词与工具调用历史的简化对话文本，让参考模型生成分析意见。
- 参考模型的分析意见随后被追加到最新一轮用户输入的末尾。
- 作为行动主体的聚合模型接收到分析意见后，在完整的工具 schema 与系统提示词下生成最终答复并执行工具调用。

HermesBench 基准测试中，使用 Claude-Opus-4.8 作为聚合模型、GPT-5.5 作为参考模型的 MoA 预设取得了 82.02% 的成绩，相比单独运行 Claude-Opus-4.8 提升了 6 个百分点（约 8%），相比单独运行 GPT-5.5 提升了 11%。

为保证提示词缓存（Prompt Cache）效率
- 参考模型的对话输入被简化以保持稳定缓存；
- 聚合模型则将参考意见置于 prompt 最尾部，确保历史对话前缀的字节稳定性，从而避免破坏已缓存的上下文。

用户可以通过运行 hermes moa list 查看预设，或使用 hermes moa configure [name] 新增和修改预设。

预设配置中，系统严禁递归嵌套，MoA 预设的聚合模型不能指向另一个 MoA 预设。当参考模型出现凭证失效等错误时，Hermes Agent 不会中断任务，而是将错误信息一并传入聚合模型的上下文并继续执行。目前 MoA 机制已在 CLI、网关、桌面端及 TUI 界面同步启用。
...
官方 [Bug: Iteration-exhaustion summary call has no output budget when agent.max_tokens is unset#35979](https://github.com/NousResearch/hermes-agent/issues/35979)
