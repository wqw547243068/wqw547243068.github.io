...
二者区别
- 分类是有监督的学习，同时学习每个类，二类问题就是正例和负例，多类就多个类别标签。
- 异常检测只学习正常样本，不学习不正常样本。

适应场景不同，假如样本里有三类A、B、C。
- 当A、B、C三类样本都可以学习时，就可以用分类模型学习。
- 只关心正常样本，其他类样本不容易学习时，就用异常检测。

比如A类样本是正常样本，B、C为异常样本。异常样本数量过少时，无法学习。而且异常样本**不服从** `独立同分布`而是由多个分布构成，甚至异常样本的分布未来还会变化。

又不关注异常样本具体是B类还是C类，只关心是非A即可，就学习A即可，此时就用异常检测。

### 异常检测为什么用聚类算法


为什么异常检测使用聚类算法而不是分类算法
- 正样本很容易收集，但负样本很难收集，或无穷无尽，没有办法让机器学习到每种负类样本的特征
...
三大类异常检测方法。前提是在假设数据集中大多数实例都是正常的前提下，
...
参考 [](https://blog.csdn.net/qq_45496282/article/details/120665498)
