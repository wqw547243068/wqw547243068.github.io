...
categories: 深度学习 
tags: 深度学习 NLP Transformer BERT GPT Attention BeamSearch seq2seq 杨植�? XLNet 循环智能 roformer rwkv 苏剑�? 检�? 芯片 序列�? 注意�? 三蓝一�? 帕累�? retnet yoco kan 通用逼近定理 叠加定理 样条 可视�? ttt 三蓝一�?
excerpt: Transformer 是大模型唯一选择么？
mathjax: true
...
然而，Transformer 并不完美，因为仅解决了所谓“`impossible triangle`”的**�?**条臂�?

“不可能三角”代表当前序列模型无法同时实�?**训练并行�?**�?**低成本推�?**以及**强大性能**的所�?3个期望维度�?
...
> 包括 GPT-4 在内的当�? AI 系统所做的正是压缩�?

提出的新深度网络架构 CRATE，通过数学方式验证了这一点�?
- CRATE 是一�?**白盒 Transformer**，其不仅能在几乎所有任务上�?**黑盒 Transformer** 相媲美，而且还具备非常出色的**可解释�?**�?

基于此，马毅教授还在 Twitter 上分享了一个有趣的见解�?
- 既然当前�? AI 只是在压缩数据，那么就只能学习到数据中的**相关�? / 分布**，所以就并不真正具备**因果或逻辑推理**或抽象思考能力�?

因此，当今的 AI 还算不是 AGI，即便近年来在处理和建模大量高维和多模态数据方面，深度学习在实验中取得了巨大的成功�?

这种成功归功于深度网络能有效学习数据分布�?**可压缩的低维结构**，并将该分布转换为简约（即紧凑且结构化的）表征。这样的表征可用于帮助许多下游任务，比如视觉、分类、识别和分割、生成�?
...
为了优化这个目标，提出学习一�?**增量映射序列**，模拟展开目标函数的某些类似梯度下降的迭代优化方案。这得到一个类�? Transformer 的深度网络架构，并且它完全是一个「白盒」—�? 其优化目标、网络算子和学习到的表征在数学上是完全可解释的�?

这个白盒深度架构命名�? `CRATE` �? `CRATE-Transformer`，这�? `Coding-RATE transformer` 的缩写。还通过数学方式证明这些增量映射在分布的意义上是可逆的，并且它们的逆映射本质上由同一类数学算子构成�?

因此，可以将几乎完全一样的 CRATE 架构用于编码器、解码器或自动编码器�?

## 模型结构

如果�? RetNet 是从**平行推理效能**的角度革新了网络架构，那�? BitNet 则从正交角度提升了推理效率�?
...
微软�? RetNet 位于这个“`impossible triangle`”的正中心，胜过了所有尝试过但未能实现这一壮举的方法。RetNet 设法在单个框架下实现所有属性�?
...
这速度比当前的 SOTA �?**几个数量�?**，同时还提供更好的性能！如果其他团队能够复制这一点并且进入开源领域，这将是巨大的进步，但目前微软绝对是「遥遥领先�?

RetNet的主要贡献可以概括为两大�?
- RetNet引入**多尺度保留机�?**来替�?**多头注意�?**。这是消除自注意力机制中的魔鬼这一组成部分的关键。尽管如此，这种保留机制有一个小小的理论上的缺点�?
- RetNet 适用于三种计算范式，而只有一�? Transformer 在训练和推理过程中使用相同的序列处理范式�?
  - A. **并行**表示使训练并行性能够充分利�? GPU 设备�?
...
�?2024-2-29】[BitNet b1.58：开�?1-bit大语言模型时代](https://mp.weixin.qq.com/s?__biz=MzAwMTA3MzM4Nw==&mid=2649498640&idx=1&sn=a860101ceee6bc3a777f465bdd1586da&chksm=82c7cd94b5b0448231f0017d2694e59f6e41369ea14a38a3a19a32a9ba18c3fe0f934e214bee&scene=21#wechat_redirect)
...
该模型每个参数仅使用三值表示，�?-1, 0 �? 1。因此，�? LLM 的矩阵乘法操作中只需要整数加法，而不需要任何浮点数乘法或加法。在语言模型困惑度和下游任务性能的评估中
- BitNet b1.58 能够与具有相同参数量和训练数据量的全精度（即FP16或BF16）Transformer LLM 相匹敌�?
- 与此同时，它在速度、内存使用、吞吐量和能耗等方面具有大幅优势�?

BitNet b1.58 为训练新一代高性能高效率的 LLMs 确立了新�?**扩展定律**（scaling law）和方法。此外引领了一种全新的计算范式，并为开发专�? 1-bit LLMs 优化的硬件设备铺平了道路�?

BitNet 是第一个支持训�?1比特大语言模型的新型网络结构，具有强大的可扩展性和稳定性，能够显著减少大语言模型的训练和推理成本�?

与最先进�?8比特量化方法和全精度 Transformer 基线相比，BitNet 在大幅降低内存占用和计算能耗的同时，表现出了极具竞争力的性能�?

此外，BitNet 拥有与全精度 Transformer 相似�?**规模法则**（Scaling Law），在保持效率和性能优势的同时，还可以更加高效地将其能力扩展到更大的语言模型上，从而让1比特大语言模型�?1-bit LLM）成为可能�?
...
微软亚洲研究院推出了一种创新性的 Decoder-Decoder 架构 `YOCO`（You Only Cache Once）。通过**自解码器**�?**交叉解码�?**的独特架构，YOCO 仅需缓存一次键值对，从而显著降�? GPU 内存的使用�?
- 论文 [You Only Cache Once: Decoder-Decoder Architectures for Language Models](https://arxiv.org/abs/2405.05254)

模型评估中，YOCO 展现出与同规�? Transformer 模型相媲美的性能，并在语言建模评估、模型大小扩展以及长上下文处理方面具有显著优势。特别是在降�? GPU 内存占用和缩短预填充延迟方面�?

YOCO 整体架构设计如下，分为`自解码器`（Self-Decoder）和`交叉解码器`（Cross-Decoder）两部分�?

YOCO 实现了�?**模型越大，内存越�?**”，为自然语言处理领域带来了全新的研究和应用范式�?
- YOCO 仅缓存一次键值对，可大幅降低 GPU 内存需求，且保留全局注意力能力�?

打破 GPT 系列开创的 `Decoder-Only` 架构——提�? `Decoder-Decoder` 新型架构，名�? `YOCO` (You Only Cache Once)�?
- 在处�? 512K 上下文长度时，标�? Transformer 内存使用�? YOCO �?6.4倍，预填充延迟是 YOCO �?30.3倍，�? YOCO 的吞吐量提升到标�? Transformer �?9.6倍�?
...
�?2021-3-23】Rotary Transformer，简�? `RoFormer`，是追一科技`苏剑林`自研的语言模型之一，主要是为Transformer结构设计了新的`旋转式位置编码`（Rotary Position Embedding，`RoPE`）�?
- `RoPE`具有良好的理论性质，且是目�?**唯一**一种用到线性Attention的绝对位置编码，目前来看实验结果也颇为不错�?
- 参考配置：�?24G显存�?3090上，跑maxlen=1024，batch_size能跑�?8以上�?
...
增大模型并不是提升性能的唯一路径，用一种搜�?/查询信息的方式来增强模型，小的生成语言模型也能达到之前大模型才能达到的性能�?

语言模型的任务是�?**填空�?**，这对于语言信息有意义，但是对于事实信息和世界知识信息是无效的�?
- 有时需要与事实有关的信�?

代表
- DeepMind �? RETRO Transformer
  - DeepMind �? RETRO（Retrieval-Enhanced TRansfOrmer）模型。该模型�? GPT-3 性能相当，但参数量仅�? GPT-3 �? 4%�?
- OpenAI �? WebGPT


### 2021.12.16 WebGPT

OpenAI 推出 WebGPT, 解决 long-form quesion-answering (LFQA) 的方�?, 开放域QA回复更长更可靠�?
...
WebGPT 思路更进一步，完全模拟了人使用搜索引擎的方�?(有更多action: 搜索、点击、翻页、回退等等)，而非仅生成search query并使用其结果�?
...
- �?2022-1-4】[参数量仅�?4%，性能媲美GPT-3：开发者图解DeepMind的RETRO](https://www.jiqizhixin.com/articles/2022-01-04-8)
...
结构
- RETRO �? **编码�? - 解码�?**模型，像原始�? Transformer�?
- 然而在检索数据库的帮助下增加�?**输入序列**�?
...
这里的数据库是一�?**键值存�?**（key-value store）数据库�?
- key 是标准的 **BERT 句子嵌入**，value 是由两部分组成的**文本**�?
- Neighbor，用于计�? key�?
- Completion，原文件中文本的延续�?

RETRO 数据库包含基�? MassiveText 数据集的 2 万亿个多语言 token。neighbor chunk �? completion chunk 的长度最多为 64 �? token�?
...
- 将这些添加到语言模型的输入中
  - 检索出的文本成�? RETRO 输入的一部分，Transformer �? RETRO 块将信息合并到它们的处理�?
  - ![](https://image.jiqizhixin.com/uploads/editor/ff98762d-0d34-4771-8753-56d6b7762648/1641280203796.png)


#### 高层次的 RETRO 架构

RETRO 架构由一�?**编码�?**堆栈和一�?**解码�?**堆栈组成�?
- 编码器由标准�? Transformer 编码器块（self-attention + FFNN）组成。Retro 使用由两�? Transformer 编码器块组成的编码器�?
  - 编码器堆栈会处理检索到的近邻，生成后续将用于注意力�? KEYS �? VALUES 矩阵
- 解码器堆栈包含了两种解码�? block�?
  - 标准 Transformer 解码器块（ATTN + FFNN�?
  - RETRO 解码器块（ATTN + Chunked cross attention (CCA) + FFNN�?
- 解码�? block �? GPT 一样处理输入文本。对提示 token 应用自注意力（因此只关注之前�? token），然后通过 FFNN 层。只有到�? RETRO 解码器时，它才开始合并检索到的信息。从 9 开始的每个第三�? block 是一�? RETRO block（允许其输入关注近邻）。所以第 9�?12�?15�?32 层是 RETRO block�?
...
- 最强的GPT-4也才最大支持一次处�?32k token，相当于50页文字�?
- 而能够只�?1分钟看完一本数万字小说的Claude，其token数也不过“才�?100k�?10万）�?

一次性扩展到10亿，并且这个数字理论上其实还是无限的，这不就意味着：不久的将来，整个语料库甚至互联网都能视为一个序列？

作者提出一个Transformer变体：`LongNet`，它应用了一种叫做�?**膨胀注意�?**（dilated attention）”的机制，可以随着距离的增长，让注意力场（模型感知范围）呈指数级扩展�?
...
多层感知器（MLP）被称为**全连接前�?**神经网络，是当今深度学习模型的基础构建块�?
...
MIT 提出�? KAN 灵感来源�? Kolmogorov-Arnold 表示定理的网络�?
- 论文：[KAN: Kolmogorov-Arnold Networks](https://arxiv.org/pdf/2404.19756)
- Github：[pykan](https://github.com/KindXiaoming/pykan)

KAN 在准确性和可解释性方面表现优�? MLP，而且能以非常少的参数量胜过以更大参数量运行的 MLP�?

有研究者将 KAN 创新架构的理念扩展到卷积神经网络，将卷积的经典线性变换更改为每个像素中可学习的非线性激活函数，提出并开�? KAN 卷积（CKAN�?
...
Kolmogorov 1957 年就发现�?**多层**神经网络，比 Rumerhart、Hinton �? William �? 1986 年论文发表的时间要早得多，但他却被西方忽视了�?

一种有前景的多层感知器（MLP）的替代方案，称�? Kolmogorov-Arnold Networks（KAN）�?
- MLP 的设计灵感来源于`通用近似定理` （通用逼近定理�?
- �? KAN 设计灵感则来源于 `Kolmogorov-Arnold 表示定理`�?

Kolmogorov-Arnold 表示定理
- Vladimir Arnold �? Andrey Kolmogorov 证明了如�? f 是一个在有界域上�?**多变量连续函�?**，那�? f 可以写成一�?**单变量连续函�?**�?**二元加法运算**的有限组合�?

�? MLP 类似，KAN 拥有**全连�?**结构。�? MLP 在节点（神经元）上放置固定激活函数，KAN 则在边（权重）上放置可学习的激活函数�?

因此，KAN **完全没有线性权重矩�?**�? [对比图](https://image.jiqizhixin.com/uploads/editor/2ea4a752-4eb5-4bd7-a21f-1f228efcc427/640.png)
- 每个权重参数都被替换为一个可学习的一维函数，参数化为**样条**（spline）�?
- KAN 的节点仅对传入信号进行求和，而不应用任何非线性变捀�?
- ![对比图](https://image.jiqizhixin.com/uploads/editor/2ea4a752-4eb5-4bd7-a21f-1f228efcc427/640.png)

尽管 KAN 数学解释能力不错，但实际上只�?**样条**�? **MLP** 的组合，利用了二者的优点，避免了缺点的出现�?
- 样条在低维函数上准确度高，易于局部调整，并且能够在不同分辨率之间切换。然而，由于样条无法利用组合结构，因此存在严�? COD 问题�?
- 另一方面，MLP 由于其特征学习能力，较少受到 COD 的影响，但在低维空间中却不如样条准确，因为它们无法优化单变量函数�?

KAN 的最大瓶�?: 训练速度慀�?
- 相同数量的参数下，KAN 的训练耗时通常�? MLP �? 10 倍�?
...
- `MQA` �? `GQA` 虽然在一定程度上可以减少KV Cache的占用，但效果通常不如 `MHA`�?
...
- MQA（multi-query attention）让所有的 head 共享 1 �? KV projection 矩阵�?
- GQA（grouped-query attention ）使�? 8 �? KV projections（选择8是因为A100 8GPUs�? 来减少内存占用�?

�? 30B 模型上训�? 150B tokens，发�? GQA 效果�? MHA 差不多，�? MQA 要好；在 1 个node�? 8 �? A100 GPUs 上推理速度 GQA �? MQA差不多，�? MHA 要好（MQA 在推理的时候，要把 KV projections 复制�?8张卡上）�?

#### MQA: Muti Query Attention

MQA �? 2019 年提出的一种新�? Attention 机制，其能够在保证模型效果的同时加快 decoder 生成 token 的速度�?
- 论文�? [Fast Transformer Decoding: One Write-Head is All You Need](https://arxiv.org/pdf/1911.02150.pdf)
- 所�? head 之间**共享**一�? key �? value 的参�?

MQA �? encoder 上的提速没有非常明显，但在 decoder 上的提速是很显著的
- ![](https://pic1.zhimg.com/80/v2-150a48c2eadeacd0aca50408ea391710_1440w.webp)

Multi Query Attention（MQA�? �? Multi Head Attention（MHA）只差了一个单词，从「Head」变成了「Query」�?

MQA �?**所有的头之�? 共享 同一�? Key �? Value 矩阵**，每个头只单独保留了一�? Query 参数，从而大大减�? Key �? Value 矩阵的参数量�?
- 「参数共享」并不是新奇思路，Albert 通过使用**跨层共享参数**（Cross-layer parameter sharing）方式来大大减少 bert 的参数量
- MQA 实际上是�? head 中的 key �? value 矩阵抽出来单独存为一份共享参数，�? query 则是依旧保留在原来的 head 中，每个 head 有一份自己独有的 query 参数�?
...
`MLA`(Multi-head Latent Attention) �? 杭州**深度求索**人工智能在`DeepSeek` V2 提出的一�?**注意力机制变�?**�?

MLA 解决推理过程�?, 由于attention机制�?**KV Cache占用过多内存**而导致的性能瓶颈问题�?

MLA 引入�?**低秩KV压缩**技术，有效减少了KV Cache 大小，从而缓解了这一问题�?
...
为什么LoRA提出这么久了，直�? MLA 才提出对KV Cache低秩分解的做�??
...
�?2023-12-19】美国芯片初创公�? [Etched AI](https://www.etched.ai/) 宣称开创了一项新的技术，�? Transformer 架构直接“烧录”到了芯片中?，创造出了世界上最强大的专门用于Transformer推理的服务器。可以运行万亿参数的模型�?? 甩英伟达icon几百条街?
- ![](https://assets-global.website-files.com/6570a6bdf377183fb173431e/6570b5e6b0cd5f0189cf79b8_hero.webp)

�? Transformer架构直接“烧录”到芯片中，这意味着Transformer模型的推理可以在专门的硬件上运行，而不需要依赖传统的CPU或GPU。这将大大提高推理速度，降低功耗，并提高模型的性能�?
- 解码速度远超 A100, H100: NVIDIA A100(1x) < NVIDIA H100(5x) < Etched Sohu(15+x)

功能�?
- ? **实时**语音代理：能够在毫秒内处理成千上万的词�?
- ? 更好的编码与**树搜�?**：可以并行比较数百个响应�?
- ? 多播推测解码：实时生成新内容�?
- ? 运行未来的万亿参数模型：只需一个核心，支持全开源软件栈，可扩展�?100T参数模型�?
...
�?2024-6-19】DeepMind 论文提出�?**混合架构**方法，解决Transformer模型�?**推理**缺陷�?
...
- 无需更改任何模型架构，吞吐量比原�? HF Transformers 高出**24�?**�?
...
为了解决这个问题，引入了 PagedAttention，一种受传统操作系统**虚拟内存**�?**分页**概念启发的注意力算法�?
- 与传统注意力算法不同，PagedAttention 允许�?**连续的键和值存储在非连续的内存空间**中�?
...
�?2023-7-4】[FasterTransfomer](https://github.com/NVIDIA/FasterTransformer) �? NVIDIA 高度优化�? Transformer 模型库，在生成时达到 **2.5�?**的速度，详�? [Inference with FasterTransformer](https://github.com/THUDM/GLM-130B/blob/main/docs/inference-with-fastertransformer.md) 

#### MHA -> DCMHA

KAN

�?2024-5-25】[ICML2024高分论文！大模型计算效率暴涨�?200%](https://mp.weixin.qq.com/s/8650CfLSSRUPfiYUTakkNQ)
...
彩云科技 对Transformer计算最耗时的核心组件—�?**多头注意力模�?**（MHA）下手，将Transformer计算性能提升了有2倍之高�?
...
承载Transformer计算量的核心模块�?**多头注意�?**（MHA）模块，位置（position=i）上的每一�?**注意力头**（attention head）会与全部位置上的注意力头计算出一个注意力分布矩阵�?
- 这个过程中，位置 i 上的各个注意力头计算出来的注意力分布矩阵是相互独立的�?

这种多头独立计算的机制会带来两大问题�?
- 低秩瓶颈（Low-rank Bottleneck）：注意力矩阵的秩较低，模型的表达能力受�?
- 头冗余（Head Redundancy）：不同的注意力头可能会学习到相似的模式，导致冗�?
...
论文通过实验表明�? `DCFormer` 在不同的架构和模型规模下，在语言建模方面显著优于Transformer，与计算量增�?1.7倍至2倍的模型性能相匹配�?

DCFormer可提�?70%~100%的模型计算效�?
- DCFormer 在不同参数规模下�?405M�?6.9B参数），�? Transformer �? Transformer++ 模型的性能提升显著
...
DCFormer在不同的架构和模型规模下，在语言建模方面显著优于Transformer，与计算量增�?1.7倍至2倍的模型性能相匹配�?


### 长度限制

文本长度一直是 transformer 的硬伤�?
- 不同�? RNN，transformer 在训练时必须卡在一�?**最大长�?**上，这将导致训练好的模型无法在一个与训练时的长度相差较远的句子上取得较好的推理结果�?

Transformer 中，由于 token �? token 之间是没有顺序之分的. 因此，通常在输入添�? Position Embedding 来表征每一�? token 在句子中的位置�?
...
- 正弦位置编码：这是早�? transformer 使用的位置编码，论文中有尝试做实验，这种编码会随着训练/预测时的文本长度差异增大，（超过 50 个token 后）性能显著下降�?
- 旋转编码：论文中提到这种方式是比较不错的，只不过因其在每一层都要做一次向量旋转，从而降低训练和推理的速度�?

transformer 这类模型�? 时间复杂度、内存使用复杂度都是 n^2（n为序列长度）
- 当序列长度超�? 512 时，模型对算力的要求将会大幅提高�?

最近一些文�? Longformer, Performer, Reformer, Clustered attention 都试图通过近似全注意力机制改善该问题�?
...
- 自注意力的计算复杂度�? `O(N^2)` ，当包含长句时，内存使用量会随着输入量的增加而呈4倍增长�?

Longformer 是基�? Transformer 的可扩展模型，用于处�?**长文�?**，可轻松执行各种文档�? NLP 任务，而无需对长输入进行分块或缩短，也无需使用复杂的架构来组合各块信息�?
...
效果
- Longformer 还在 text8 �? enwik8 任务中取得了最佳性能�?
- Longformer 在长文档表现一直优�? RoBERTa，并且在预训练后�? WikiHop �? TriviaQA 任务中表现最佳�?

RoBERTa 只有 512 个位置嵌入，因此需要复�? 8 个位置嵌入来容纳 4096 个字。尽管它很简单，但据称却非常有效，这显然是因为复制消除了分区边界�?

#### �?2021-1-8】谷�? BigBird


�?2021-1-8】谷歌推�? BigBird, 基于**稀疏注意力**的Transformer，将基于Transformer的模型（例如 BERT）扩展到更长的序列�?
- 平方级别的依赖降成线�?
- 同等硬件条件下，长度扩充8�?
...
开源中�? bigbird 预训练模型，从tiny至base�?5个级别预训练模型。可从[huggingface hub](https://huggingface.co/models?language=zh&sort=downloads&search=bigbird)直接下载使用

BigBird 模型实现了三种注意力机制�?**随机注意�?**�?**窗口注意�?**�?**全局注意�?**，这与LongFormer几乎相似

与BERT同等计算力下，可处理序列长度达到4096�?
- 很多长文本序列的任务上达到SOTA效果，例如：长文本摘要、长文本问答�? 
- BigBird RoBERTa 模型在Transformers仓库中使用�?

BigBird的注意力机制是一个近似BERT�?**全注意力机制**，因此不是比BERT的注意力机制效果更好，而是**运行效率更高**�?
- BERT的注意力机制存储与序列长度是二次方关系，在长文本情况下的存储需求就已经开始令人难以忍�?
- �? BigBird �? block sparse attention 就是为了解决这个问题。无限长长度序列上，计算无穷�? 次时，把BERT的全注意力机制换�? block sparse attention�? 
...
ALiBi �? 2022 年提出的一种方法，解决 transformer **训练和推理时文本长度不一�?**的难题，
- 论文中在训练时候使�? 1024 的最大长度，但在推理时用 2048 的最大长度推理，并且�? PPL 指标持平�?
- ALiBi 都是在测试集的句子最大长度的「一半长度」上进行训练，�? Sinusoidal 则是正常在「测试集长度」上进行训练�?
- [TRAIN SHORT, TEST LONG: ATTENTION WITH LINEAR BIASES ENABLES INPUT LENGTH EXTRAPOLATION](https://arxiv.org/pdf/2108.12409.pdf)

如何实现�?
- ALiBi 实现思路很直觉，模型在接收输入时直接去掉 Position Embedding 向量，而是�? Attention 中计�? query·Key 的值后面加入一个偏置常量（非训练变量），来达到注入位置信息的效果。这个常量是一�? 事先计算�? 的数值，并且每个头（head）的值都有所不同�?
...
�?2024-4-11】[Google 提出Infini-Transformer架构，可让LLMs处理无限长上下文，内存节�?114倍](https://mp.weixin.qq.com/s/factToEEJdWcs5WJG1Ljfg)
- [Leave No Context Behind: Efficient Infinite Context Transformers with Infini-attention](https://arxiv.org/pdf/2404.07143.pdf)

对于批量大小�? 512、上下文长度�? 2048 �? 500B 模型，注意力键�? (KV) 状态的内存占用�? 3TB
...
- 每个片段�? 计算 standard causal 点积attention context（注意力上下文）。因此，点积注意力计算在某种意义上是**局�?**的，覆盖了索引为 S 的当前片段的总共 N 个标记�?
- 然而，局部注意力在处理下一个片段时会丢弃前一个片段的注意力状态。在Infini-Transformer中，并没有忽略旧的键值（KV）注意力状态，而是通过内存压缩技术重新使用它们来保持整个上下文历史�?
- 因此，Infini-Transformer的每个注意力层都具有**全局**压缩�?**局�?**细粒度状态，这就是前面提到的无限注意力（Infini-attention）�?
...
问题
- 长上下文的挑战是 RNN 层本质上所固有的：与自注意力机制不同，RNN 层必须将上下文压缩为固定大小的隐藏状态，更新规则需要发现数千甚至数百万�? token 之间的底层结构和关系�?

斯坦福大学、加州大学伯克利分校、加州大学圣迭戈分校�? Meta 设计了一种新架构 TTT，用**机器学习模型**取代�? **RNN 隐藏状�?**�?
- 该模型通过输入 token 的实际梯度下降来压缩上下文�?
- 测试时训练（Test-Time Training�?
- TTT 层直接取�? Attention，并通过表达性记忆解锁线性复杂性架构，使我们能够在上下文中训练具有数百万（有时是数十亿）个 token �? LLM�? 

TTT 层作为一种新的信息压缩和模型记忆机制，可简单地直接替代 Transformer 中的自注意力层�?
- �? Mamba 相比，TTT-Linear 的困惑度更低，FLOP 更少（左），对长上下文的利用更好（右）：

全新的大语言模型（LLM）架构有望代替至今在 AI 领域如日中天�? Transformer，性能也比 Mamba 更好�?
- 论文：[Learning to (Learn at Test Time): RNNs with Expressive Hidden States](https://arxiv.org/abs/2407.04620)
- 代码�? jax 训练和测试：[ttt-lm-jax](https://github.com/test-time-training/ttt-lm-jax)
...
- DeepSpeed提供�? **稀�? attention kernel** —�? 支持**长序�?**模型输入，包括文本输入，图像输入和语音输入�?
...
- `Sparse Self Attention`: OpenAI在image transformer中引入了Sparse self-attention，把两者结合在一块，既可以学习到局部的特性，又可以学习到远程稀疏的相关�?
...
Attention的核心在于Q,K,V 三个向量序列的交互和融合，其中Q,K 的交互给出了两两向量之间的某种相关度（权重），而最后的输出序列则是把V按照权重求和得到�?

理论上，Self Attention **计算时间**�?**显存占用�?**都是 ?(n^2) 级别的（n是序列长度）
- 如果序列长度变成原来�?**2�?**，显存占用量就是原来�?**4�?**，计算时间也是原来的**4�?**�?
- 当然，假设并行核心数足够多的情况下，计算时间未必会增加到原来�?4倍，但是显存�?4倍却是实实在在的，无可避免，这也是微调Bert时OOM的原因�?

为什么是 ?(n^2)�?
- 要对序列中的任意两个向量都要计算相关度，得到一�?$n^2$大小的相关度矩阵
- ![](https://spaces.ac.cn/usr/uploads/2019/07/775103900.png)
- 左边显示�?**注意力矩�?**，右变显示了**关联�?**，这表明每个元素都跟序列内所有元素有关联�?

所以，节省显存，加快计算速度，一个解法是**减少关联性计�?**
...
### Atrous Self Attention 膨胀注意�?

Atrous Self Attention，�?**膨胀**自注意力”、�?**空洞**自注意力”、�?**带孔**自注意力”等�?
- 名称是自定义, 原论文《Generating Long Sequences with Sparse Transformers》没有出现过这两个概�?
...
### Local Self Attention 局部自注意�?

Local Self Attention，中文称“局部自注意力”�?
- **自注意力**机制在CV领域统称为“Non Local�?
- 而Local Self Attention则要放弃全局关联，重新引�?**局部关�?**。约束每个元素只与前后k个元素以及自身有关联，如下图所示：
- ![](https://spaces.ac.cn/usr/uploads/2019/07/713126535.png)
- Local Self Attention的注意力矩阵（左）和关联图示（右�?
- 从注意力矩阵来看，就是相对距离超过k的注意力都直接设�?0�?

其实 Local Self Attention 跟普通卷积很像了，都是保留了一�? 2k+1 大小的窗口，然后在窗口内进行一些运算，不同的是普通卷积是把窗口展平然后接一个全连接层得到输出，而现在是窗口内通过注意力来加权平均得到输出。对于Local Self Attention来说，每个元素只�? 2k+1 个元素算相关性，这样一来理想情况下运行效率和显存占用都变成�? ?((2k+1)n)??(kn) 了，也就是说随着n 而线性增长，这是一个很理想的性质——当然也直接牺牲了长程关联性�?

### Sparse Self Attention -- OpenAI改进，综合以上两�?

现在可以很自然地引入OpenAI�? Sparse Self Attention了�?
- Atrous Self Attention 有一些洞，�? Local Self Attention正好填补了这些洞，所以一个简单方式就是将Local Self Attention和Atrous Self Attention 交替使用，两者累积起来，理论上也可以学习到全局关联性，也省了显存�?
...
从注意力矩阵上看就很容易理解了，就是除了相对距离不超过k的、相对距离为k,2k,3k,�? 的注意力都设�?0，这样一来Attention就具有“局部紧密相关和远程稀疏相关”的特性，这对很多任务来说可能是一个不错的先验，因为真正需要密集的长程关联的任务事实上是很少的�?
...
Transformer 原始论文发表之后，「Generating Wikipedia by Summarizing Long Sequences」提出用另一�? transformer 模块�?**排列方式**来进行语言建模
- 直接扔掉了所有的 transformer 编码器模块……「Transformer-Decoder」模型�?

早期的基�? transformer 的模型由 6 �? transformer 解码器模块堆叠而成�?
- ![](https://pic3.zhimg.com/80/v2-19720b1c70a294558dc9456477156b06_1440w.webp)

解码器模�?
- �? transformer 原始解码器模块相比，去掉了第二个自注意力层�?

一个相似的架构�?**字符**级别的语言建模中也被验证有效，使用更深的自注意力层构建语言模型，一次预测一个字�?/字符�?

所有解码器模块都一样。使用带掩模的自注意力层�?
- 该模型在某个片段中可以支持最�? **4000** 个单词的序列，相较于 transformer 原始论文中最�? **512** 单词的限制有了很大的提升�?
