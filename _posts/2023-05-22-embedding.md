...
| 方法 | 原理 | 结果含义 | 推荐场景 |
| :--- | :--- | :--- | :--- |
| **余弦相似度** | 计算两向量夹角的余弦值，**关注方向，忽略大小** | 取值范围 `[-1, 1]`，越接近 1 代表向量方向越一致、越相似 | 文本搜索、NLP 首选（比如语义向量匹配） |
| **欧氏距离** | 两点之间的直线距离，**关注绝对位置差异** | 取值范围 `[0, +∞)`，越接近 0 代表两点越近、越相似 | 图像检索、坐标系数据（比如特征点匹配） |
...
所以，不管使用 openai api embedding 功能还是直接通过 向量数据库 直接查询，都需要将加载进来的数据 Document 进行**向量化**，才能进行向量运算搜索。
...
- 项目地址：[turbovec](https://github.com/RyanCodrai/turbovec)
