...
label_info = {"annotator": ['小王', '小明', '小张', '小赵'], "pass_rate":0.75, "inspector": ['小李'], "busy_pool": []}
...
for task in task_list:
    ...
    for name in label_info['annotator']:
        # 检查哪些人空闲
        if name in label_info['busy_pool']:
            continue
        lock.acquire(True) #锁定
        label_info['busy_pool'].append(name)
        lock.release()#释放
        # 发布任务
        res = getLLM(model, system_prompt_a.replace('[query]', task))
        task_result.append(res)

        lock.acquire(True) #锁定
        label_info['busy_pool'].remove(name)
        lock.release()#释放
        #print(f'{name}\t{res[1]}\t{task}')
