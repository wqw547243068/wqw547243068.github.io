...
【2026-5-28】[框架评测](https://mp.weixin.qq.com/s/is7zXXH5rucs5OiIazsTTw)

Qwen3.6-27B 堪称 RTX 3090 24G 显存显卡本地部署的首选 —— 稳定可用、不闪退。

相同硬件环境与相同任务条件下，对 Qwen3.6-27B Q4 量化模型进行了三种主流本地 AI 工具的性能测试结果如下
- llama-cpp 整体性能表现最佳，在推理速度、资源利用率以及运行效率方面都更具优势。更适合对性能要求较高的本地部署场景，例如高并发推理、服务化部署以及专业开发环境。
- LM Studio 紧随其后，整体性能接近 llama-cpp。最大优势在于**图形化界面与易用性**，用户无需复杂命令即可快速加载和管理模型，适合日常使用、模型测试以及桌面端 AI 应用场景。
- Ollama 明显低于前两者，但其优势在于**部署简单、命令直观、生态成熟**。对于希望快速体验本地大模型的用户而言，Ollama 学习成本最低，更适合作为轻量化、本地化 AI 工具使用。

| 框架 | model | avg(b.eval_tps) |
| --- | --- | --- |
| llama-cpp | Qwen3.6-27B-Q4_K_M.gguf | 30.26 |
| LM Studio | qwen/qwen3.6-27b | 28.76 |
| ollama | qwen3.6:27b | 16.46 |

选型
- 追求极致性能与部署灵活性，推荐 llama-cpp；
- 兼顾性能与易用性，推荐 LM Studio；
- 看重快速安装与简单操作，则 Ollama
