...
categories: 大模型
tags: vllm bert 分布式 moe deepseek
...
【2026-4-25】[vLLM 0.20.0 实战：2026最新大模型推理部署全流程（避坑+调优）](https://mp.weixin.qq.com/s/nXQIpeTyoEkuCBV7asyfYw)

vLLM 迭代到0.20.0+、新模型架构层出不穷（DeepSeek-V3、Qwen3.6、Gemma 4），部署仍面临版本兼容、参数配置、性能调优等挑战。

版本选型：选对版本少走90%的坑

关键结论：
- v0.18.0 是2026年生产部署的分水岭
- v0.20.0 带来2-bit KV缓存革命性优化。
- Qwen3.6 系列在开源模型中性能领先。

| 模型系列          | 推荐vLLM版本 | 关键特性支持                     | 显存参考（FP16） | 推荐硬件         |
|-------------------|--------------|----------------------------------|------------------|------------------|
| Qwen3.6-35B-A3B   | ≥0.20.0      | MoE、Thinking保留、262K上下文    | 50–60GB          | A100/H100 80G×1  |
| DeepSeek-V3       | ≥0.20.0      | 专家并行、Chunked Prefill        | 单卡≥80GB        | 8×H100 80G       |
| Gemma 4-26B       | ≥0.20.0      | 多模态、工具调用修复             | 52GB             | H100 80G×1       |
| Qwen3.5-27B       | ≥0.18.0      | FlashAttention 4、完整支持      | 54GB             | A100 80G×1       |
| MiniMax M2.5      | ≥0.19.0      | 专家并行、工具调用解析           | 220GB+           | 多卡并行         |
...
【2026-7-2】vllm （[v0.24.0](https://github.com/vllm-project/vllm/releases)以上）支持DSpark

官方验证 DeepSeek-V4-Pro-DSpark (NVIDIA 8×B300 GPUs):
- 推理速度 ~250 tokens/s ，batch size =1
- 投机解码接受长度 ~5，比MTP高12-42%
- vLLM官方[X帖子](https://x.com/vllm_project/status/2072545387639189798)
