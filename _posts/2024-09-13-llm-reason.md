...
大语言模型 × 因果推断：谁在因果谁?
随着 ChatGPT、Claude、Gemini 等大语言模型（LLM）日益强大，学界开始探索其在因果推断 中的潜力与挑战：一方面，我们想利用因果推断提升语言模型的鲁棒性、解释性与可靠性；另一方面，也希望借助 LLMs 强大的知识能力辅助因果结构发现、反事实生成与干预决策。比如,
	
1. 利用 LLM 进行因果结构建模
LLMs 可以辅助识别变量之间的因果关系，特别是在文本、知识图谱或非结构化数据中。例如：
•	从文献中提取“X 导致 Y”的显性/隐性结构
•	利用多轮问答评估变量之间的干预关系
代表工作：
•	LLM作为“因果判断者”（e.g., “Can language models infer causality?” NeurIPS 2022）：探索 LLM 在结构学习中的 prompt engineering 与 few-shot 表现。
•	CausalQA 数据集：训练 LLMs 在问答框架中识别干预与反事实关系。
	
2. 将因果推断方法嵌入 LLM 系统
目标是让语言模型不仅“预测”，还能“解释+干预”：
