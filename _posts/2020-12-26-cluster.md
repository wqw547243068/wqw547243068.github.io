...
categories: 机器学习
tags:  无监督学习 聚类 评估 sklearn 文本相似度 kd nsw knn 群体 集体 异常检测
...
聚类算法对比总结, sklearn [Overview of clustering methods](https://scikit-learn.org/stable/modules/clustering.html)，[知乎](https://zhuanlan.zhihu.com/p/637550803)
- ![知乎图](https://pic4.zhimg.com/v2-3588231ff56f7d0979d53dd132cfac03_r.jpg)
- ![原版对比图](https://scikit-learn.org/stable/_images/sphx_glr_plot_cluster_comparison_0011.png)
- 任意形状聚类： 谱聚类(`Spectral Clustering`)、密度聚类（`DBSCAN`/`HDBSCAN`/`OPTICS`）
  - 谱聚类: 不适合密集平面集, 不会识别噪声点, 需要指定聚类数目
  - 密度聚类: 整体效果最好, `HDBSCAN`/`OPTICS`, 需要指定最小成员数/近邻数
- 凸集聚类
...
![](https://pica.zhimg.com/v2-e16276e1f41a7312dad5226dc5193922_1440w.jpg)


### 扭矩聚类

【2025-2-17】[AI扭矩聚类算法：自主学习、无需标注，准确率高达97.7%的颠覆性突破](https://www.163.com/dy/article/JOA0KCQK0538J014.html)

`悉尼科技大学`研究人员开发了一种名为“**扭矩聚类**”（Torque Clustering）的全新 AI 算法，不需要**人工干预**，可以极大提升 AI 系统自主学习和识别数据模式的能力。

该算法模拟自然智能，在测试中准确率高达 **97.7%**，超越现有方法，有望引领 AI 学习的范式转变。

悉尼科技大学的杰出教授`CT Lin`表示：
- “在自然界中，动物通过观察、探索和与周围环境互动来学习，而不需要明确的指示。
- 几乎所有AI技术都依赖于‘`监督学习`’，需要人类使用预定义的类别或标签来标记大量数据，以便AI能够进行预测和发现关系。
- 然而，监督学习存在许多局限性：标记数据不仅成本高、耗时长，而且在处理复杂或大规模任务时往往不切实际。相比之下，无监督学习不需要标记数据，能够直接揭示数据集中的固有结构和模式。”
- 下一代人工智能——‘`无监督学习`’，正是试图模仿这种自然的学习方式。”

Torque Clustering 方法的论文，刚发表在人工智能领域的顶级期刊《`IEEE模式分析与机器智能学报`》上。
- Jie Yang, Sheng-Ku Lin, "[Shedding New Light on Traditional Image Clustering: A Non-Deep Approach With Competitive Performance and Interpretability](https://ieeexplore.ieee.org/document/10831855)", 2024 IEEE
- 非深度学习模型 MaxFeature Torque Clustering (MFTC) 在图像聚类任务中达到sota
- 【2025-2-11】悉尼科技大学官网 [Truly autonomous AI is on the horizon](https://www.uts.edu.au/news/tech-design/truly-autonomous-ai-horizon)
- 代码实现 [TorqueClustering](https://github.com/JieYangBruce/TorqueClustering)

该算法通过快速查找**质量和距离峰值**来实现自主聚类，表现优于传统的无监督学习方法，具有潜在的范式转变意义。它完全自主、无需参数调整，并且能够以极高的计算效率处理大型数据集。

扭矩聚类的独特之处:
- 基于物理学中扭矩概念，自主识别聚类，并无缝适应不同**形状**、**密度**和**噪声程度**的数据类型。
- 高效地处理**大型**数据集。与传统监督学习相比，扭矩聚类**无需人工标注数据**即可识别模式，使其更具扩展性和效率。对于复杂或大规模的任务，监督学习需要大量人工标注的数据，成本高、耗时长，且不切实际。而扭矩聚类算法的诞生，无疑为这些问题提供了完美的解决方案。

灵感源于**星系合并**过程中引力相互作用的扭矩平衡，基于质量和距离宇宙的两个基本属性，通过模拟自然界中的学习方式，让 AI 像动物一样通过观察、探索和与环境互动来学习。

扭矩聚类算法在 1000 个不同的数据集上进行了严格测试，平均调整互信息（AMI）得分高达 97.7%，而其他最先进的方法得分仅在 80% 左右。

应用范围极其广泛。
- 不仅适用于生物学、化学、天文学、心理学，还适用于金融和医学等领域。
- 通过发现疾病趋势、识别**欺诈**活动和理解人类行为等，扭矩聚类算法的应用场景不胜枚举。


然而，AI扭矩聚类算法并非无所不能。
- 尽管它在某些领域表现出色，但在处理**某些特定类型**的数据时可能效果不佳。
- 算法的准确率受到**数据集质量**的影响，如果数据集存在较大噪声或误报，算法的表现可能会受到影响。
...
4种点的关系
- ![](https://picx.zhimg.com/80/v2-8eccee4fb980350ea6ce7ad31873495e_1440w.webp?source=1def8aca)
...
1. DBSCAN 可对**任意形状**的稠密数据集进行聚类，而 k-means 之类的聚类算法一般只适用于**凸数据集**
2. 聚类同时发现**异常点**，对数据集中的异常点不敏感。
3. 聚类结果没有偏倚，而 k-means 之类算法的初始值对聚类结果有很大影响。

结论：聚类效果好于K-means

![](https://pic1.zhimg.com/80/v2-a8cd7414cf0e670f5899bd020a737a11_1440w.webp?source=1def8aca)
...
- 当样本集**密度不均匀**、**聚类间距差相差很大**时，DBSCAN 算法聚类效果不好
...
- 数据量过大时, 容易报错**内存溢出**，聚类收敛时间较长，故可能需要数据削减策略
  - 改进：对搜索最近邻时建立的**KD树**或者**球树**进行规模限制来改进。
- 聚类效果依赖于**距离公式**的选取，实际中常用的距离是**欧几里得距离**，由于‘`维数灾难`’，距离的度量标准已变得不再重要。
  - 分类器性能随着特征数量的增加而不断提升，但过了某一值后，性能不升反而下降，这种现象称为**维数灾难**。对于维度灾难的理解：[维度灾难的理解](https://blog.csdn.net/ustbbsy/article/details/80961157)
...
选型速查
- **大规模工业图（百万节点级）** → LPA / Leiden / Louvain
- **有向流网络（引用、交通、信息流）** → Infomap
- **追求统计严谨、可解释性** → SBM（graph-tool）
- **中小规模、需要理论保证** → 谱聚类
- **快速基线、无调参** → LPA

| 方法 | 原理 | 实施方案（工具/代码） | 优点 | 缺点 |
|------|------|----------------------|------|------|
| **有向谱聚类**<br>Directed Spectral Clustering | 将邻接矩阵构造为**厄米特矩阵（Hermitian）**或对转移矩阵做SVD分解，取前k个特征/奇异向量做K-means聚类；利用图的谱性质捕捉有向连通结构 | **igraph**：`spectral_community()`<br>**scikit-learn** + 手动构造对称化拉普拉斯<br>**Python**：`sklearn.cluster.SpectralClustering`（需先对称化处理有向图） | 数学基础扎实，聚类结果理论有保障；适合中等规模稠密图；能发现非球形社区 | 时间复杂度高（O(n³)），大图吃力；需预先指定k；对稀疏有向图效果不稳定 |
| **有向模块度优化（Louvain）**<br>Directed Modularity Louvain | 最大化**有向模块度Q**（社区内部边数与随机期望的差值），通过局部节点移动+社区聚合两层迭代贪心优化；有向版本分别计入出/入度 | **igraph**：`cluster_louvain(G, directed=True)`<br>**networkx**：`community.louvain_communities(G)`（支持DiGraph）<br>**Python**：`python-louvain` / `community` 包 | 速度快，接近线性时间；无需指定社区数；加权图友好；工业界最常用 | 存在分辨率限制（小社区易被吞并）；可能产生内部不连通社区；局部最优非全局最优 |
| **Leiden算法**<br>（Louvain改进版） | 在Louvain基础上增加**细化（refinement）**阶段，保证社区强连通；支持CPM（Constant Potts Model）质量函数解决分辨率问题；有向图通过有向模块度适配 | **igraph**：`cluster_leiden(G)`<br>**leidenalg** Python包：`find_partition(G, ModularityVertexPartition)`<br>**networkx**：`community.leiden_communities(G)` | 比Louvain更快、质量更高；保证社区连通性；可调分辨率参数；支持有向/加权图 | 调参（gamma分辨率）需经验；社区数量受参数影响大；仍属贪心近似 |
| **Infomap（Map Equation）** | 基于**信息论**：将图上随机游走的路径编码压缩，最小化描述长度（社区内编码+社区间切换编码）；天然支持有向边，按转移概率计算信息流 | **igraph**：`cluster_infomap(G, directed=True)`<br>**Python**：`infomap` 独立库<br>R：`igraph::cluster_infomap` | 天然支持有向、加权图；对层级流结构敏感；适合交通/引用/信息流网络；无需指定k | 对稀疏图噪声敏感；社区数量不可直接控制；模块度指标上表现不如Louvain |
| **标签传播 LPA**<br>Label Propagation | 每个节点初始唯一标签，每轮迭代采纳**邻居中出现最多的标签**；密集连接组快速达成标签共识；有向图可按入边/出边权重传播 | **igraph**：`cluster_label_prop(G)`<br>**Neo4j GDS**：`gds.labelPropagation.stream`<br>**networkx**：`community.label_propagation_communities(G)` | 近线性时间O(m)，极快；无需参数、无需目标函数；适合超大规模图；天然支持有向 | 结果不稳定（随机性强）；可能产生巨型社区；二部图上标签震荡不收敛；质量一般 |
| **Girvan-Newman**<br>（边介数法） | 反复移除**介数中心性最高**的边（有向边按最短路径经过次数计算），直到图分裂为多个连通分量；自顶向下分裂式 | **igraph**：`community_edge_betweenness(G, directed=True)`<br>**networkx**：`community.girvan_newman(G)` | 原理直观；能生成层次化社区结构（树状图）；适合小图精细分析 | 复杂度极高O(m²n)，大图不可用；需人工选择切割层数；仅识别弱连通分量 |
| **随机块模型 SBM**<br>Stochastic Block Model | 概率生成模型：假设同社区节点间连边概率由块矩阵B决定；通过**极大似然/变分推断/MCMC**反推节点所属社区；有向版本B矩阵非对称 | **graph-tool**：`minimize_blockmodel_dl(G)`（推荐，支持有向度校正SBM）<br>R：`sbm`包 / `noisySBM`<br>**Python**：`graph-tool` / `sknetwork` | 统计推断框架，可评估模型置信度；自动确定社区数（贝叶斯版本）；支持度校正、有向、加权 | 计算量大、收敛慢；初始化敏感；小图效果不稳定；graph-tool安装复杂 |
| **双模块度 Bimodularity** | 针对有向图定义**发送社区-接收社区**双向划分，分别优化出边和入边的模块度指标Q_bi；通过SVD奇异向量谱方法求解 | **Python**：参考PNAS论文自实现（`scipy.sparse.linalg.svds`）<br>R：暂无成熟包 | 专门为有向图设计，充分利用方向信息；能发现源/汇型社区结构；谱方法效率尚可 | 工业界工具少，需手写代码；理解门槛高；适用场景偏学术 |
...
ANN（Approximate nearest neighbor）是一系列用于解决**最近邻查找**问题的近似算法，通过牺牲**精度**来换取**时间**和**空间**的方式从大量样本中获取最近邻。 
...
【2024-7-17】[HNSW算法原理](https://zhuanlan.zhihu.com/p/441470968)
...
- ![](https://pic1.zhimg.com/80/v2-695d9072fdc29168027e3183b7c04f30_1440w.webp)

如图所示，向图中逐个插入点，对于每个新插入点，使用近似kNN搜索（approximate kNN search）算法从图中找到与其最近f个点集合，该集合中所有点与插入点连接。

通过上述方式构建的是近似`Delonay图`，与标准Delonay图相比，随着越来越多的点被插入到图中，初期构建的短距离边会慢慢变成长距离边，形成可导航的小世界，从而在查找时减少跳数加快查找速度。
...
![](https://pic4.zhimg.com/80/v2-7e5c1c494ed1f3d760ed62db7c5548e7_1440w.webp)
...
建图流程
- ![](https://pic2.zhimg.com/80/v2-c77f738919460c2dac11165ec3921c9d_1440w.webp)

搜索流程
- ![](https://pic2.zhimg.com/80/v2-b61914c0ce49c79992c0cf59a1262f49_1440w.webp)


#### 实现

- C++ 版本: [hnswlib](https://github.com/nmslib/hnswlib)
