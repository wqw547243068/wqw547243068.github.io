...
【2025-11-13】通用Agent能力包含哪些？
1. 工具调用能力：要能准确地调用工具，知道什么时候调用什么工具，什么时候不调用工具，能准确地提取工具需要的参数；
2. 规划能力：要能准确地进行规划，在规划出问题时，能重新进行规划；
3. 长上下文记忆能力：Agent往往需要几十轮的交互，而且包括工具的输出。所以模型需要记住上下文，并能根据上下文进行推理；
4. 代码能力：Agent的一大用途是用于编程，而且程序可以解决很多问题，因为Agent应具备强大的代码能力；
5. 多模态能力：这个可能非必要，但是实用，现在的Agent往往需要处理工作中出问题的截图，所以需要多模态能力；

agent能力逐步被基座模型吸收，如 deepseek r1、qwen3、kimi等
