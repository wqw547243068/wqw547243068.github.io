...
-   **[ScaNN](https://link.zhihu.com/?target=https%3A//github.com/google-research/google-research/tree/master/scann)**（Scalable Nearest Neighbors）：ScaNN的主要创新在于各向异性向量量化。它将数据点 $x_i$$x_i$x\_i 量化为 $\tilde{x}_i$$\tilde{x}_i$\\tilde{x}\_i ，使得内积 $\langle q, x_i \rangle$$\langle q, x_i \rangle$\\langle q, x\_i \\rangle 尽可能与原始距离 $\angle q, \tilde{x}_i$$\angle q, \tilde{x}_i$\\angle q, \\tilde{x}\_i 相似，而不是选择最接近的量化质心点。
