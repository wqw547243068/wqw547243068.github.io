...
categories: 编程语言
tags: git svn gitlab github cli
...
【2023-1-27】[GitHub CLI 命令行工具（gh)](https://zhuanlan.zhihu.com/p/601200139)

Github CLI 是Go 编写的命令行工具，让开发者通过命令行于 GitHub 进行无缝协同工作，进行 pull requests、issues 等其他功能。
...
【2026-7-14】clone一个非空项目到本地，一切正常，最后是空文件夹

参考：[克隆仓库后惊现“空文件夹”？](https://blog.csdn.net/qq_41797451/article/details/157775205)

可能原因
- 不在主分支上（最常见): 可能不在默认分支，特别是当仓库使用 main 而不是传统 master
- 仓库确实只有空README文件
- 子模块（Submodules）未初始化
- 启用了稀疏检出（Sparse Checkout）
- Git版本或配置问题

原因：主分支不是master导致
解法：git clone -b main repo 
