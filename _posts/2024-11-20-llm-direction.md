...
categories: 大模型
tags: LLM 大模型 AGI 世界模型 系统 快思考 慢思考 灾难 遗忘 幻觉 推理  可解释 大脑 类脑 json 缩放定律 鹦鹉 意识 o1 ttt ssm mamba 脉冲 自学习 符号主义 不确定 稳定 openai 测试时计算 模仿学习 强化学习 联想记忆 数学
excerpt: 大模型会往哪个方向发展？
mathjax: true
...
LLM 幻觉根本原因: <span style='color:red'>训练和评估流程**奖励猜测**行为而非承认**不确定性**</span>。
...
大语言模型的结构化推理能力非常有限。
- SudokuBench 上面各个大模型的表现，最顶尖的GPT5模型对9x9的Sukoku，解决率也只有百分之二十左右，很多大模型都是0。

【2026-3-11】MIT博士`Christos Tzamos`团队突破——把 WebAssembly 解释器直接"编译"进Transformer权重里。
-团队 Percepta
- 介绍 [Can LLMs Be Computers?](https://www.percepta.ai/blog/can-llms-be-computers)，[知乎解答](https://zhuanlan.zhihu.com/p/2017234636586123860)

数学计算不用靠猜

现在能100%解世界最难数独、精准比大小，但CPU跑得还不够快，金融建模这类高精度场景还在验证阶段。真正的革命在于：AI终于不用靠外挂工具做计算了。

Percepta 思路：能不能让1.0的代码放到2.0的模型里面去执行？他们认为可以，只要在LLM里面搭建一个计算机。
...
【2026-3-18】[奥特曼宣判Transformer死刑，Mamba‑3正在接管下一代AI架构](https://mp.weixin.qq.com/s/xGhc1US97SHpl6wdaqW6pg)

2023 年底，Albert Gu 和 Tri Dao 提出全新架构：Mamba，彻底绕开了注意力机制，改用`状态空间模型`（SSM）来处理序列。
- Transformer 读一句话要让每个词和其他所有词“对视”一遍，而 Mamba 只维护一个**固定大小的记忆状态**，**线性**时间就能完成，推理吞吐量直接快上五倍。
...
【2026-4-24】[Attention根本不是必需，格拉斯曼流才是序列建模的未来](https://mp.weixin.qq.com/s/3Wy0QCZhinJp3nmdzVFiug)

Transformer 致命软肋
1. 自注意力本质：高维张量“升维爆炸”
  - 张量升维（Tensor Lifting）三大原罪：维度爆炸、不可解释、数学不可追踪
  - 大模型不可解释，不只是因为大，而是核心升维操作本身就数学不可追踪。
2. 真正需要的不是Attention，而是“表示的几何演化”
...
- 【2025-12-22】zhang chong 发表 arXiv 论文《[Attention Is Not What You Need：Grassmann Flows as an Attention-Free Alternative for Sequence Modeling](https://arxiv.org/abs/2512.19428)》

没有去优化稀疏注意力、线性注意力，而是直接抛出结论： 
> 序列建模根本不需要显式 Attention，用`格拉斯曼流`（Grassmann Flows）就能做到接近甚至局部超越Transformer，还更可解释、复杂度更低。
...
【2026-1-9】[Transformer已死？DeepMind正在押注另一条AGI路线](https://mp.weixin.qq.com/s/mLyAM3hn2KTW4DeQWOSQXw)：联想记忆 
...
如果Transformer开启了Scaling时代，那么`嵌套学习`正在开启真正的AGI时代
...
由此，谷歌的团队探讨了不同层级之间的知识传递方式，提出了`嵌套学习`。
...
【2025-12-17】[Nature重磅！深度学习+符号学习，竟是AGI的唯一通关密码？](https://mp.weixin.qq.com/s/1AOUbILk958DSXdIHNLuTQ)
- 地址 [nature](https://www.nature.com/articles/d41586-025-03856-1)
...
许多人认为“神经网络 + 符号机制”是向AI注入逻辑推理的**最佳**甚至**唯一**方法，全球科技巨头IBM就押注神经符号技术，但也有人怀疑
- 现代 AI之父之一`Yann LeCun`认为:神经符号方法与深度学习机制“不兼容”
- Richard Sutton坚持“苦涩的教训”仍适用于今天的AI。
