...
categories: 计算机视觉
tags: 深度学习 计算机视觉 GAN OCR ocr 豆包 多模态 图片编辑 pdf deepseek diffusion
...
【2024-12-18】 开源 OCR工具 [Surya](https://www.datalab.to/surya)，性能炸裂，更新了 **表格识别**功能，不仅能识别表格的行、列、单元格，还能识别旋转的表格和复杂布局，而且支持90多种语言，简直无敌。
...
目前GitHub上收藏人数超过1万（10K），不仅**免费开源**，还能应用于商业场景。
- [文章介绍](https://mp.weixin.qq.com/s/5YKsTrYIjgeqxrAeLK4jUg)
- GitHub 地址：[surya](https://github.com/VikParuchuri/surya)
...
可选参数
- `--langs` 指定语种, 支持90+种, 多语种用逗号分隔，语种[代号](https://en.wikipedia.org/wiki/List_of_ISO_639_language_codes), 详见 代码 `surya/languages.py`
- `--lang_file` 多文件时,指定各个文件语种, json 格式, 如 `{"file1.pdf": ["en", "hi"], "file2.pdf": ["en"]}`
- `--images` 保存图片和检测出来的文本
- `--results_dir` 指定结果保存目录
- `--max` 指定最大处理页数
- `--start_page` 指定开始处理页码

输出 results.json 文件, 字段格式
- `text_lines` 检测出来的文本、边界框
  - `text` 文本
  - `confidence` 置信度
  - `polygon` 坐标点数组（从左上角开始顺时针）
  - `bbox` 方框数组 (左上角坐标),(右下角坐标)
- `languages` 语种
- `page` 页码
- `image_bbox` 指定图片识别区域
...
- OmniAI 91.7%、`Gemini 2.0 Flash` 86.1% 和 Azure 85.1%。
- OmniAI 每 1000 页 10 美元，`Gemini 2.0 Flash` 1.12 美元和 Azure 10 美元

OmniAI 传统付费方案

- 图见原帖 [x](https://x.com/_philschmid/status/1893926592977477751)

OCR 实时榜单
- AlphAxiv: [State of the Art/OCR](https://www.alphaxiv.org/state-of-the-art/tasks/ocr)
...
[Ollama-OCR](https://github.com/bytefer/ollama-ocr) 特点
...
[MinerU](mineru.org.cn) 上海AI实验室出品，诞生于 InterLM 预训练时期
- 【2024-9-27】[MinerU: An Open-Source Solution for Precise Document Content Extraction](https://arxiv.org/pdf/2409.18839)
- 客户端[安装](https://mineru.org.cn/client)
- [MinerU 代码](https://github.com/opendatalab/MinerU)
- 在线体验: [官方](https://mineru.org.cn/OpenSourceTools/Extractor/PDF), [MinerU](https://www.modelscope.cn/studios/OpenDataLab/MinerU) 选项多，带示例

A high-quality tool for convert PDF to Markdown and JSON.一站式开源高质量数据提取工具，将PDF转换成Markdown和JSON格式。
- ![](https://webpub.shlab.tech/dps/opendatalab-web/mineru-website-csr-prod.1860/assets/home-section-1-intro-BkTSN5fL.webp)

#### 原理

当前文档内容抽取的主流方案
- （1）`基于OCR的文本抽取`(OCR-based Text Extraction): 使用 OCR 模型从文档中抽取文本内容，容易受图/表/公式等元素影响
- （2）`基于工具包的文本解析`(Library-based Text Parsing): 使用 Python 工具包（如PyMuPDF）直接读取文档内容，快速有效，但同样受图/表/公式等元素影响
- （3）`多模块文档解析`(Multi-Module Document Parsing)：分步骤进行
  - 布局检测模块识别版面信息，图/表/标题等要素
  - 各功能模块针对性识别，如：文本标题、公式、表格等
  - 问题：理论上识别效果好，但实际表现不佳，书籍、论文、研究报告、报纸
- （4）`端到端多模态文档解析`(End-to-End MLLM Document Parsing)：
  - 案例: Donut, Nougat, Kosmos-2.5, Vary, Vary-toy, mPLUG-DocOwl-1.5, mPLUGDocOwl2, Fox, GOT
  - 问题: 多样性数据上表现不足，推理成本高

一站式文档解析工具 [MinerU](mineru.org.cn) 基于（3）多模块文档解析策略，融合  PDF-Extract-Kit 
...
![](https://pic4.zhimg.com/100/v2-f9af1752e0306543b552cc3865833687_r.jpg)
...
MinerU 模型目录: [PDF-Extract-Kit-1.0](https://modelscope.cn/models/0penDataLab/PDF-Extract-Kit-1.0/files)
...
- LayoutLMV3 开源微调
  ...
  - [LayoutLMv3： Pre-training for Document AI with Unified Text and Image Masking 解读](https://zhuanlan.zhihu.com/p/605168424)
- YOLOV10_ft 开源微调 [YOLO V10](https://github.com/THU-MIG/yolov10)
  - YOLOv10: Real-Time End-to-End Object Detection - NeurIPS 2024
- doclayout_yolo_ft 开源微调
  - 【2024-10-16】上海AI Lab 论文 [DocLayout-YOLO: Enhancing Document Layout Analysis through Diverse Synthetic Data and Global-to-Local Adaptive Perception](https://arxiv.org/pdf/2410.12628)
  - 基于 YOLO-v10，通过多样性文档预训练及适配文档检测的模型结构优化，可针对多样性文档进行实时鲁棒的检测
  - 数据集 [DocSynth300K](https://opendatalab.com/zzy8782180/DocSynth300K), [示例](https://opendatalab.com/zzy8782180/DocSynth300K/tree/main/raw/DocSynth300K), 模型 [文件](https://huggingface.co/spaces/opendatalab/DocLayout-YOLO)
  - 训练方法 [数据准备 & 预训练](https://github.com/opendatalab/DocLayout-YOLO/blob/main/README-zh_CN.md#%E6%95%B0%E6%8D%AE%E5%87%86%E5%A4%87--%E9%A2%84%E8%AE%AD%E7%BB%83)
  - DocLayout-YOLO 适用于多样性文档布局检测，包括但不局限于论文、教科书、试卷、幻灯片等多种文档类型。
  - 方法[解读](https://zhuanlan.zhihu.com/p/4016022077)
...
- [UniMERNet: A Universal Network for Real-World Mathematical Expression Recognition](https://github.com/opendatalab/UniMERNet/blob/main/README-zh_CN.md)
- UniMERNet: 一个用于真实世界数学表达式识别的通用网络
- 数据集: [UniMER-Dataset](https://opendatalab.com/OpenDataLab/UniMER-Dataset) 2GB, 106万(1,061,791) LaTeX-图像对

表格识别:
- StructEqTable 开源, 采用 InternVL2-1B 基础模型
  - [StructEqTable-Deploy: A High-efficiency Open-source Toolkit for Table-to-Latex Transformation](https://github.com/Alpha-Innovator/StructEqTable-Deploy)
...
|doclayout_yolo_ft|布局检测|[官方文档](https://github.com/opendatalab/DocLayout-YOLO/blob/main/README-zh_CN.md#%E6%95%B0%E6%8D%AE%E5%87%86%E5%A4%87--%E9%A2%84%E8%AE%AD%E7%BB%83): 预训练使用8卡以上，指令数据集DocSynth300K有113G|同上||
|yolo_v8_ft|公式检测|-|||
|UniMERNet|公式识别|数据集: [UniMER-Dataset](https://opendatalab.com/OpenDataLab/UniMER-Dataset) 2GB, 106万(1,061,791) LaTeX-图像对|||
...
- 支持**纯CPU**环境运行，并支持 GPU(CUDA)/NPU(CANN)/MPS 加速
- 兼容Windows、Linux和Mac平台


![](https://webpub.shlab.tech/dps/opendatalab-web/mineru-website-csr-prod.1857/assets/home-section-1-intro-BkTSN5fL.webp)
...
使用命令
- 官方文档 [command_line.html](https://mineru.readthedocs.io/en/latest/user_guide/usage/command_line.html)
...
[api.html](https://mineru.readthedocs.io/en/latest/user_guide/usage/api.html)
- [Convert PDF](https://mineru.readthedocs.io/en/latest/user_guide/quick_start/convert_pdf.html)
- [Convert Image](https://mineru.readthedocs.io/en/latest/user_guide/quick_start/convert_image.html)
- [Convert Doc](https://mineru.readthedocs.io/en/latest/user_guide/quick_start/convert_ms_office.html)
...
- 提交 [issue](https://github.com/opendatalab/MinerU/issues/1715)

表格识别使用第三方开源项目 [RapidTable](https://github.com/RapidAI/RapidTable) 获得支持
...
然而， AGPL v3 授权具有**传染性**，直接集成会迫使项目整体开源，这在商业项目中往往难以接受。
...
三种模式以满足不同场景的需求：
- **快速**模式（simple） 基于 pdfminer（`markitdown` 内置的 pdf 解析器），专注于高效文本提取，适合对文本要求较低的场景。 
- **高级**模式（advanced） 结合 MinerU 的深度解析，不仅能精准提取文本，还能识别并转换复杂表格和图像，还将图像自动转换为 Markdown 中的网络引用形式，例如： ![](http://localhost:20926/images/xxxx.jpg)
- **云端**模式（cloud） 研发中，未来将为用户提供更多云端解析能力。
...
- ![](https://picx.zhimg.com/v2-6dcdaa44cf37bebb61010d7611941e79_1440w.jpg)
- 图像转换: Markify 将图像上传至服务器并嵌入 Markdown 中，使得图文混排效果更为直观美观
- 整体版面: Markify 对文本内容提取精准，整体排版清晰易读
- ![](https://picx.zhimg.com/v2-93f8885fced0d9056691a0cf07de6543_1440w.jpg)

[Mountain](https://zhuanlan.zhihu.com/p/29068922417)
...
- 【2025-2-25】[olmOCR：PDF文档转换为文本，支持表格、公式和手写内容的识别](https://www.aisharenet.com/olmocr/)
- 【2025-2-28】[olmOCR端到端解析万亿级PDF](https://mp.weixin.qq.com/s/ByNAszVxZreMZAfUVTtZmw)

[olmOCR](https://olmocr.allenai.org/) 是 Allen Institute for Artificial Intelligence (AI2) 的 AllenNLP 团队开发的一款**开源**工具
- 将 **PDF 文件**转换为**线性化**文本，特别适合用于大规模语言模型 (LLM) 的数据集准备与训练。
...
- 在线体验 [olmocr](https://olmocr.allenai.org/)
- 数据集 [olmOCR-mix-0225](https://huggingface.co/datasets/allenai/olmOCR-mix-0225)
- 模型下载 [olmOCR-7B-0225-preview](https://huggingface.co/allenai/olmOCR-7B-0225-preview)
- [olmocr](https://huggingface.co/collections/allenai/olmocr-67af8630b0062a25bf1b54a1)
...
- ![](https://private-user-images.githubusercontent.com/178819005/407884445-128922d1-63e6-4d34-84f2-d7901237da1f.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIyODc0MTYsIm5iZiI6MTc0MjI4NzExNiwicGF0aCI6Ii8xNzg4MTkwMDUvNDA3ODg0NDQ1LTEyODkyMmQxLTYzZTYtNGQzNC04NGYyLWQ3OTAxMjM3ZGExZi5wbmc_.gJ4aMWx_BOqtTrOC1DAKbLNKzf4hiom0cTU3D1VzKVo)
...
- Issue: [Any solution to speed it up ?](https://github.com/allenai/olmocr/issues/128)
- SGLang 加速 [sglang or vllm api interface](https://github.com/allenai/olmocr/issues/51)
...
【2025-3-6】[复杂表格超96%：mistral-ocr登顶最强OCR](https://mp.weixin.qq.com/s/Fj9bC77Q4JdwzxgQVv1eQQ)
- [mistral-ocr](https://mistral.ai/news/mistral-ocr)
- [文档](https://docs.mistral.ai/capabilities/document/)
...
【2025-4-12】 [忘掉OCR吧，LAYRA用“看”的方式理解文档-最新视觉RAG项目 LAYRA 开源了](https://zhuanlan.zhihu.com/p/1894142828529051355)

RAG领域老问题：OCR导致的结构损失和语义错乱

传统文档问答系统将PDF、扫描件等文档“**硬转**成文本”，不仅破坏了原始**排版**信息，还对图表、流程图等非文本内容束手无策。
...
LAYRA 开源前后端分离、UI简约的企业级的**视觉优先**的RAG（Retrieval-Augmented Generation）系统。
- 打破传统 OCR + 文本抽取 方式，直接以文档图片为输入，通过colpali系列最新的 colqwen2.5-v0.2 模型进行向量化理解，实现保留排版结构、图表信息的智能问答体验。
- github [layra](https://github.com/liweiphys/layra/blob/main/README_zh.md)
...
![](https://pic2.zhimg.com/v2-0fef972220202d9d2a5c0a2a191496b1_1440w.jpg)
...
大多数公开的图像转文本模型主要侧重于从图像中提取**纯文本**，然而无法区分常规内容和**水印**、**签名**或**页码**等元素。
...
【2025-06-12】Nanonets 团队开源了 [Nanonets-OCR-s](https://nanonets.com/research/nanonets-ocr-s/)，该模型基于`Qwen2.5-VL-3B`微调，**9G显存**就能跑。
...
参考链接 
- [](https://llmtrend.com/nanonets-ocr-s%ef%bc%9a%e4%b8%ballm%e6%89%93%e9%80%a0%e7%9a%84%e5%bc%ba%e5%a4%a7%e5%9b%be%e6%96%87%e8%bd%acmarkdown%e5%bc%95%e6%93%8e/)
...
- GitHub: [Dolphin](https://github.com/bytedance/Dolphin)
- 工具链接：[Dolphin](http://115.190.42.15:8888/dolphin/)

Dolphin 模型以仅**322M**的轻量级参数，在多个基准测试中全面超越了`GPT-4V`、`Claude`等大模型
- 第一阶段：页面级布局分析，生成按阅读顺序排列的结构化布局元素序列
- 第二阶段：基于异构锚点提示的并行元素内容解析
- ![](https://picx.zhimg.com/v2-f9dde588efd31223f15fe96de6e7d95d_1440w.jpg)
...
参考
- [Dolphin：文档图像解析领域的两阶段突破](https://llmtrend.com/agentic%e6%96%87%e6%a1%a3%e6%8f%90%e5%8f%96/)

### dots.ocr

【2025-8-2】dots.ocr 是小红书 hi lab 开源的多语言文档布局解析模型。 1.7B参数超强OCR大模型dots.ocr，超越GPT-4o和olmOCR
- GitHub仓库：[dots.ocr](https://github.com/rednote-hilab/dots.ocr)
- HuggingFace模型库：[dots.ocr](https://huggingface.co/rednote-hilab/dots.ocr)
- 在线体验[Demo](https://dotsocr.xiaohongshu.com/)

[解读](https://blog.csdn.net/xyzroundo/article/details/150502733)
...
<img width="1200" height="675" alt="image" src="https://github.com/user-attachments/assets/ec2b14a8-6e7a-4899-a271-9fb39bbaa002" />
...
| 视觉编码器类型       | 代表模型       | 优势                                  | 缺点                                  |
|----------------------|----------------|------------------------|---------------------------|
| 双塔架构（Dual-Tower）| Vary     | 1. 视觉词汇参数可控，便于调整模型复杂度；<br>2. 激活内存消耗可控，降低基础资源压力。 | 1. 需对图像进行双重预处理，增加部署复杂度；<br>2. 训练阶段编码器流水线并行难度高，影响训练效率。 |
| 基于分块（Tile-Based） | InternVL2.0| 1. 通过图像分块实现并行计算，高分辨率场景下激活内存消耗低；<br>2. 支持处理极高分辨率图像。 | 1. 原生编码器分辨率低（通常低于512×512）；<br>2. 大尺寸图像会被过度分割，生成大量视觉 tokens，增加计算负担。 |
| 自适应分辨率编码（Adaptive Resolution） | Qwen2-VL| 1. 采用 NaViT范式，通过基于 patch 的分割直接处理全图，无需分块并行；<br>2. 可灵活适配不同分辨率图像，兼容性强。 | 1. 处理大尺寸图像时激活内存消耗极高，易导致 GPU 内存溢出；<br>2. 训练阶段序列打包需极长序列长度，推理阶段的预填充和生成速度会因长视觉 tokens 变慢。 |

<img width="1082" height="247" alt="image" src="https://github.com/user-attachments/assets/6db6697d-43d5-438a-bb31-a3ce2a1bcaa0" />
...
DeepSeek-OCR 设计思路：通过**多分辨率**的视觉编码机制，实现极高的信息压缩效率。
...
<img width="1080" height="529" alt="image" src="https://github.com/user-attachments/assets/2751abc1-5cab-4bbc-ad52-e8419de980f9" />

#### 架构

DeepSeek-OCR架构图

<img width="1074" height="305" alt="image" src="https://github.com/user-attachments/assets/ce10c2a5-6c51-4618-bfd6-f8cd7f9ab28b" />


该模型通过创新的**光学二维映射压缩**技术，在长文本识别场景中实现**97%**的识别精度，为OCR领域树立新的技术标杆。

该模型采用**双模块**架构，由 DeepEncoder 视觉编码器与 DeepSeek3B-MoE-A570M 混合专家解码器构成。
- 其中，DeepEncoder 处理高分辨率图像时自动维持低激活状态，通过动态压缩生成最优数量的**视觉特征令牌**（visual tokens），较传统方法减少**60%**的计算冗余。
...
- **模块3：视觉知识（全局注意力主导）**
  采用CLIP-large（300M参数），作用：通过全局注意力整合压缩后的token，提炼图像全局语义（如文档布局、文本逻辑），为解码提供结构化视觉知识。
...
<img width="800" height="100%" alt="image" src="https://github.com/user-attachments/assets/42fdcf34-ec5b-443e-b207-c527637f1b40" />


这项突破性成果源于对`光学二维映射压缩`技术的深度探索。通过将**图像特征**转化为**离散令牌**序列，模型成功解决了长文本场景中视觉信息与语义输出的匹配难题。
...
【2025-10-22】[再谈DeepSeek-OCR的信息压缩论！附DeepSeek-OCR与PaddleOCR实测对比](https://mp.weixin.qq.com/s/ksokCduU_xfnl6dR7P67_g)

测试链接：
- [PaddleOCR-VL_Online_Demo](https://huggingface.co/spaces/PaddlePaddle/PaddleOCR-VL_Online_Demo)
- [DeepSeek-OCR-Demo](https://huggingface.co/spaces/axiilay/DeepSeek-OCR-Demo)
...
DeepSeek-OCR [测试脚本](https://mp.weixin.qq.com/s/GtO0oT0O5WJC0I_DmGAIJQ)
...
- 更根本的问题 —— 对大语言模型（LLM）来说，**像素**是否比**文本**更好的输入形式？文本 token 会不会其实是一种“浪费而糟糕”的输入方式？
...
输入可以天然地使用**双向注意力**（bidirectional attention），而不是像语言模型那样必须**自回归**（autoregressive）地逐步处理，这在结构表达上更强大。
...
- 那篇由字节跳动实习生`Tian Keyu`领衔的工作，采用“由粗到细”的多尺度预测方式——先低分辨率生成，再逐步提升清晰度。
- 同时，豆包团队也在论文中展示了相同的数据压缩路径：512×512 图像可编码为 64 个 token，256×256 甚至能压到 32 个 token。
...
- 【2025-10-22】EMNLP 2025 Findings，[Text or Pixels? It Takes Half: On the Token Efficiency of Visual Text Inputs in Multimodal LLMs](https://arxiv.org/abs/2510.18279)


### 【2026-1-27】DeepSeek-OCR 2

【2026-1-27】[DeepSeek-OCR 2发布：识别性能提升3.73%，让AI“读懂”复杂文档](https://news.qq.com/rain/a/20260127A03ZD700)
...
- [img](https://img-s.msn.cn/tenant/amp/entityid/AA1V3p0N.img?w=768&h=423&m=6)

![](https://img-s.msn.cn/tenant/amp/entityid/AA1V3p0N.img)
...
【2026-1-21】[LightOnOCR-2-1B：1B小模型吊打9倍大模型,速度快3倍还开源了](https://mp.weixin.qq.com/s/5ZLHpf623cCTBOt1cY9AMw)

LightOn 公司推出 LightOnOCR-1B，介绍: [lightonocr](https://huggingface.co/blog/lightonai/lightonocr)
- 【2026-1-20】论文 [LightOnOCR: A 1B End-to-End Multilingual Vision-Language Model for State-of-the-Art OCR](https://arxiv.org/pdf/2601.14251)
- 模型地址：[LightOnOCR-2-1B](https://huggingface.co/lightonai/LightOnOCR-2-1B)
- 数据集链接：[LightOnOCR-bbox-mix-0126](https://huggingface.co/datasets/lightonai/LightOnOCR-bbox-mix-0126)
- 在线演示: [LightOnOCR-2-1B-Demo](https://huggingface.co/spaces/lightonai/LightOnOCR-2-1B-Demo)
...
【2026-1-5】[多模态文档解析模型进展：UNIREC-0.1B](https://zhuanlan.zhihu.com/p/1991642447818794189)

起因
- 2025年已发布的模型: DeepSeekOCR，QwenVL，HunyuanOCR，PaddleVL-OCR，Monkey-OCR，MinerU，Dolphin等作品。虽然都取得不错效果，但在一些业务场景中想落地还是有一段距离，比如说**性能**。

【2025-12-24】复旦+字节提出 UNIREC-0.1B ，公式+文本识别
- 论文[UniRec-0.1B: Unified Text and Formula Recognition with 0.1B Parameters](https://arxiv.org/pdf/2512.21095)
- 代码 [Codebase and Dataset-OpenOCR](https://github.com/Topdu/OpenOCR)
...
- ![](https://pic2.zhimg.com/v2-b172489c317a4fad398e9c95566122e9_1440w.jpg)
...
- 论文 MinerU-Diffusion: Rethinking Document OCR as Inverse Rendering via Diffusion Decoding
 OCR 真的应该“逐字生成”吗？过去十年里，文档OCR几乎都默认一个范式：👉 从左到右，一个 token 一个 token 生成
但文档不是语言，它是结构
OCR本质不是“写句子”，而是：👉 把二维文档结构，还原成一维符号序列

💡 MinerU-Diffusion：用“扩散”，而不是“生成”
把OCR从“序列生成”改写为：文本结构的逆向渲染（Inverse Rendering）
- 🔧 核心方法：离散扩散解码（Diffusion Decoding）  
- 📊 实验结果：在保证精度的前提下：
- ⚡ 最高可达 5.1× 推理加速
- ⚡ 常规配置下 2–3× 性能提升
在挑战性场景（Semantic Shuffle）中：
- ❌ 自回归模型：性能显著下降（依赖语言先验）
- ✅ 扩散模型：性能基本稳定（依赖视觉信号）   
