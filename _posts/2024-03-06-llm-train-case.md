...
【2026-2-6】[资料](https://zhuanlan.zhihu.com/p/2003041773023428952)

Llama 3 后训练数据获取采用策略：“多源融合与迭代优化”
- 第一，多源数据融合。合成数据生成、人工标注数据、严格质量控制三大体系。
- 第二，数据迭代优化。通过6轮迭代完成，每轮整合拒绝采样结果、领域专用合成数据及人工反馈，形成动态进化的数据集。

![](https://picx.zhimg.com/v2-d2c69b20dc531b0b821625ca834a0643_r.jpg)

多源数据融合：
- 第一，**合成数据**生成。包括拒绝采样、长文本合成等。
  - 拒绝采样（Rejection Sampling）：从当前最优模型或领域专家模型（如代码专家、多语言专家）采样10-30次回复，由奖励模型（RM）选出最佳样本作为SFT数据。
  - 长文本合成：针对128K窗口需求，通过短窗口模型生成多轮问答、长文档摘要和代码仓库级推理数据。SFT阶段混入0.1%合成长文本数据（16K-128K），避免预训练获得的长窗口能力退化。
- 第二，**人工标注**与专家数据。超1000万条人工注释示例，覆盖数学推理、多语言对话等复杂场景。
- 第三，严格**质量控制**。
  - 规则清洗与去重：过滤含过量emoji、感叹号或过度道歉（如频繁出现“I'm sorry”）的低质量样本，通过启发式规则移除格式错误的代码片段。语义去重使用RoBERTa对样本聚类，按“质量×难度”排序后保留余弦相似度低于阈值的样本。
  - 质量评分：结合RM打分（保留前25%高分样本）和Llama 3自评估（分准确性、指令遵循性等维度），双重筛选提升数据可靠性。
  - 领域专用过滤：代码数据需通过编译测试或“模型即裁判”二分类（正确性+风格），复杂任务样本会人工修正至符合标准。
- 第四，**数据配比**：SFT数据包含50%通用知识、25%数学推理、17%代码及8%多语言数据，偏好数据（Preference data）则积累式增长，每轮新增标注并保留历史高质量样本。

每轮后训练包含“RM训练→拒绝采样→SFT→DPO”流程，高质量数据（如优质代码示例）会重复训练3-4次，普通数据则降采样使用。

后训练数据生成包括: 质量大于数量、人机协同、数据生成、数据增强、数据质量评估和清洗共5部分。

（1）质量大于数量：

相比于数量，更应该专注于数据质量的打磨。有可能对于某个task，几百条高质量数据优于几万条一般质量数据。

数据质量中，数据的格式很重要，不同的大模型通常有不同的数据格式，比如是否包含thinking？比如一些特定的标识符要注意。

（2）人机协同：

后训练数据生成别想着完全靠人工获得（成本太高），也别想着完全靠大模型生成（质量上限低），二者都不可行，而是应该如何结合人和模型的优缺点协作生成后训练数据。
- 人的优点：高质量**种子**问题和回答的生成、人对数据的质量评估、关键错误清洗。
- 模型优点：通过在prompt优化大批量生成query或answer、多个模型的协作投票可以做数据的质量评估。

（3）数据生成：

数据生成让大模型扮演数据query和answer的生成者，核心是通过策略上的优化提升模型输出数据的质量，那么有哪些可以改进输出质量的方案呢？
- 第一，在数据生成过程中，要使用效果最好的通用大模型或垂域大模型，站在巨人的肩膀上。同时可以尝试不同大模型来增加回答的多样性，提升数据泛化性。
- 第二，拒绝采样1问N答，从中选优，LLaMA3中N可以到30。
- 第三，提升prompt质量，可以尝试引入专业人士角色扮演和情景代入、引入思考thinking过程、引入情感嘞索、Prompt格式和规定answer格式。
- 第四，可以结合具体任务来改变模型生成的置信度。确定性任务要保证高可控性，多样性任务要保证高多样性。

（4）数据增强：

数据增强的对象是question，让微调数据和用户提问question在提问风格、内容上尽可能接近。提升泛化性，降低应用风险。

数据增强的方式包括：同义替换、风格迁移、反向构造。例如反向构造：这是一个回答，请你写一个用户可能提问的问题。

（5）数据质量评估和清洗：

数据质量判断和清洗对整个过程至关重要，一般两步走，第一步走通过模型进行初筛，后通过人工进行复查。

生成大模型和质量评估大模型尽可能不同，由此保证数据的客观性。

数据质量判断和清洗的四种方式包括：大模型、多个大模型协同投票、训练一个reward model、通过人工的方法。

大模型投票筛选数据的维度可以考虑从准确性-有无错误、query+think+answer的相关性、逻辑性-缺乏推理的猜想、帮助性这4个角度考虑。注意，大模型评价存在reward hacking现象，即大模型喜欢又臭又长的回答


#### CPT 数据处理

CPT使用大规模领域文本，数据质量无法保证，需要预处理、清洗及去重。

基本预处理
- 繁简转换：将繁体字转换为简体字。
- 移除多余空白符：删除数据中多余的空白符、占位符、换行符等(这里在 ds-v2 的论文中有提及保留单个的结构占位符)。
- 移除个人信息：删除数据中的电子邮件、电话号码、ip 地址等
- 移除链接：删除数据中的链接。
- 大小写转换：转换为小写。
- 长度过滤:
  - 移除过短的句子。
...
预训练语料去重的必要性:
- 避免数据重复导致过拟合，减少冗余并保持语料库的多样性。
- 提升预训练有效数据利用率
- 避免模型产生幻觉与信息偏置
- 去重有助于使语言建模分布更均匀，减少幻觉（hallucination）倾向。

各个主流模型的技术报告中，不止一次提及了去重的重要性
- 主流的去重方式: MinHash+嵌入向量化+FAISS 检索相似文本去重。简单高效且可以根据相似度在去重粒度上进行控制。

工具
- EXACTSUBSTR(精确子字符串去重)，Google 提出的高效算法，在 llama 系列的模型
- MinHash+LSH：MinHash 可以高效估计两个集合的 Jaccard 相似度，并且可用于 LSH（局部敏感哈希）实现快速近似去重

详见：[知乎](https://zhuanlan.zhihu.com/p/1931461283091292419)
...
阿里 [Data-Juicer](https://datajuicer.github.io/data-juicer/zh_CN/main/index_ZH.html)

Data-Juicer (DJ) 将原始数据转化为 AI 就绪的智能。将数据处理视为可组合的基础设施——提供模块化构建块，在整个 AI 生命周期中清洗、合成和分析数据，释放每份数据的潜在价值。
- 【2023-9-5】[论文《Data-Juicer: A One-Stop Data Processing System for Large Language Models 》](https://arxiv.org/pdf/2309.02033)
- 解读 [Data-Juicer：大语言模型的数据处理利器](https://zhuanlan.zhihu.com/p/688774123)

无论您是在去重网络规模的预训练语料库、整理智能体交互轨迹，还是准备特定领域的 RAG 索引，DJ 都可以从您的笔记本电脑无缝扩展到数千节点的集群——无需编写胶水代码。

Data-Juicer 采用细粒度的数据处理流程抽象，内置了50多个可自由组合和扩展的运算符。通过集成可视化和自动评估功能，形成了数据处理和LLM训练的闭环。

此外，Data-Juicer还与LLM训练、评估和分布式计算生态系统无缝集成，为LLM的数据中心研究提供了强大支持。
- ![](https://pic3.zhimg.com/v2-0257aa0b3e51ea6173584d33139aee4e_1440w.jpg)

Data-Juicer 采用 Huggingface-datasets 库作为统一的数据表示，支持多种文本输入格式，如txt、JSON、parquet等，并将它们统一成包含“text”、“meta”和“stats”三部分的结构化格式。
- 其中，“text”部分存储原始文本数据，“meta”部分存储元信息（如日期和版本），“stats”部分存储可以由Data-Juicer的其他运算符和工具生成和消费的统计信息。
- 这种接口在文本样本或数据集级别工作，独立于底层的内存或磁盘数据布局，为运算符开发者消除了对异构数据格式的潜在担忧。

Data-Juicer 运算符池包括格式化器（Formatters）、映射器（Mappers）、过滤器（Filters）和去重器（Deduplicators）。格式化器负责统一数据格式；映射器用于原地文本编辑；过滤器用于条件过滤文本；去重器用于去除重复样本。这些运算符为LLM数据处理提供了全面的功能。

Data-Juicer 运算符具有高度的可组合性。每个运算符可以处理不同的文本字段，并具有可配置的参数。用户可以轻松组合使用不同的运算符来处理各种数据类型，每个运算符可以处理不同的文本字段，具有可配置的参数，为用户提供了强大的数据处理能力。

Data-Juicer 提供了可视化、自动评估等功能，形成了数据处理和LLM训练的闭环。还引入了超参数优化，加速了数据处理的迭代。此外，Data-Juicer与LLM训练和评估生态系统无缝集成，支持自动评估。
- ![](https://pica.zhimg.com/v2-7b1363b26e9262fb0d65b92673d33c3a_1440w.jpg)
