...
categories: 大模型
tags: llm 对话系统 全双工 tts 语音 贾扬清 克隆 李沐 阿里 阶跃星辰 数字人 websocket webrtc 客服
excerpt: 端到端语音交互方案汇总
mathjax: true
permalink: /end_vioce
---
...
【2026-8-24】帖子[解读](https://www.xiaohongshu.com/explore/6a8a96aa00000000330200de?xsec_token=CBlnnuvUWxJgS1QZzJaACyaX82xrYZWVoJTWGxabdo7I0=&xsec_source=app_share)

普通流式语音和真正全双工的分界
- 打断语音助手时，“检测到声音就闭嘴”，还是仍然理解了在说什么，并知道该暂停、继续、附和，还是重新回答？
	
纠正时间点：截至 2026-08-22，没有找到 8 月 1 日发布的更新同主题 survey。
- 最新直接聚焦 Full-Duplex Speech Language Models 的仍是 6 月 17 日 arXiv 2606.19453；
  - 浙大、阿里千问、腾讯混元、字节综述论文 [A Survey of Full-Duplex Spoken Dialogue Systems: Architectural Hierarchy, Interaction Ontology, and Decision State Machine](https://arxiv.org/abs/2606.19453)
- 8 月 18 日的 arXiv 2608.17605 范围更广，并非替代版。

进化线路：不是模型越来越大，而是控制权逐步内生：
- 先学双人声学节奏
- 再补语义和打断
- 随后把双路音频、文本内心独白与控制状态统一到流式生成

|时间|模型名称|核心特点|
|---|---|---|
|2022.03|dGSLM|无文本双声道，先学会停顿与重叠|
|2024.08|LSLM|边说边听，用 IRQ token 接住打断|
|2024.10|Moshi|双音频+文本内心独白，实时对话成形|
|2024.10|OmniFlatten|三路流压平为普通自回归序列|
|2025.01|MinMo|分阶段对齐，外接轻量双工预测器|
|2026.06|BayLing‑Duplex|四个状态token 直接承担话语权控制|

18 张卡把主线拆成三段：
- 第一段是架构演化。
  - dGSLM 从双声道原始音频学停顿与重叠；
  - LSLM 让模型说话时继续监听；
  - Moshi 用双音频流和“文本内心独白”同时保语义与实时性；
  - OmniFlatten 把三路并发压成普通自回归序列；
  - MinMo 把话权交给轻量预测器；
  - BayLing-Duplex 再把控制收回四个状态 token。
- 第二段是训练。
  - DuplexChat 把真实双声道数据扩到 41.5 万小时；
  - 两篇 DuplexGen 分别用人类偏好校准场景化话权、以及把内容/时间/声学彻底解耦；
  - Lychee-FD 发现深层语义与声学梯度会互相冲突；
  - DuplexPO 则只在开口、附和、抢话附近的关键时间窗做强化学习，尽量不伤知识与推理。
- 第三段是系统工程。
  - Metronome 用滑动窗口与准入控制解决长会话 KV 缓存耗尽；
  - PACE 则处理播放—上下文错位：模型已经生成到第五条，用户可能只听到第三条。它让客户端回报播放进度，在打断时重建上下文。
	
判断：
- 双工语音研究已经从“模型能不能边听边说”，进入“数据、训练、推理与播放状态能否共同对齐”的阶段。
- 它不只是更快的 ASR＋LLM＋TTS，而是把什么时候听、什么时候说、什么时候停，也变成模型持续预测的一部分。
	
但也别被“全双工”三个字骗了。多数公开系统仍主要解决抢话、附和与快速轮换；持续并发说话、多方开放麦、误触发安全、跨语言轮次规范和统一成本评测，仍是没有真正攻下的深水区。
...
- [小红书视频](http://xhslink.com/o/1PIePOm16nY)
...
2026-4-10，复旦NLP实验室推出 [MOSS-TTS-Nano](https://github.com/OpenMOSS/MOSS-TTS-Nano)，0.1b 参数，模型文件压缩后不到300m，纯 CPU 运行，不需要显卡！
- 在线 demo [MOSS-TTS-Nano-Demo](https://openmoss.github.io/MOSS-TTS-Nano-Demo/)
...
![](https://openmoss.github.io/MOSS-TTS-Nano-Demo/assets/moss-tts-nano.png)
...
星语智能基于qwen2和fishspeech研发的 [Westlake-Omni](https://github.com/xinchen-ai/Westlake-Omni/tree/main)
- 【2024-10-27】 [Westlake-Omni ：一款开源的中文情感语音交互大语言模型，可实现更为智能、自然的人机交互体验](https://mp.weixin.qq.com/s/SgE-2Bb3raGbJWpplxS2fw)
...
西湖心辰团队研发的 Westlake-Omni 模型的开源，为中文情感语音交互领域带来了新的突破与机遇，西湖大学[蓝振忠](https://www.westlake.edu.cn/faculty/zhenzhong-lan.html)
- 【2024-9-10】[AI读懂“话外之音”？杭产大模型实现全国首创，背后是85后博士](http://wzs.org.cn/sjzs/news?id=18216)
- 创始人[蓝振忠](https://dll-wu.github.io/)任教于西湖大学深度学习实验室，是自然语言处理预训练语言模型“ALBERT”第一作者，曾位列《麻省理工科技评论》评选的2021年度亚太地区“35岁以下科技创新35人”。
...
### 【2026-8-5】字节 SeedRealtime

【2026-8-5】字节 Seed 推出原生音视频全双工大模型 SeedRealtime，用统一架构融合音频、视频与文本，支持“边看、边听、边说”的实时交互。
...
相比级联模型，其音视频对话节奏问题减少一半。该模型已在豆包 App 全量上线，更新至最新版本后可在对话框选择“打电话”体验。
...
【2025-12-29】浙江大学、[西湖大学](https://en.westlake.edu.cn/)和蚂蚁集团共同提出了一种名为 [OmniAgent](https://kd-tao.github.io/OmniAgent/) 的新型智能体框架，或许能彻底改变这一现状。
- 论文: [OmniAgent: Audio-Guided Active Perception Agent for Omnimodal Audio-Video Understanding](https://arxiv.org/abs/2512.23646)
...
【2026-5-12】[北大校友Lilian Weng出镜，爆出120亿估值首个交互模型！](https://mp.weixin.qq.com/s/iEqptzS_vL8jVIjkA7sbQg)

前OpenAI CTO `Mira Murati` 创办的明星公司，在0产品、0论文的情况下，就凭借全明星创始团队完成了高达20亿美元的种子轮融资，由a16z领投，英伟达、AMD、微软等巨头跟投，估值直接冲上120亿美元，刷新了硅谷早期融资纪录。
- 2025年10月, 发布首个开发者平台`Tinker`（一个让大模型微调变得像呼吸一样简单的工具）之后，这一次，他们又交出了一份让业界刮目相看的成绩单。
- 2026年5月，发布交互模型

北大校友`翁荔`首次出镜，介绍 `Thinking Machines` 又一产品——`交互模型`（Interaction Models）！
...
【2026-6-2】[推文](https://x.com/xiaohu/status/2061414052916547705) OpenAI 语音黑客之夜（[OpenAI Voice Hack Night](https://cerebralvalley.ai/e/openai-voice-hack-night/hackathon/gallery)）冠军：语音优先的**手机Agentic操作系统**, 颠覆性的“代理手机”（agentic phone）Demo, 搭载全新“代理操作系统”
- 核心理念: “UI 即系统”——手机**彻底抛弃了传统 APP**，所有界面均由**端侧模型**实时生成，复杂推理则交由云端 GPT 处理。
- 这种“即时生成”的交互方式不仅展现了大众理想中的 AI 助理形态，更对现有的 App Store 商业模式发起了挑战。
- 演示中，开发者全程通过语音指挥手机订票、管理日历、查新闻和发邮件，甚至没有回避中途因“未配置登录”导致发邮件失败的翻车插曲。


### 【2026-8-6】LiveKit Agents

【2026-8-6】[实时语音 Agent 不再只是 Demo：1.2 万 Star 开源框架把 WebRTC + LLM 打通了](https://mp.weixin.qq.com/s/8JPsmAvA44uL7ZTYJS9UyQ)

市面上大多数「语音 AI」是文字聊天机器人，套了层 TTS 外壳——用户说完话，服务端把音频转文字，喂给 LLM，LLM 吐出文字，再转成语音。每步都在排队、加延迟。

这套流程叫 `级联流水线`（Cascade Pipeline）。

生产环境里容易塌方：
- 网络抖动时 WebSocket 断开，整个对话中断
- 用户说完话后沉默 2 秒，Agent 不知道该不该接
- 用户在 Agent 说话时插话，系统完全无响应
- 工具调用（比如查数据库、发邮件）卡住整个对话流

直到 2024-2025 年，局面改变：
- 第一，WebRTC 被搬到 AI Agent 层。 以往 WebRTC 是视频会议（Zoom、Google Meet）的技术底座，但 LiveKit 证明同样是 AI 语音 Agent 的最优传输方案——低延迟、抗弱网、原生支持音视频多轨道并行。
- 第二，语义轮次检测模型成熟。 不再靠「沉默 500ms = 说完了」这种粗暴规则，而是用小型 Transformer 模型实时判断：这个人话讲完了吗？还是中间在思考措辞？
- 第三，语音 Agent 框架 1.0 化了。 LiveKit Agents 2025 年 4 月发布 1.0，把上述所有问题封装成统一抽象，让开发者不需要关心 WebRTC 信令、STUN/TURN 穿透、音频编码这些底层细节。

"Build voice agents, not WebRTC infrastructure." —— LiveKit 官方文档的第一句话，恰好概括了整个框架的设计哲学。

实时语音DEMO 不够聪明，因为中间那层「管道」：音频流怎么传？延迟压到多低？抢话了怎么办？工具怎么调用？电话怎么接？

`LiveKit Agents` 开源框架。
- 每年承载超过 25 亿通 AI 通话
- 程序以「通话参与者」身份加入实时音视频房间的框架

LiveKit Agents 的客户名单是一串很难忽视的名字：
- `OpenAI`：ChatGPT Advanced Voice 的实时音视频传输层就跑在 LiveKit Cloud 上
- `Salesforce`：客服场景的语音 Agent
- `xAI`：Grok 的语音交互
- `Skydio`：无人机远程操控的实时音视频链路
- `Retell AI`、`Character.ai`：语音 Agent 产品的底层传输

不只是「把 STT、LLM、TTS 串起来」的胶水层，而是首个把 WebRTC 实时传输、语义轮次检测、MCP 工具调用、多 Agent 协作、电话集成和全栈自托管全部打通的开源框架。

市面上已有语音 Agent 方案（Vapi、Retell AI、Pipecat、Bland AI，还有 OpenAI 自己的 Realtime API）都声称最方便。

但 LiveKit Agents 有几个能力，并列框架做不到.
1. WebRTC 不是「支持」，是「长在上面」
  - 大多数用WebSocket 传音频。 WiFi 下稳定可用，但 4G/移动网络下，WebSocket 断连重连、TCP 队头阻塞、信令延迟——每一个都是体验杀手。
  - LiveKit Agents 从第一天就基于 WebRTC，UDP 传输、自适应码率、NAT 穿透、自动重连全部内置。端到端音频延迟可以压到 200-300ms，达到运营商级 VoIP 水平。
2. 语义轮次检测，不再抢话
  - 传统方式：检测到 500ms 静音 = 用户说完了 → Agent 开始回答。
  - 问题：每个人说话都有自然停顿。500ms 一刀切，要么 Agent 频繁打断用户，要么永远在等。
  - LiveKit 采用双信号模型：
    - 声学 VAD（Voice Activity Detection）：检测是否有声波
    - 语义 EOU（End-of-Utterance）：一个约 1.35 亿参数的 Transformer 模型，实时判断用户当前说的内容是否构成一个完整语义单元
  - 两个信号都满足时，Agent 才接话。拧紧度可调：客服场景设 400ms 偏快，医疗问诊设 600ms 偏慢。
3. 电话呼入呼出：SIP/PSTN 原生
  - 2025 年 LiveKit 发布原生 SIP 中继和电话号码集成。Agent 可以直接接听电话（不用先转成 WebRTC 再桥接 SIP）、主动拨出电话、通话中可以把 Agent 替换为人工座席（同一个 Room，无缝切换）、全程录音、转录、上下文一并保留
  - 以前做电话 Agent，得搭 Twilio + WebRTC Bridge + 自己的媒体服务器。现在 LiveKit 一条龙。
4. MCP 工具调用：真正的「动手」能力
  - 语音 Agent 只会说话，跟 Siri 没区别。生产级 Agent 需要能查订单、写数据库、调 API。
  - LiveKit Agents 原生支持 `MCP`（Model Context Protocol），一行代码接入 MCP Server 暴露的工具
  - 工具调用结果流式反馈到对话中——用户说「查一下我的订单」，Agent 在查询期间会说「好的，正在帮您查询……」而不是沉默 3 秒。
5. 多 Agent Handoff
  - 支持同级 Agent 之间的上下文保留切换。上一个 Agent 的对话历史、用户身份、已查询数据全部传递给下一个 Agent，用户完全无感知。
6. 内置测试与 LLM Judge
  - LiveKit Agents 提供内置的测试框架：模拟用户说话 → Agent 应答 → 用 LLM Judge 打分。评估维度包括：响应的正确性、语气匹配度、工具调用是否合理、延迟是否超标。
7. 自托管完技术栈

```py
# 接入 MCP 工具只需一行
mcp_tools = await ctx.connect_mcp_server("my-mcp-server")
```

架构核心：Agent as Participant（**Agent 即参与者**）。

LiveKit Agent 生命周期：
1. Worker 进程常驻运行，等待调度信号
2. 用户发起通话（通过 App、浏览器或电话拨入），LiveKit 服务器创建一个 Room
3. Worker 收到调度请求，启动一个 Job 子进程
4. Job 中的 AgentSession 加入 Room，成为 WebRTC 参与者
5. Agent 订阅用户的音频轨道 → 送入 STT → LLM → TTS → 发布自己的音频轨道
6. 整个流程是流式并行的——LLM 还在生成后面的词，TTS 已经在播前面的词了
...
建议
- 场景一：做语音 Agent MVP，月通话量 < 1 万分钟。
  - 用 Vapi 或 Retell AI 起步，2-3 小时就能跑通。先验证产品市场契合度，别在基础设施上花 2-6 周的工程时间。
- 场景二：语音 Agent 月通话量突破 1 万分钟，成本开始肉疼。
  - 切 LiveKit Agents 的最佳时机。单通成本从 $0.10-0.15 降到 $0.05-0.08，按一万分钟算就是每月省 $500-700。
- 场景三：数据合规硬需求，或深度定制 Agent 行为。
  - 直接上 LiveKit Agents 自托管。医疗、金融、政务领域的数据不能出机房，而 LiveKit 的 Apache 2.0 协议让全栈私有化部署成为可能。同时，框架的 Python SDK 提供足够的扩展点——自定义 VAD 策略、自定义 LLM 路由、自定义工具调用流程——这些在托管平台上做不到。

### 【2026-8-28】PhoneLLM Alpha 语音客服

【2026-8-28】[Pipecat](https://www.pipecat.ai/) 刚发布 PhoneLLM Alpha 1，就是冲着这种场景来的。

为什么要单独做语音 Agent LLM？

当前 LLM 领域缺少同时满足以下两点的模型：(1) 低延迟，(2) 工具调用准确。

Pipecat 判断:
- 通用模型在开启 reasoning 后容易拉高延迟；
- 而关闭 thinking 后，多轮工具调用又更容易出错，比如嘴上说“已经订好了”，实际并没有调用对应工具。

PhoneLLM 专门针对无 thinking 的工具调用和长对话训练。微调后的小模型针对特定用途进行微调，在准确性、推理速度、成本和数据隐私方面均优于通用的前沿模型。

基于 [NVIDIA Nemotron 3 Nano 30B-A3B](https://huggingface.co/nvidia/NVIDIA-Nemotron-3-Nano-30B-A3B-BF16), 用 [NVIDIA NeMo 框架](https://docs.nvidia.com/nemo-framework/index.html)训练 全参数微调，采用 MoE 架构，实际激活约 3.5B 参数。

团队用大量真实的电话客服、金融、医疗、零售、酒店等通话数据进行训练，重点让模型在关闭 thinking 的情况下，依然能准确调用工具、保持多轮对话连贯，同时说话简洁、不跑题。

同时发布 PhoneBench，评估电话 Agent 的对话质量、工具调用、**say/do 一致性、事实 grounding、认证与升级处理**等指标，并把延迟和运行成本也纳入评估。

PhoneBench 上，和 `GPT-5.6 Terra` 几乎打平，72.3% vs 72.4%；但 P95 首 token 延迟快约 1300ms，成本大约只有 1/18。放到完整的语音链路里，延迟也能压到自然对话可以接受的范围。

![](https://modelscope.cn/models/pipecat-ai/phonellm-alpha-1/resolve/master/images/01-phonebench-v1-leaderboard.png)

部署上支持 vLLM、SGLang。
- 官方 B200 环境中，单请求 TTFT P95 低于 100ms；
- 另外也提供 NVFP4 量化版本。这里的延迟表现依赖具体硬件和并发配置，不能直接套到其他 GPU。

权重已经开源, 可以自建，一键部署到 Modal。
- Hugging Face：[pipecat-ai/phonellm-alpha-1](https://huggingface.co/pipecat-ai/phonellm-alpha-1)，BSD 许可
- modelscope [phonellm-alpha-1](http://modelscope.cn/models/pipecat-ai/phonellm-alpha-1)
- 演示视频 [X帖子](https://x.com/kwindla/status/2093014818647339026)

客服电话这种又要快、又要自然还要真能办事的场景，专用小模型开始显得比通用大模型更对路了。https://x.com/kwindla/status/2093014818647339026

评论
- 语音 agent 上线别只看延迟和价格，还要压测三项：**打断恢复、工具调用确认语、噪声环境下的拒答边界**；这几项决定客服场景能不能用
- 1/18的成本，有点意思。但voice agent的关键还是turn-taking的**自然度**，模型小容易尬聊。等个实测音频再吹。
- PhoneLLM 开源模型确实戳中了语音代理行业最痛的神经。现在各家都在卷**大模型参数和推理能力**，但真正落地到电话客服这种场景，延迟就是生死线，客户等不起3秒钟的思考时间。Pipecat 用30B参数的MoE模型，在关闭思考的模式下专攻多轮工具调用，硬是把延迟做到GPT-5.6
