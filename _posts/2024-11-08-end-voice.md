...
它是 NVIDIA Nemotron 3 Nano 30B-A3B 的全参数微调版，采用 MoE 架构，实际激活约 3.5B 参数。团队用大量真实的电话客服、金融、医疗、零售、酒店等通话数据进行训练，重点让模型在关闭 thinking 的情况下，依然能准确调用工具、保持多轮对话连贯，同时说话简洁、不跑题。

在 PhoneBench 上，它和 GPT-5.6 Terra 几乎打平，72.3% vs 72.4%；但 P95 首 token 延迟快约 1300ms，成本大约只有 1/18。放到完整的语音链路里，延迟也能压到自然对话可以接受的范围。

权重已经开源在 Hugging Face：pipecat-ai/phonellm-alpha-1，BSD 许可，可以自建，也可以一键部署到 Modal。
