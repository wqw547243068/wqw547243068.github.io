...
categories: 机器学习
tags: 机器学习 AUC PR 混淆矩阵 NCE 评分卡 评价 互联网金融 损失函数 kappa 马修斯 mcc 指标 mae elo 竞技 博弈 精确 召回
author : 鹤啸九天
...
【2026-9-17】Precision / Recall 目标设置

P/R上限理论上都是 1.0，但现实里两者互相拉扯，能同时逼近 1.0， 说明问题太简单

**核心结论：没有通用标准，取决于业务场景中"误报"和"漏报"的代价谁更高。**

> **先问"漏一个和错杀一个，哪个更贵"，答案就是目标偏哪边。** 不谈业务场景直接定 0.9 / 0.9，是耍流氓——疾病筛查 Recall 0.95 是底线，垃圾邮件 Precision 0.95 是起点，场景不同，数字含义天差地别。

筛查场景下，双 0.95 不合适——Recall 0.95 合理，Precision 0.95 通常是过度设计。

判断依据只有一个：筛查结果后面有没有人工复核。

一、按代价矩阵决定偏重哪个

| 场景类型 | 漏报（FN）代价 | 误报（FP）代价 | 策略 | 典型参考线 |
|---------|--------------|--------------|------|-----------|
| **疾病筛查** | 极高（漏诊致命） | 低（多做一次检查） | 保 Recall，牺牲 Precision | R ≥ 0.95+，P 可低至 0.3-0.5 |
| **垃圾邮件过滤** | 低（多看一封垃圾邮件） | 高（漏掉重要邮件） | 保 Precision，牺牲 Recall | P ≥ 0.99，R 可接受 0.85 |
| **欺诈检测** | 高（漏一笔损失大） | 中（人工审核成本） | 权衡，略偏 Recall | P ≈ 0.7-0.8，R ≈ 0.8-0.9 |
| **推荐系统** | 低（漏推一条无感） | 中（推错影响体验） | 权衡，偏 Precision | P ≈ 0.8-0.9，R 不强求 |
| **人脸解锁** | 中（解锁失败重试） | 极高（误解锁安全风险） | 极端保 Precision | P ≈ 0.9999+，R ≈ 0.9 |
| **内容审核** | 高（违规内容外泄） | 中（误杀可申诉） | 偏 Recall | R ≥ 0.95，P ≈ 0.7-0.8 |
...
- **Ratio >> 1**（漏报贵）→ 目标偏 Recall，如医疗、安全审核
- **Ratio << 1**（误报贵）→ 目标偏 Precision，如支付风控、安全解锁
- **Ratio ≈ 1** → 看综合指标（F1 或 F-beta）

② 样本不平衡程度

| 正样本比例 | 影响 |
|-----------|------|
| 极低（< 1%） | Precision 天然难做高，0.3-0.5 已可接受；Recall 目标可设 0.7-0.8 |
| 中等（5%-20%） | P、R 双 0.8+ 可行 |
| 接近均衡（40%-60%） | P、R 双 0.9+ 可追求 |

> 样本越不平衡，Precision 越难拉高——因为负样本基数大，少量 FP 就把 Precision 拉下来。

③ 下游是否有人工兜底

- **有人工复核**（如欺诈先机审、后人审）→ 偏 Recall，让人工来纠 Precision
- **全自动决策**（如自动封号）→ 偏 Precision，避免误伤


实操建议

设目标的 4 步法
1. **算 Cost Ratio**：量化漏报 vs 误报的业务损失
2. **看基线**：现有系统的 P、R 是多少，作为起点
3. **设约束优化**：
   - 固定一个，优化另一个：如 `Recall ≥ 0.95` 下 `max(Precision)`
   - 或设 F-beta（β > 1 偏 Recall，β < 1 偏 Precision）
4. **A/B 验证**：用真实业务指标（损失金额、用户投诉率）校准
...
| β 值 | 含义 | 适用 |
|------|------|------|
| β = 2 | Recall 权重 ×4 | 医疗、安全审核 |
| β = 1 | 等权（即 F1） | 通用 |
| β = 0.5 | Precision 权重 ×4 | 垃圾邮件、支付风控 |
...
- `混淆矩阵`中有着Positive、Negative、True、False的概念，其意义如下：
...
- AUC, 即**ROC曲线下的面积**，而ROC曲线的横轴是FPRate，纵轴是TPRate，当二者相等时，即y=x，如下图:
...
在统计和机器学习中，常用 AUC 评估**二分类**模型的性能。

### AUC 定义

AUC 全称: area under the curve，即曲线下的面积。
- 曲线指的是[受试者操作曲线(Receiver operating characteristic, ROC)](https://en.wikipedia.org/wiki/Receiver_operating_characteristic)。
- 相比于`准确率`、`召回率`、`F1值`等依赖于判决**阈值**的评估指标，AUC 没有这个问题。
...
结合上面式子可知TPR和FPR相等，对应的ROC曲线是一条直线！
...
【2024-9-27】[信用评分卡（A卡/B卡/C卡）的模型简介及开发流程](https://blog.csdn.net/varyall/article/details/81173326)

银行、消费金融公司等各种贷款业务机构，普遍使用信用评分，对客户实行打分制，以期对客户有一个优质与否的评判。

不是所有人都知道信用评分卡还分A,B,C卡三类
- `A卡`（Application score card）**申请**评分卡
- `B卡`（Behavior score card）**行为**评分卡
- `C卡`（Collection score card）**催收**评分卡

评分机制区别：
1. 使用时间不同。分别侧重**贷前**、**贷中**、**贷后**；
2. 数据要求不同。`A卡`一般可做贷款**0-1年**的信用分析，`B卡`则是在申请人有了一定行为后，有了较大数据进行的分析，一般为**3-5年**，`C卡`则对数据要求更大，需加入催收后客户反应等属性数据。
3. 每种评分卡的模型会不一样。在`A卡`中常用的有`逻辑回归`，`AHP`等，而在后面两种卡中，常使用`多因素逻辑回归`，精度等方面更好。
