...
Grouped-Query Attention：对于更大参数量、更大的 context length、更大的 batchsize 来说，原始的`MHA`（multi-head attention）内存占用会更高（因为在计算时要缓存pre token的K、V矩阵）。
...
- `GQA-H`：**组数=头数**，基本上与 Multi-Head Attention (`MHA`) 相同。
...
$ I_i^{(r)} = \operatorname{TopK}_{b \in \{1,\dots,B\}}\big(M_{i,\cdot}^{\mathrm{idx},(r)}, k\big) $
