...
![](https://pic2.zhimg.com/v2-db5dd504b1ded672d1292309c7f4f67b_1440w.jpg)
...
草稿模型选型注意
- ✅ 分词器、vocab 完全一致
- ✅ RoPE / 位置编码、special token 对齐
- ✅ 参数规模为主模型 1/10~1/100
- ✅ 同训练分布 / 蒸馏微调，实测接受率≥40%
- ✅ dtype、并行配置和 vLLM 兼容
- ✅ Draft 生成速度显著快于主模型单步推理

（1）硬性要求（不满足直接报错 / 无法运行）
1. Tokenizer 词表必须完全一致（最高优先级）
  - 草稿模型和目标模型`分词器`、`词表` vocab 完全一模一样，token ID 一一对应，不能混用不同分词器。
  - 原因：目标模型并行校验多 token 时依靠 token 下标对齐；词表不一致会出现 ID 错乱、校验逻辑崩坏、生成乱码、接受率异常。
  - 红线：Llama 不能用 Qwen 草稿、BERT 不可搭配 GPT，跨家族混用基本不可行。
  - vLLM 行为：词表不匹配会抛出警告，运行时校验错乱，加速完全失效。
2. 基础模型架构兼容（Transformer 范式对齐）
  - 主流仅支持标准**自回归 Transformer**（LLaMA、Qwen、OPT、Mistral 等），必须和目标模型都是 Causal LM 因果解码器。
  - 编码器（BERT）、编码器 - 解码器（T5）不能当 Draft；
  - EAGLE、MTP 这类专用 Draft 属于定制结构，需要 vLLM 专门适配加载，不能随便用普通小模型替代vLLM。
3. 推理框架加载兼容（vLLM 专属约束）
  - 权重格式支持 Hugging Face 标准格式；自定义训练的 Draft 需要合规保存 config.json、tokenizer 文件。
  - 张量并行配置：speculative_draft_tensor_parallel_size ≤ 主模型tensor_parallel_size，否则多卡 OOM、卡推理；
  - 精度建议统一：Draft 与目标模型 dtype 一致（bf16/fp16），混用精度容易出现分布偏移、接受率下跌；
  - 显存必须能同时放下双模型，显存不足可把 Draft 放 CPU，但会拉高 Draft 生成耗时、抵消加速收益。

（2）性能要求（决定加速效果，无硬性报错，但直接影响提速倍数）
1. 参数量**远小于**目标模型，推理速度足够快
  - 尺寸区间经验标准，Draft 参数量 = 目标模型 1/10 ~ 1/100，太大耗时长，太小能力弱
  - Draft 串行生成 γ个 token 的耗时 ＜ 目标模型串行跑γ次单 token 耗时
  - 70B 主模型 → 推荐 0.7B~7B 草稿；
  - 7B 主模型 → 100M~1B 草稿
2. 输出概率分布高度贴近目标模型（最关键指标：`接受率`）
  - 加速上限完全由 token 接受率决定，理想接受率≥60%，最低可接受底线≥30%。
3. 上下文对齐逻辑一致
  - 同样采用 causal mask、相同位置编码（RoPE/ALiBi/Nope 完全一致）；RoPE 缩放参数不同会导致长文本分布严重错位；
  - 输入上下文处理逻辑一致，prompt 模板、特殊 token（<bos>、<eos>、终止符）完全对齐，否则上下文理解不一致，猜测全错。

（3）KV 缓存与运行时隐性要求
- Draft 需要独立 KV 缓存，支持逐 token 自回归缓存；目标模型支持 KV 回滚（拒绝 token 时切回上一合法位置）；
- 支持和主模型共用 vLLM 块管理器（Block Manager），方便显存复用；
- 量化兼容：Draft 可单独做 GPTQ/AWQ 量化，但量化不能破坏概率分布；极端 4bit 量化容易分布跑偏、接受率下降
...
- 解读 [阶跃开源 JetSpec，解码提速近 10 倍](https://zhuanlan.zhihu.com/p/2055406615193560568)
...
![](https://pic3.zhimg.com/v2-131d8b24b13f8cd3df63c7d1f003cddc_1440w.jpg)
