...
DeepSpeed 参数集包括以下内容：
- 1) `–deepspeed`：启用 DeepSpeed 的布尔标志 
- 2) `–deepspeed_config` `<json 文件路径>`：配置 DeepSpeed 运行时的 json 配置文件的路径。


#### 新增自定义参数

DeepSpeed 使用 argparse 库来为 DeepSpeed 运行时提供命令行配置。

使用 `deepspeed.add_config_arguments()` 将 DeepSpeed 内置参数添加到应用程序的解析器中。
...
| ------------ | ----------------------- | ----------------------- | ----------------------- |
| 分片对象     | 优化器状态              | 优化器状态 + 梯度       | 参数 + 梯度 + 优化器状态 |
| 显存占用     | $N \times (P + G) + \frac{O}{N}$ | $N \times P + \frac{G + O}{N}$ | $\frac{P + G + O}{N}$ |
| 显存节省     | 中等                    | 高                      | 最高                    |
| 通信开销     | 低（仅同步优化器状态）  | 中等（同步梯度）        | 高（同步参数、梯度）    |
| 适用模型规模 | 中小型模型              | 中大型模型              | 超大规模模型            |
| 硬件要求     | 单机多 GPU 或低带宽集群 | 分布式集群              | 高带宽分布式集群        |
| 典型应用场景 | 优化器状态占显存主导    | 梯度和优化器状态占主导  | 参数占显存主导          | 
...
总结
- ✅ ZeRO‑1、ZeRO‑2：纯数据并行；
- ✅ ZeRO‑3：**数据并行的增强版本，不是标准模型并行**，虽然参数被切分到多卡，但计算逻辑仍是每张卡完整执行模型。


|阶段|分片对象|每张卡计算行为|并行类别|
|---|---|---|---|
|ZeRO‑1|仅优化器状态|参数、梯度完整复制，每张卡跑完整前向/反向|**数据并行DP**|
|ZeRO‑2|优化器状态 + 梯度|参数完整复制，每张卡跑完整前向/反向|**数据并行DP**|
|ZeRO‑3|优化器状态 + 梯度 + 参数|参数分片存储；计算时AllGather临时拿到完整层参数，算完释放；每张卡依然独立跑完整计算图|**增强型数据并行DP（参数分片DP）**|

关键区分
1. **传统模型并行 TP/PP**：把**计算图切开**，每张卡只负责一部分网络层的计算，一张卡永远算不完完整forward。
2. **ZeRO（哪怕stage3）**：**计算图完整不变**，每张卡都可以独立跑完整前向反向；只是权重/梯度/优化器状态不常驻显存，按需通过通信拉取参数。没有把网络计算任务拆分给不同GPU。

通俗：
> - TP/PP：活分给不同卡干；
> - ZeRO‑3：活每张卡都全干，只是权重平时不全部放自己卡上，要用再临时借过来。

工程上

ZeRO可以和TP/PP组合使用（3D并行）：节点内TP，节点间ZeRO‑3数据并行。
- ZeRO‑1/2：完全不碰参数，和普通DDP行为接近；
- ZeRO‑3只是**存储层面分片参数**，不属于张量/流水线模型并行。
