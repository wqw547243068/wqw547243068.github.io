...
### 【2026-6-29】SupraSafety-18M

18m安全审核模型 

【2026-6-29】文章 [70MB 就能跑的内容审核模型。18M 参数，最小开源安全审核模型发布](https://mp.weixin.qq.com/s/WE3OB6wNiqpzjQDA9-kuLg)

5 人非营利研究小组 [SupraLabs](https://huggingface.co/SupraLabs)，不追求模型越大越好，而是专注于在消费级硬件上构建实验性的 AI 系统，刻意把算力需求压到最低，让更多人用得上。

在 HuggingFace 上，一共托管了 23 个模型，涵盖：
- 文本生成、表格数据分类和多模态任务，
- 此外还有一个天气预测模型 SupraWeather1.5-Small、50M 参数级别的通用模型 Supra-1.5-50M 的多个版本、10M 参数的故事生成模型 StorySupra-10M，

以及一系列对话标题生成模型。

这次发布仅 18M 参数的 SupraSafety-18M，从零训练、70MB 大小、能在边缘设备跑，F1 达 85.9%。并且完全开源。
- 用户输入文本，判断安全还是不安全，然后输出一个概率分数
- [SupraSafety-18M](https://huggingface.co/SupraLabs/SupraSafety-18M)

模型采用自定义的 BERT-like 架构（一种经典的文本分类神经网络结构），总共 6 层、8 个注意力头、512 维的隐藏层，18,264,578 个参数，整个权重文件只有 70MB。

- Google ShieldGemma 基于 Gemma 大模型构建，参数量级在数十亿；
- NVIDIA 的 Llama Nemotron Safety Guard v2 同样是重量级选手；
  - [Nemotron-3.5-Content-Safety-Dataset](https://huggingface.co/datasets/nvidia/Nemotron-3.5-Content-Safety-Dataset)
  - [safeguard-agentic-ai-systems-with-the-nvidia-safety-recipe/](https://developer.nvidia.cn/blog/safeguard-agentic-ai-systems-with-the-nvidia-safety-recipe/)
- 即便是市面上相对轻量的 NeMo Guard 系列，也需要 8B（80 亿）参数打底。

而 SupraSafety-18M 的 18M，差不多是它们的两千分之一。

效果如何？
- 准确率 81.2%，精确率 86.9%，召回率 84.9%，F1 分数 85.9%。

模型的最大处理长度是 512 个 token（大约相当于三四百个英文单词），对于简单对话还好。
