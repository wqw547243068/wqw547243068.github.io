...
PromptWizard 利用**自我进化**、**自我适应**的机制，通过反馈驱动的批评和综合过程，在探索和开发之间实现了有效平衡，迭代地优化提示指令和上下文示例，生成可读、特定任务的提示。

这种引导式方法系统地提高了提示质量，此外，成本分析显示API调用、token使用和总成本大幅减少。

微软开源提示词优化框架：[PromptWizard](https://microsoft.github.io/PromptWizard/)，采用**自我进化**机制，让模型自己生成、评价和改进提示词及生成示例，通过不断反馈来提高输出质量。
...
PromptWizard 效果超过: [img](https://github.com/microsoft/PromptWizard/raw/main/images/curve.png)
- APE
- InstructZero
- PromptBreeder
- EvoPrompt
...
![](https://github.com/microsoft/PromptWizard/raw/main/images/curve.png)
...
| mutation_rounds | 生成多风格提示词时，执行**变异**操作的总轮数 |
...
1. `mutate_refine_iterations`：先对任务描述做变异生成、再对指令进行精调的迭代轮数
2. `mutation_rounds`：生成多种风格提示词时，执行变异操作的总轮次
3. `refine_task_eg_iterations`：同步精调任务描述与上下文示例的迭代次数
4. `style_variation`：提示词变异环节产出的不同思考风格变体总数
5. `questions_batch_size`：训练步骤中，单次批量向大模型输入的问题数量
6. `min_correct_count`：一条提示词被认定为效果良好，所需答对的最少问题批次数量
7. `max_eval_batches`：评估当前提示词性能时，最多使用的小批量样本数
8. `top_n`：打分阶段筛选出排名靠前的最优提示词数量，进入下一轮迭代
9. `seen_set_size`：训练阶段从训练集中抽取使用的样本数量
10. `few_shot_count`：最终生成提示词里需要搭载的上下文示例条数
...
| ---- | ---- | ---- | ---- |
| 场景一：无示例提示词优化 | 无任何训练数据 | `use_examples: false`<br>`mutate_refine_iterations: 2` | 仅优化指令文本；基于`prompt_library.yaml`模板生成多版指令变体，迭代速度更快 |
| 场景二：自动生成训练示例 | 现有训练数据量少、小样本场景 | `generate_synthetic_examples: true`<br>`num_train_examples: 20` | 分析已有样本特征，自动生成20条多样化合成训练示例补充数据集 |
| 场景三：完整训练数据优化 | 拥有充足标注数据（如SVAMP等标准数据集） | `run_without_train_examples: false`<br>`seen_set_size: 50` | 同时优化提示指令+配套训练示例，利用50条训练样本的正反例对比持续调优Prompt效果 |
...
- 论文 [Self-Supervised Prompt Optimization](https://arxiv.org/pdf/2502.06855)
- 代码 [SPO](https://github.com/FoundationAgents/SPO)
