...
- [CPU inference](https://huggingface.co/docs/transformers/v4.49.0/en/perf_infer_cpu) 上，提供
  - [BetterTransformer](https://pytorch.org/blog/a-better-transformer-for-fast-transformer-encoder-inference/) 推理加速，pytorch代码转torchscript
  - Intel CPU: 使用 [pytorch intel扩展](https://intel.github.io/intel-extension-for-pytorch/cpu/latest/index.html)里的 [graph-optimization](https://intel.github.io/intel-extension-for-pytorch/cpu/latest/tutorials/features.html#graph-optimization)
...
英特尔CEO帕特·基辛格2023年底：
>- “从经济学角度看推理应用，我不会打造一个需要花费**四万美元**的全是`H100`的后台环境，因为它耗电太多，并且需要构建新的管理和安全模型，以及新的IT基础设施。”
>- “如果我能在标准版的英特尔芯片上运行这些模型，就不会出现这些问题。”

京东云推出搭载第五代英特尔® 至强® 可扩展处理器的新一代服务器
- 与相同内置AI加速技术（AMX，高级矩阵扩展）的前一代（第四代至强® 可扩展处理器）相比，深度学习实时推理性能提升高达**42%**；
- 与内置上一代AI加速技术（DL-Boost，深度学习加速）、隔辈儿（第三代至强® 可扩展处理器）相比，AI推理性能更是最高提升至**14倍**。
...
![](https://pic3.zhimg.com/v2-a2480def601545c99bdd325d65458766_1440w.jpg)
...
【2024-3-27】[拿CPU搞AI推理，谁给你的底气？](https://zhuanlan.zhihu.com/p/689335405)


### 部署框架

【2024-11-16】[如何选择 LLM 部署框架](https://zhuanlan.zhihu.com/p/7319336605)

LLM 推理部署的生态有很多开源框架，例如: `vLLM`、`LMDeploy`、`TGI`

选择依据
- **模型**是否支持：优先从已支持的框架中选择
  - 现在的模型结构相对统一，所以如果想要在某个框架中支持，难度也不会特别大。
- 框架生态的**活跃度**：例如 vLLM、LMDeploy 的生态就很活跃，框架一直持续添加新功能以及支持新的模型
- 框架的**服务稳定性**：基础要求，即模型部署好之后，能否稳定地处理用户的请求，而不会时常报错，例如 OOM、Segmentation fault 等
- 框架性能的**评价指标**：例如首 token 延迟（Time to First Token, TTFT）、每秒处理的 token 数（Tokens Per Second, TPS）等


## 评价指标

评价框架性能的[常用指标](https://docs.nvidia.com/nim/benchmarking/llm/latest/metrics.html)：
- `TTFT` (Time to First Token)：`首 token 延迟`，即用户发出请求后看到第一个 token 被输出的时间，如图 1 所示：
  - ![](https://pica.zhimg.com/v2-e5a2eccca5d98e93943383744f6e45f8_1440w.jpg)
- `E2EL` (End-to-End Request Latency)：用户发出请求并得到完整回复的延迟，如图 2 所示：
  - ![](https://picx.zhimg.com/v2-08de6421ed50a4aeb93da224cd311669_1440w.jpg)
- `ITL` (Inter-token Latency)：`非首 token 延迟`，也称为 TPOT (Time Per Output Token)，如图 3 所示：
  - ![](https://pic2.zhimg.com/v2-b7cd14b6bb769ebed7695060a2e05275_1440w.jpg)
  - 计算公式是 
  - 注意: 实际评测时，ITL 和 TPOT 的计算方式会有些许差别。
- `TPS` (Tokens Per Second)：`每秒处理 token 数`。在介绍如何计算 TPS 前，先简单介绍计算涉及的参数，如图 4 所示：
  - ![](https://pic4.zhimg.com/v2-115419771e7bad4fc908ac4d3d7fad01_1440w.jpg)
...
评测指南
- 见原文 [如何选择 LLM 部署框架](https://zhuanlan.zhihu.com/p/7319336605)
...
[netron](https://netron.app/) 库 可视化导出的**静态模型**和**动态模型**
- [netron online](https://netron.app/) 在线体验
- 源码 [server.py](https://github.com/lutzroeder/netron/blob/main/source/server.py)

yolov3-tiny的[onnx模型](https://github.com/onnx/models/tree/master/vision/object_detection_segmentation/tiny-yolov3/model),`Netron`可视化
...
- ONNX Runtime 可**大大提高多个模型性能**。

onnxruntime 对 onnx 模型提供**推理加速**，支持CPU、GPU加速
- GPU加速版本为 `onnxruntime-gpu`
- 默认版本为CPU加速。

人脸检测 ULFD 模型上
- 未用 onnxruntime 加速，对于320x240分辨率的图片，在CPU上需要跑要**50~60ms**;
...
ONNXRuntime 与 CUDA[版本对应](https://blog.csdn.net/qq_38308388/article/details/137679214)
- cuda 12.2 版本对应 1.17
...
转换
- 将 HuggingFace 上的标准模型（如 LLaMA）转换为 GGUF 格式（使用 llama.cpp 工具链）
...
MLX（Apple Machine Learning eXtension）
...
特点
- 针对苹果芯片优化（M1/M2/M3 等）。
- 加载 PyTorch 模型，并进行本地推理。

常见用法
- 使用 mlx 库加载并运行模型（如 LLaMA、TinyLlama 等）
- 将 GGUF 格式的模型转换为 MLX 格式运行
