...
四代演进核心脉络
1. **Qwen2 → Qwen2.5**：上下文从32K扩至128K，数学/代码/结构化能力显著增强，闭源推出MoE版API。
2. **Qwen2.5 → Qwen3**：首次引入MoE架构，上下文扩至256K，评测全面追赶闭源模型，Thinking能力初步成型。
3. **Qwen3 → Qwen3.5**：MoE专家扩容4倍+共享专家，原生多模态统一预训练，Thinking默认深度思考，评测全面超越GPT-5.2/Claude 4.5，成为开源新标杆。
