...
Qwen 长期目标：打造“全能统一模型”，能说、能看、能听、能写的智能体。
...
各个版本部署总结
- [参考](https://www.zhetao.com/content1638)

| 模型版本       | 参数规模 | CPU要求       | 内存要求                  | 存储要求                                  | GPU要求                                  | CPU部署 | 测试环境                              | 纯CPU推理速度 | 单套硬件成本（仅硬件） |
|----------------|----------|---------------|---------------------------|-------------------------------------------|------------------------------------------|---------|---------------------------------------|----------------|------------------------|
| Qwen-0.6B      | 6亿      | 4核心以上     | 16GB以上（DDR4以上）      | M2 SSD 64GB以上，不建议普通HDD/SSD         | 可纯CPU推理                               | 支持    | i5+8GB内存+2TB M2 SSD                 | 15 Tokens/秒   | 0.6万左右              |
| Qwen-1.7B      | 17亿     | 8核心以上     | 32GB以上（DDR4以上）      | M2 SSD 256GB以上，不建议普通HDD/SSD        | 可纯CPU推理                               | 支持    | i7+64GB内存+2TB M2 SSD                | 20 Tokens/秒   | 0.8万左右              |
| Qwen-4B        | 40亿     | 8核心以上     | 32GB以上（DDR4以上）      | M2 SSD 256GB以上，不建议普通HDD/SSD        | 可纯CPU推理；推荐8GB以上GPU（3050、3060等）| 支持    | i7+64GB内存+2TB M2 SSD                | 12 Tokens/秒   | 1.5万以上              |
| Qwen-8B        | 140亿    | 12核心以上    | 64GB以上（DDR4以上）      | M2 SSD 512GB以上，不建议普通HDD/SSD        | 可纯CPU推理；推荐16GB以上GPU（3090、4080等）| 支持    | i7+64GB内存+2TB M2 SSD                | 5 Tokens/秒    | 5万以上                |
| Qwen-14B       | 140亿    | 16核心以上    | 96GB以上（DDR4以上）      | M2 SSD 512GB以上，不建议普通HDD/SSD        | 推荐A10、A16；条件允许用A100              | 支持    | i7+64GB内存+2TB M2 SSD                | 1 Tokens/秒    | 30万以上               |
| Qwen-32B       | 320亿    | 20核心以上    | 128GB以上（DDR4以上）     | M2 SSD 1TB以上，不建议普通HDD/SSD          | 推荐A100×2以上、H2等                      | 支持    | 未提及                                | 未提及         | 40万以上               |
| Qwen3-235B     | 2350亿   | 48核心以上    | 768GB以上（DDR5以上），推荐1.5TB | M2 SSD 2TB以上，不建议普通HDD/SSD          | 推荐A100×4（4卡/8卡机），显存≥512GB       | 未提及  | 未提及                                | 未提及         | 150万以上              |
