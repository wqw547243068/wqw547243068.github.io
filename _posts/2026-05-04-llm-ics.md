...
excerpt: 大模型智能客服应用案例分析, Usecase of Intelligent Customer Service agentscope
author: 鹤啸九天
mathjax: true
permalink: /llm_ics
---
...
客服电话这种又要快、又要自然还要真能办事的场景，专用小模型开始显得比通用大模型更对路了。

[X帖子](https://x.com/kwindla/status/2093014818647339026)



### 【2026-7-9】customer_work

【2026-7-9】[基于 AgentScope2.0 智能体客服后台应用系统作品展示](https://mp.weixin.qq.com/s/BTp7b2nDkNo3T2SJ7W7Hfg)

基于 AgentScope 2.0-RC4 构建；
- 前端模块是 customer-admin-web, 先 `npm install` ，然后 `npm run dev` 即可看到可访问的前端地址。
- 后端是 customer-admin-server，springboot 直接跑起来 带 `-Dspring.profiles.active=dev` 即可。
- 细节内容查看文档接口。依赖mysql和redis。 用docker启动就可以。
- 支持账号密码登录 账号：admin、admin ，第一次登录需要改密码后重新登录，也可以支持单点登录（LADP方式），需要你配置;

代码：[customer_work](https://github.com/liulangjietou/customer_work/tree/rc2.0)



### 【2026-6-9】DuetBench 自进化客服Agent测试基准

【2026-6-9】[DuetBench: An evaluation of self-improving customer service agents](https://decagon.ai/blog/duetbench)

角色上的根本转变：从“智能体构建者”到“智能体管理者”

现有测试标准仅能衡量agent能否解决特定类型的问题，但无法评估其**自我改进**的能力。目前存在三个主要缺陷：
- 静态测试：针对某个固定状态的解决方案，而不是现实中那种动态的、迭代的处理过程。
- 忽视了进化能力：只关注最终结果，而不考虑自我提升的能力。某个系统可能在静态测试中表现优异，但实际上仍需要持续维护，而这正是企业面临的真正难题。
- 缺乏实际复杂环境：简化环境中运行，无法反映实际生产环境中的情况。其任务主要集中在遵循指令、使用工具上，而不会涉及调试程序中的错误、修正智能体的逻辑问题或对各项变更进行压力测试。

DuetBench 衡量的是 Autopilot 是否真的能够带来可验证的改进效果，而不仅仅是制造出看似合理的表面变化而已。

DuetBench 从四个实验场景来评估 Autopilot 的性能，涵盖智能体开发的整个流程：问题诊断、智能体构建、模拟环境生成以及端到端的测试。

通过这些测试，可以判断 Autopilot 是否能够正确识别问题、构建相应的智能体、验证修改后的结果，并完成整个开发流程

“Duet Autopilot”在 93%的诊断任务中都取得了成功，这一成绩超过了人类的平均水平。

Duet Autopilot：
- 首个经过验证、具备自我改进能力的客服优化工具。
- 首个专为评估客服人员自我提升能力而设计的基准测试工具

始终在线的辅助系统，Autopilot 能将用户在使用过程中的反馈转化为有效的优化方案，经过人工审核后再被应用到实际系统中。

测试阶段，各团队在审核后接受了 93%的优化方案，这充分证明了 Autopilot 能够生成可用于实际应用的优化内容。
