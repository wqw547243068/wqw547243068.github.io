...
严格意义上来说，通过图灵测试并不能代表机器已经具有**自然对话能力**。一篇关于图灵生平的文章提到图灵在 1952 年被判犯有同性恋行为，并被迫接受化学阉割，两年后图灵自杀身亡。

而图灵测试其实就是反映了在上世纪 50 年代的英国，每一位**同性恋**男性必须通过的日常测试：你是否能伪装成一个异性恋者？根据图灵的看法，未来的计算机就像当时的同性恋者，计算机有没有意识并不重要，重要的是人类会怎么想。

即便是图灵测试，也可以看做是一个特定的「**闭域**」，在这个闭域中，聊天的状态是预先可以设计的，有很多的策略可以让对话在这个特定的闭域顺畅的进行下去。而很多聊天机器人厂商给自己挖的坑，是要做「**开域**」（也就是通用域）的聊天。

现有技术条件下相当于给自己的产品判了**死刑**。因为做通用域聊天，就等同于想要模拟人类真实的对话，这在目前是不可能完成的任务。
...
【2025-3-31】GPT-4.5：首个通过图灵测试的大模型 

美国加州大学圣地亚哥分校研究成果，首次提供“人工智能系统能够通过标准三方图灵测试的证据”。

研究分析了四种 AI 系统：GPT-4.5、LLaMa-3.1-405B、GPT-4o 和 ELIZA。

实验设计：
- 每轮对话中，裁判与两个人类和一个AI系统进行对话，以判断何者为人类。

结果：
1. GPT-4.5 以 73% 比率被认作人类，显著超越真实人类参与者
2. LLaMa-3.1-405B 获得 56% 识别率，与人类无显著差异
3. 基线模型（ELIZA和GPT-4o）成功率显著低于随机概率（分别为23%和21%）

图灵测试本质上测量的是**拟人性**（humanlikeness）而非直接智能。

参考
1. 论文链接：[Large Language Models Pass the Turing Test](https://arxiv.org/abs/2503.23674)
2. [新智元报道](https://mp.weixin.qq.com/s/CnNk008iXfa7sqOt3XBBPA)
...
【2023-11-15】几种语言的编译、执行动图, [描述](https://www.toutiao.com/w/1782544482052299)
- ![](https://p3-sign.toutiaoimg.com/tos-cn-i-ezhpy3drpa/2546157e93374dab95a694fb970a953c~tplv-obj:1280:1664.image?_iz=97245&from=post&lk3s=06827d14&x-expires=1707782400&x-signature=7OQRqHdw1bHnzU5qPRg9GsLI0QE%3D)
...
VPL 可视化编程，一种使用**图形组件**的语言，如图标、按钮和符号的编码形式。
- 允许对计算机生成的编码概念进行视觉说明，帮助非技术用户以大多数初学者能够理解的方式解释图表和过程。
- 可视化编程语言还允许用户简单地使用拖放界面，并在低代码平台上最有效地运行。

尽管这种语言很简单，但被证明麻烦，因为有丰富的图形。
- 这种编程语言的尺寸较大，因此在计算机上占用更多的空间，进一步导致功能减慢，因为它在驱动器上需要大量的内存。
- VPLs在其使用的表达功能的范围上非常有限。

更难执行更复杂的操作，因此，这种类型的语言很少被编程界的技术巨头使用。
